...
def render_multi_pdf_customer_input() -> bool:
    ...
    if auto_customer_data and any(auto_customer_data.values()):
        ...
        with col1:
            st.text_input(
                "Kunde", value=auto_customer_data.get(
                    "name", ""), disabled=True)
            st.text_input(
    ...
    with st.form("multi_pdf_customer_form"):
        ...
        with col1:
            name = st.text_input(
                "Kundenname*",
                value=st.session_state.multi_offer_customer_data.get(
                    "name",
                    ""))
        ...
        with col2:
            zip_code = st.text_input(
                "PLZ", value=st.session_state.multi_offer_customer_data.get(
                    "zip_code", ""))
            city = st.text_input(
                "Ort", value=st.session_state.multi_offer_customer_data.get(
                    "city", ""))
            phone = st.text_input(
                "Telefon",
                value=st.session_state.multi_offer_customer_data.get(
...
def render_multi_pdf_company_selection(
        available_companies: list[dict[str, Any]]) -> list[int]:
    """
    ...
    if not available_companies:
        st.warning(
            "⚠️ Keine Firmen verfügbar. Bitte erst Firmen im Admin-Panel anlegen.")
        return []

    st.info(
        f"📊 {
            len(available_companies)} Firmen verfügbar - wählen Sie beliebig viele aus")
    ...
    if col_quick1.button("✅ Alle auswählen"):
        st.session_state.multi_offer_selected_companies = [
            c["id"] for c in available_companies]
        st.rerun()
        ...
        with col_extend:
            if is_selected:
                extend_pdf = st.checkbox(
                    ...
                    value=st.session_state.multi_offer_company_extended.get(
                        company_id,
                        False),
...
def render_multi_pdf_settings():
    ...
    with col2:
        ...
        if settings["price_increment_percent"] > 0:
            st.info(
                f"💡 Firma 1: 100% | Firma 2: {
                    100 + settings['price_increment_percent']:.1f}% | Firma 3: {
                    100 + 2 * settings['price_increment_percent']:.1f}%")
    ...
    if heatpump_enabled:
        ...
        if hp_offer:
            ...
            if selected_wp:
                ...
                st.warning(
                    "⚠️ Keine Wärmepumpe ausgewählt - bitte in Bedarfsanalyse konfigurieren")
        else:
            st.info(
                "ℹ️ Keine Wärmepumpen-Daten vorhanden - wird in Bedarfsanalyse konfiguriert")
...
def create_multi_pdf_zip(
        pdf_paths: dict[str, str], customer_name: str) -> bytes:
    """
            ...
            if os.path.exists(pdf_path):
                ...
                safe_company_name = "".join(
                    c if c.isalnum() or c in (
                        ' ', '-', '_') else '_' for c in company_name)
    ...
    with col_preview:
        st.info(
            f"📊 **Zusammenfassung:**\n- {
                ...
                st.session_state.multi_offer_customer_data.get('name')}\n- Wärmepumpe: {
                '✅ Ja' if heatpump_enabled else '❌ Nein'}")

    with col_generate:
        if st.button(
            "🎯 Alle Angebote generieren",
            type="primary",
