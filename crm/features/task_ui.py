...
def render_task_overview():
    ...
    with col3:
        ...
        st.markdown(f"""
            ...
            ">
                ...
                <p style="margin: 0; opacity: 0.8;">⚠️ Dringend</p>
            </div>
        """, unsafe_allow_html=True)
    ...
    with col3:
        st.metric("⚠️ Überfällig", overdue)
...
def render_task_card(task: dict):
    ...
    with st.container():
        ...
        with col5:
            if st.button("🗑️ Löschen", key=f"delete_{task_id}", use_container_width=True):
...
def render_task_notifications():
    ...
    st.subheader("⚠️ Benachrichtigungen")
