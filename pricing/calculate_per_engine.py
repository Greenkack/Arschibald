...
class CalculatePerEngine:
        ...
        try:
            ...
            if context:
                ...
                if any(marker in context_text for marker in (
                    ...
                    result = self._calculate_per_piece(base_price, 1, context)
                    result.calculation_notes.append(
                        "Storage safety: battery storage priced as one selected product position"
                    )
                    return result
    ...
    def _calculate_per_square_meter(
        ...
        result = CalculationResult(
            ...
            unit_price=unit_price,
            total_price=total_price,
            calculation_factor=area_m2 / quantity if quantity > 0 else 0.0,
            context_used=context
        )
    ...
    def _calculate_per_hour(
        ...
        result = CalculationResult(
            ...
            unit_price=unit_price,
            total_price=total_price,
            calculation_factor=hours / quantity if quantity > 0 else 0.0,
            context_used=context
        )
