...
def render_pv_mounting_selection(
    ...
    if not PV_MOUNTING_DB_AVAILABLE:
        st.warning("PV-Unterkonstruktions-Datenbank nicht verfügbar. Bitte Admin kontaktieren.")
        return
        ...
        with col_calc2:
            if st.button(" Berechnen", key='calc_mounting_quantities'):
                ...
                if module_count > 0:
                    ...
                    st.warning("Bitte zuerst Modulanzahl in Step 1 angeben.")
        ...
        with col2:
            ...
            if details.get(f'pv_mounting_{key}_manufacturer'):
                ...
                if not product_names:
                    st.warning(f"Keine Produkte für '{details[f'pv_mounting_{key}_manufacturer']}' verfügbar.")
