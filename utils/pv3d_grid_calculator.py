...
def calculate_module_grid(
    ...
    if available_length < module_width or available_width < module_height:
        print(f"⚠️ Insufficient space: Available area ({available_length:.2f}m x {available_width:.2f}m) "
    ...
    if actual_modules < module_quantity:
        print(f"ℹ️ Requested {module_quantity} modules, but only {actual_modules} fit on roof")
