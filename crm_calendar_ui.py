...
class CRMCalendar:
    ...
    def render_calendar_interface(self, texts: dict[str, str]):
        ...
        tab1, tab2, tab3 = st.tabs(
            [" Kalenderansicht", " Neuer Termin", " Terminliste"])
    ...
    def _render_calendar_grid(self, current_date: datetime):
        ...
        appointments = self._get_appointments_for_month(
                ...
                with cols[i]:
                    if day == 0:
                        ...
                        if day_appointments:
                            ...
                            # Termine anzeigen
                            # Max 2 Termine anzeigen
                            for apt in day_appointments[:2]:
                                apt_type = self.appointment_types.get(
                                    apt['type'], self.appointment_types['consultation'])
                                st.markdown(
                                    f"<small>{apt_type['icon']} {apt['title'][:15]}...</small>", unsafe_allow_html=True)

                            if len(day_appointments) > 2:
                                st.markdown(
                                    f"<small>+{len(day_appointments) - 2} weitere</small>", unsafe_allow_html=True)
                        else:
                            st.markdown(f"""
    ...
    def _render_new_appointment_form(self):
        ...
        with st.form("new_appointment_form"):
            ...
            with col1:
                title = st.text_input(
                    "Titel *", placeholder="z.B. Beratungstermin Herr Müller")

                appointment_type = st.selectbox(
                    "Termintyp *",
                    options=list(
                        self.appointment_types.keys()),
                    format_func=lambda x: f"{
                        self.appointment_types[x]['icon']} {
                        self.appointment_types[x]['name']}")
            ...
            with col2:
                ...
                location = st.text_input(
                    "Ort", placeholder="z.B. Büro, Kundenadresse")
            ...
            notes = st.text_area(
                "Notizen", placeholder="Zusätzliche Informationen zum Termin")

            submitted = st.form_submit_button(
                " Termin erstellen", type="primary")
    ...
    def _render_appointment_list(self):
        ...
        with col1:
            filter_type = st.selectbox(
                ...
                list(
                    self.appointment_types.keys()),
                format_func=lambda x: "Alle Typen" if x == 'all' else f"{
                    self.appointment_types[x]['icon']} {
                    self.appointment_types[x]['name']}")
        ...
        appointments = self._get_filtered_appointments(
            filter_type, filter_period, filter_status)
    ...
    def _render_appointment_card(self, appointment: dict[str, Any]):
        ...
        apt_type = self.appointment_types.get(
            appointment['type'], self.appointment_types['consultation'])
        ...
        with st.container():
            ...
            with col2:
                ...
                st.markdown(
                    f" {appointment['appointment_date'].strftime('%H:%M')} Uhr")
    ...
    def _get_appointments_for_month(
            self, year: int, month: int) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                appointment = {
                    ...
                    'customer_name': f"{
                        row[12] or ''} {
                        row[13] or ''}".strip() if row[12] or row[13] else None}
    ...
    def _get_filtered_appointments(self,
                                   filter_type: str,
                                   filter_period: str,
                                   filter_status: str) -> list[dict[str,
                                                                    Any]]:
...
def render_crm_calendar(texts: dict[str, str], module_name: str | None = None):
