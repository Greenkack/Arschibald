...
class EnhancedPDFGenerator:
    ...
    def _generate_pricing_keys(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Generated {len(self.pricing_keys)} pricing keys for PDF")
    ...
    def _process_pricing_data(self) -> None:
        ...
        try:
            ...
            if hp_pricing:
                ...
                logger.debug(
                    f"Generated {
                        len(hp_keys)} heat pump pricing keys")
            ...
            if combined_pricing:
                ...
                logger.debug(
                    f"Generated {
                        len(combined_keys)} combined pricing keys")
    ...
    def _process_analysis_results(self) -> None:
        ...
        try:
            ...
            if pricing_relevant:
                ...
                logger.debug(
                    f"Generated {
                        len(analysis_keys)} analysis pricing keys")
        ...
        try:
            ...
            if 'selected_module' in self.project_data:
                ...
                if isinstance(module_data, dict):
                    components['module_price'] = module_data.get(
                        'price_euro', 0)
                    components['module_quantity'] = self.project_data.get(
                        'module_quantity', 0)
            ...
            if 'selected_inverter' in self.project_data:
                ...
                if isinstance(inverter_data, dict):
                    components['inverter_price'] = inverter_data.get(
                        'price_euro', 0)
                    components['inverter_quantity'] = self.project_data.get(
                        'inverter_quantity', 1)
            ...
            if 'selected_storage' in self.project_data:
                ...
                if isinstance(storage_data, dict):
                    components['storage_price'] = storage_data.get(
                        'price_euro', 0)
                    components['storage_quantity'] = self.project_data.get(
                        'storage_quantity', 1)
            ...
            if components:
                ...
                logger.debug(
                    f"Generated {
                        len(project_keys)} project component keys")

        except Exception as e:
            logger.error(f"Error processing project data: {e}")

    def _get_keys_by_category_safe(
            self, category: KeyCategory) -> dict[str, Any]:
    ...
    def generate_pricing_breakdown_data(self) -> dict[str, Any]:
        ...
        try:
            ...
            component_keys = self._get_keys_by_category_safe(
                KeyCategory.COMPONENTS)
            discount_keys = self._get_keys_by_category_safe(
                KeyCategory.DISCOUNTS)
            surcharge_keys = self._get_keys_by_category_safe(
                KeyCategory.SURCHARGES)
            ...
            breakdown['components'] = self.key_manager.format_for_pdf(
                component_keys)
    ...
    def get_enhanced_dynamic_data(self) -> dict[str, str]:
        ...
        try:
            ...
            logger.info(
                f"Enhanced dynamic data with {
                    len(enhanced_data)} total keys")
    ...
    def generate_pdf_with_pricing(self,
        ...
        try:
            ...
            if pdf_bytes:
                logger.info(
                    "Successfully generated PDF with pricing integration")
...
def create_pricing_breakdown_section(
...
def update_pdf_placeholders_with_pricing(
    ...
    try:
        ...
        logger.info(
            f"Updated placeholders with {
                len(formatted_pricing_keys)} pricing keys")
