...
class MultiCompanyOfferGenerator:
    ...
    def render_company_selection(self):
        ...
        if st.session_state.multi_offer_selected_companies:
            st.success(
                f" {len(st.session_state.multi_offer_selected_companies)} Firmen ausgewählt")
            return True
                ...
                for i, company_id in enumerate(selected_companies):
                    try:
                        ...
                        offer_data = self._prepare_offer_data(
                            customer_data, company, settings, project_data)
    ...
    def _prepare_offer_data(
        ...
        try:
            if settings.get("selected_module_id"):
                offer_data["selected_module"] = get_product_by_id(
                    settings["selected_module_id"])

            if settings.get("selected_inverter_id"):
                offer_data["selected_inverter"] = get_product_by_id(
                    settings["selected_inverter_id"])

            if settings.get("selected_storage_id"):
                offer_data["selected_storage"] = get_product_by_id(
                    settings["selected_storage_id"])
        except Exception as e:
