...
try:
    ...
    # Fallback, falls app_status.py nicht gefunden wird (sollte nicht
    # passieren, wenn es existiert)
...
# ⚡ Bolt: Cache translation dictionaries to prevent repeated disk I/O and JSON decoding.
# Impact: Reduces load time for repeated translation requests by >99%.
...
def load_translations(lang_code: str = 'de') -> dict[str, str] | None:
    ...
    default_texts = {
        ...
        # die benötigt werden, BEVOR die Haupt-TEXTS-Variable in gui.py gefüllt
        # ist.
    }

    try:
        if not os.path.exists(file_path):
            error_msg = default_texts["language_file_not_found"].format(
                filepath=file_path)
        ...
        with open(file_path, encoding='utf-8') as f:
            ...
            if not isinstance(texts, dict):
                raise ValueError(
                    "Übersetzungsdatei hat kein Dictionary-Format.")
...
if __name__ == '__main__':
    ...
    if german_texts and german_texts.get(
            "app_title") != "Ömers Solar Kakerlake":
    ...
    if non_existent_texts and non_existent_texts.get(
            "app_title") == "Ömers Solar Kakerlake":
