...
class PDFThemeManager:
    ...
    def create_enhanced_chart(
        ...
        **kwargs
        ...
        # Theme anwenden
        theme_config = self.chart_themes.get(
            theme, self.chart_themes['modern'])
        plt.style.use(theme_config['style'])

        fig, ax = plt.subplots(figsize=size)

        if chart_type == 'monthly_generation_enhanced':
            self._create_enhanced_generation_chart(
                ax, data, theme_config, **kwargs)
            ...
            self._create_amortization_timeline(
                ax, data, theme_config, **kwargs)
            ...
            self._create_standard_chart(
                ax, chart_type, data, theme_config, **kwargs)
    ...
    def _create_amortization_timeline(self, ax, data, theme_config, **kwargs):
        ...
        years = data.get('years', list(range(21)))
    ...
    def _create_energy_flow_diagram(self, ax, data, theme_config, **kwargs):
        ...
        components = {
            'pv': {'pos': (0.1, 0.5), 'size': 0.15, 'label': 'Photovoltaik-Anlage'},
    ...
    def create_custom_background(
        ...
        if pattern == 'gradient':
            ...
            # Hexagon-Muster
            hex_size = 20
            for row in range(height // hex_size + 1):
                for col in range(width // hex_size + 1):
                    x = col * hex_size * 1.5
                    ...
                    draw.polygon(points, outline=(
                        200, 200, 200, int(255 * opacity)))
        ...
        with tab2:
            ...
            with col2:
                width = st.number_input(
                    "Breite (px)", value=800, min_value=100, max_value=2000)
                height = st.number_input(
                    "Höhe (px)", value=600, min_value=100, max_value=2000)
    ...
    def render_theme_selector(self):
        ...
        with col1:
            all_themes = {**self.predefined_themes,
                          **st.session_state.custom_themes}
            selected_theme = st.selectbox(
                "Theme auswählen",
                options=list(all_themes.keys()),
                format_func=lambda x: all_themes[x].get('name', x),
                index=list(all_themes.keys()).index(
                    st.session_state.pdf_theme_settings['selected_theme'])
        ...
        if selected_theme in all_themes:
            ...
            with st.expander(" Theme-Details", expanded=False):
                ...
                colors = theme['colors'].to_dict() if hasattr(
                    theme['colors'], 'to_dict') else theme['colors']
                ...
                with col1:
                    st.text_input("Überschriften", value=fonts.get(
                        'heading', ''), disabled=True)
                with col2:
                    st.text_input("Fließtext", value=fonts.get(
                        'body', ''), disabled=True)
                with col3:
                    st.text_input("Akzente", value=fonts.get(
    ...
    def create_custom_theme(self, name: str, base_theme: str | None = None) -> str:
        ...
        with st.expander(" Neues Theme erstellen", expanded=False):
            ...
            with col2:
                base_theme = st.selectbox(
                    ...
                    format_func=lambda x: "Neu" if x is None else manager.predefined_themes[
                        x]['name']
                )

            if st.button(" Theme erstellen", type="primary"):
                if new_theme_name:
                    theme_key = manager.create_custom_theme(
                        new_theme_name, base_theme)
    ...
    with tab3:
        ...
        with col1:
            ...
            if st.button(" Aktuelles Theme exportieren"):
                ...
                all_themes = {**manager.predefined_themes,
                              **st.session_state.custom_themes}
                ...
                if current_theme in all_themes:
                    ...
                    theme_json = json.dumps(
                        theme_data, indent=2, ensure_ascii=False)
...
def get_pdf_stylesheet(color_palette: dict[str, colors.Color]) -> StyleSheet1:
    ...
    stylesheet.add(ParagraphStyle(name='Normal', fontName=FONT_FAMILY_NORMAL,
                   fontSize=10, leading=12, textColor=color_palette['text']))
    stylesheet.add(ParagraphStyle(name='NormalLeft',
                   parent=stylesheet['Normal'], alignment=TA_LEFT))
    stylesheet.add(ParagraphStyle(name='NormalRight',
                   parent=stylesheet['Normal'], alignment=TA_RIGHT))
    stylesheet.add(ParagraphStyle(name='NormalCenter',
                   parent=stylesheet['Normal'], alignment=TA_CENTER))
    stylesheet.add(ParagraphStyle(name='NormalJustify',
                   parent=stylesheet['Normal'], alignment=TA_JUSTIFY))
    stylesheet.add(ParagraphStyle(name='HeaderFooterText',
                   parent=stylesheet['Normal'], fontSize=8, textColor=color_palette['darkgrey']))
    stylesheet.add(ParagraphStyle(name='PageInfo',
                   parent=stylesheet['HeaderFooterText'], alignment=TA_CENTER))
    stylesheet.add(ParagraphStyle(name='DocumentTitleHeader',
                   parent=stylesheet['HeaderFooterText'], fontName=FONT_FAMILY_BOLD, alignment=TA_RIGHT, fontSize=9))
    stylesheet.add(ParagraphStyle(name='CompanyAddressFooter',
                   parent=stylesheet['HeaderFooterText'], fontSize=7, alignment=TA_LEFT))
    stylesheet.add(ParagraphStyle(name='OfferMainTitle', fontName=FONT_FAMILY_BOLD, fontSize=22, leading=26,
                   alignment=TA_CENTER, spaceBefore=1.5*cm, spaceAfter=1.5*cm, textColor=color_palette['primary']))
    stylesheet.add(ParagraphStyle(name='SectionTitle', fontName=FONT_FAMILY_BOLD, fontSize=16, leading=19,
                   spaceBefore=1.2*cm, spaceAfter=0.6*cm, keepWithNext=1, textColor=color_palette['primary']))
    stylesheet.add(ParagraphStyle(name='SubSectionTitle', fontName=FONT_FAMILY_BOLD, fontSize=13, leading=16,
                   spaceBefore=0.8*cm, spaceAfter=0.4*cm, keepWithNext=1, textColor=color_palette['primary']))
    stylesheet.add(ParagraphStyle(name='ComponentTitle', parent=stylesheet['SubSectionTitle'], fontSize=11,
                   spaceBefore=0.6*cm, spaceAfter=0.3*cm, alignment=TA_LEFT, textColor=color_palette['text']))
    stylesheet.add(ParagraphStyle(name='CompanyInfoDeckblatt',
                   parent=stylesheet['NormalCenter'], fontSize=9, leading=11, spaceAfter=0.8*cm, textColor=color_palette['text']))
    stylesheet.add(ParagraphStyle(name='CustomerAddressDeckblatt',
                   parent=stylesheet['NormalLeft'], fontSize=11, leading=14, spaceBefore=2*cm, spaceAfter=1*cm, textColor=color_palette['text']))
    stylesheet.add(ParagraphStyle(name='OfferDetailsDeckblatt',
                   parent=stylesheet['NormalRight'], fontSize=10, leading=12, spaceBefore=0.5*cm))
    stylesheet.add(ParagraphStyle(name='CoverLetter',
                   parent=stylesheet['NormalJustify'], fontSize=11, leading=15, spaceBefore=0.5*cm, spaceAfter=0.5*cm, firstLineIndent=0))
    stylesheet.add(ParagraphStyle(name='CustomerAddressInner',
                   parent=stylesheet['NormalLeft'], fontSize=10, leading=12, spaceBefore=0.2*cm, spaceAfter=0.8*cm))
    stylesheet.add(ParagraphStyle(name='BoldText',
                   parent=stylesheet['NormalLeft'], fontName=FONT_FAMILY_BOLD))
    stylesheet.add(ParagraphStyle(name='ItalicText',
                   parent=stylesheet['NormalLeft'], fontName=FONT_FAMILY_ITALIC))
    stylesheet.add(ParagraphStyle(name='TableText',
                   parent=stylesheet['NormalLeft'], fontSize=9, leading=11))
    stylesheet.add(ParagraphStyle(name='TableTextSmall',
                   parent=stylesheet['NormalLeft'], fontSize=8, leading=10))
    stylesheet.add(ParagraphStyle(name='TableNumber',
                   parent=stylesheet['NormalRight'], fontSize=9, leading=11))
    stylesheet.add(ParagraphStyle(name='TableLabel',
                   parent=stylesheet['NormalLeft'], fontName=FONT_FAMILY_BOLD, fontSize=9, leading=11))
    stylesheet.add(ParagraphStyle(name='TableHeader', parent=stylesheet['NormalCenter'], fontName=FONT_FAMILY_BOLD,
                   fontSize=9, leading=11, textColor=color_palette['white'], backColor=color_palette['primary']))
    stylesheet.add(ParagraphStyle(name='TableBoldRight',
                   parent=stylesheet['TableNumber'], fontName=FONT_FAMILY_BOLD))
    stylesheet.add(ParagraphStyle(name='ImageCaption',
                   parent=stylesheet['NormalCenter'], fontName=FONT_FAMILY_ITALIC, fontSize=8, spaceBefore=0.1*cm, textColor=color_palette['grey']))
    stylesheet.add(ParagraphStyle(name='ChartTitle',
                   parent=stylesheet['SubSectionTitle'], alignment=TA_CENTER, spaceBefore=0.8*cm, spaceAfter=0.3*cm, fontSize=12))
    return stylesheet
