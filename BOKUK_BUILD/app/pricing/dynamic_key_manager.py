...
from financial_calculations import calculate_gross_from_net, calculate_vat_amount
...
class DynamicKeyManager:
    ...
    def generate_vat_keys(
        ...
        gross_amount = calculate_gross_from_net(net_amount, vat_rate)
    ...
    def calculate_component_totals(
        ...
        for comp_name, comp_data in components.items():
            ...
            if vat_rate is not None:
                comp_vat = calculate_vat_amount(price, vat_rate)
                vat_total += comp_vat
        ...
        if vat_total > 0:
            ...
            self.register_key(
                ...
                calculate_gross_from_net(
                    net_total,
                    vat_rate=None if vat_total == 0 else vat_total /
                    net_total if net_total else VAT_RATE_DEFAULT),
                KeyCategory.GROSS_TOTALS.value)
            total_keys[gross_key] = calculate_gross_from_net(
                net_total,
                vat_rate=None if vat_total == 0 else vat_total /
                net_total if net_total else VAT_RATE_DEFAULT)
