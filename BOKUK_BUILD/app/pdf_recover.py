...
def numeric_attack(path, min_len, max_len, processes, charset="0123456789", limit=None):
    manager = Manager()
    queue = manager.Queue()
    pool = Pool(processes=processes)
    try:
        ...
        for res in pool.imap_unordered(worker_numeric, ((path, p) for p in gen), chunksize=100):
            if res[0] == "found":
                pool.terminate()
                pool.join()
                return res[1]
...
def main():
    parser = argparse.ArgumentParser(description="PDF Passwort Recovery (nur eigene Dateien)")
    parser.add_argument("pdf", help="Pfad zur PDF")
    sub = parser.add_subparsers(dest="mode", required=True)

    p_dict = sub.add_parser("dict", help="Wörterbuch-Angriff")
    p_dict.add_argument("-w", "--wordlist", required=True, help="Pfad zur Wortliste (ein PW pro Zeile)")
    p_dict.add_argument("-p", "--processes", type=int, default=4, help="Anzahl paralleler Prozesse")

    p_num = sub.add_parser("numeric", help="Numerischer Brute-Force (Chunks parallel)")
    p_num.add_argument("--min", type=int, default=4, help="Minimale Stellenanzahl")
    p_num.add_argument("--max", type=int, default=6, help="Maximale Stellenanzahl")
    p_num.add_argument("-p", "--processes", type=int, default=4, help="Anzahl paralleler Prozesse")
    p_num.add_argument("--charset", default="0123456789", help="Zeichensatz für Bruteforce")
    p_num.add_argument("--limit", type=int, help="Optionale Begrenzung, n Versuche")
    ...
    if args.mode == "dict":
        ...
        print(f"[+] Numerischer Angriff: {args.min}..{args.max} Stellen charset={args.charset}")
        pw = numeric_attack(args.pdf, args.min, args.max, args.processes, charset=args.charset, limit=args.limit)
