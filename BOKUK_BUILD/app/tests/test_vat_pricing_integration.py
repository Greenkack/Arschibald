...
class TestVATPricingEngineIntegration:
    ...
    def test_vat_dynamic_keys_generation(self):
        ...
        # conflict resolution)
        net_key = next((k for k in vat_result.dynamic_keys if k.startswith(
            "VAT_PV_MODULE_NET_AMOUNT")), None)
        rate_key = next(
            (k for k in vat_result.dynamic_keys if k.startswith("VAT_PV_MODULE_VAT_RATE")),
            None)
        amount_key = next((k for k in vat_result.dynamic_keys if k.startswith(
            "VAT_PV_MODULE_VAT_AMOUNT")), None)
        gross_key = next((k for k in vat_result.dynamic_keys if k.startswith(
            "VAT_PV_MODULE_GROSS_AMOUNT")), None)
        category_key = next((k for k in vat_result.dynamic_keys if k.startswith(
            "VAT_PV_MODULE_VAT_CATEGORY")), None)
...
class TestVATDynamicKeyIntegration:
    ...
    def test_mixed_vat_dynamic_keys(self):
        ...
        mixed_net_key = next(
            (k for k in vat_result.dynamic_keys if k.startswith("VAT_MIXED_TOTAL_NET")),
            None)
        mixed_vat_key = next(
            (k for k in vat_result.dynamic_keys if k.startswith("VAT_MIXED_TOTAL_VAT")),
            None)
        mixed_gross_key = next(
            (k for k in vat_result.dynamic_keys if k.startswith("VAT_MIXED_TOTAL_GROSS")),
            None)
        mixed_rate_key = next((k for k in vat_result.dynamic_keys if k.startswith(
            "VAT_MIXED_EFFECTIVE_VAT_RATE")), None)
        mixed_count_key = next(
            (k for k in vat_result.dynamic_keys if k.startswith("VAT_MIXED_CATEGORY_COUNT")),
            None)
        ...
        pv_net_key = next((k for k in vat_result.dynamic_keys if k.startswith(
            "VAT_PV_MODULE_NET_AMOUNT")), None)
        special_net_key = next((k for k in vat_result.dynamic_keys if k.startswith(
            "VAT_SPECIAL_PRODUCT_NET_AMOUNT")), None)
