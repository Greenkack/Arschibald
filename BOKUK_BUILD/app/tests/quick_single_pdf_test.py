...
def pick_products():
    ...
    if not inverters:
        raise RuntimeError("Keine Wechselrichter in der Produktdatenbank gefunden")
    # Speicher optional
...
def main():
    ...
    print("  total_investment_netto:", analysis_results.get("total_investment_netto"))
    print("  total_investment_brutto:", analysis_results.get("total_investment_brutto"))
    print("  base_matrix_price_netto:", analysis_results.get("base_matrix_price_netto"))
    ...
    pdf_bytes = generate_offer_pdf_with_main_templates(
        ...
        save_admin_setting_func=lambda k, v: None,
        list_products_func=list_products,
        get_product_by_id_func=lambda pid: next((p for p in list_products() if p.get("id") == pid), None),
        db_list_company_documents_func=lambda cid, dtype=None: [],
        active_company_id=1,
        texts=texts,
    )

    if pdf_bytes:
        out = os.path.join(os.path.dirname(__file__), f"single_pdf_test_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf")
