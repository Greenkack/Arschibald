...
class CombinedPricingTemplate:
    ...
    def get_template_structure(self) -> dict[str, Any]:
        ...
        return {
            "title": "Photovoltaik + Wärmepumpe - Gesamtpaket",
            "sections": [
                {
                    "name": "pv_system",
...
class PricingTemplateManager:
    ...
    def create_template(self, system_type: str, config: PricingTemplateConfig | None = None) -> PVPricingTemplate | HeatPumpPricingTemplate | CombinedPricingTemplate:
    ...
    def get_template(self, system_type: str) -> PVPricingTemplate | HeatPumpPricingTemplate | CombinedPricingTemplate | None:
        """Get existing template for system type"""
        return self.templates.get(system_type)

    def render_template(self, system_type: str, pricing_data: dict[str, Any]) -> dict[str, str]:
        ...
        for placeholder_key, placeholder_value in template.placeholders.items():
            ...
            if actual_value is not None:
                rendered[placeholder_key] = self._format_value(actual_value, template.config)
            else:
                rendered[placeholder_key] = ""  # Empty if no data found

        return rendered

    def _find_pricing_value(self, key_name: str, pricing_data: dict[str, Any]) -> Any:
    ...
    def _format_value(self, value: Any, config: PricingTemplateConfig) -> str:
        ...
        try:
            ...
            if isinstance(value, (int, float)):
                ...
                return f"{value:.{config.decimal_places}f} {config.currency_symbol}"
    ...
    def export_template_documentation(self, system_type: str) -> dict[str, Any]:
...
def create_pv_pricing_template(config: PricingTemplateConfig | None = None) -> PVPricingTemplate:
...
def create_heatpump_pricing_template(config: PricingTemplateConfig | None = None) -> HeatPumpPricingTemplate:
...
def create_combined_pricing_template(config: PricingTemplateConfig | None = None) -> CombinedPricingTemplate:
