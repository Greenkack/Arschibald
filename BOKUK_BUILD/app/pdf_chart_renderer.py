...
def render_all_selected_charts_to_pdf(
    ...
    for chart_key in selected_charts:
        ...
        chart_title = chart_friendly_names.get(chart_key, chart_key.replace('_', ' ').title())
...
def create_chart_overview_page(
    ...
    available_count = sum(1 for key in selected_charts if analysis_results.get(key) is not None)
...
def render_financial_tools_to_pdf(
    ...
    pdf.set_font("Arial", "B", 16)
    pdf.cell(0, 10, "Financial Tools - Detaillierte Finanzanalyse", ln=True, align="C")
    pdf.ln(5)
    ...
    if 'annuity_15y' in financial_tools_results:
        ...
        pdf.cell(0, 6, f"Monatliche Rate: {annuity['monthly']:,.2f} €", ln=True)
    ...
    if 'leasing' in financial_tools_results:
        ...
        pdf.set_font("Arial", "", 11)
        pdf.cell(0, 6, f"Monatliche Rate: {leasing['monthly_rate']:,.2f} €", ln=True)
        pdf.cell(0, 6, f"Gesamtkosten: {leasing['total_cost']:,.2f} €", ln=True)
        pdf.cell(0, 6, f"Laufzeit: {leasing.get('duration_months', 0)} Monate", ln=True)
        pdf.ln(5)
    ...
    if 'depreciation' in financial_tools_results:
        ...
        pdf.set_font("Arial", "", 11)
        pdf.cell(0, 6, f"Jährliche Abschreibung: {depreciation['annual_depreciation']:,.2f} €", ln=True)
        pdf.cell(0, 6, f"Restwert: {depreciation['residual_value']:,.2f} €", ln=True)
        pdf.cell(0, 6, f"Methode: {depreciation.get('method', 'N/A')}", ln=True)
        pdf.ln(5)
    ...
    if 'financing_comparison' in financial_tools_results:
        ...
        pdf.set_font("Arial", "", 11)
        pdf.cell(0, 6, f"  Monatlich: {comparison['loan']['monthly_payment']:,.2f} €", ln=True)
        pdf.cell(0, 6, f"  Gesamt: {comparison['loan']['total_cost']:,.2f} €", ln=True)
        
        pdf.set_font("Arial", "B", 12)
        pdf.cell(0, 7, "Leasing:", ln=True)
        pdf.set_font("Arial", "", 11)
        pdf.cell(0, 6, f"  Monatlich: {comparison['leasing']['monthly_rate']:,.2f} €", ln=True)
        pdf.cell(0, 6, f"  Gesamt: {comparison['leasing']['total_cost']:,.2f} €", ln=True)
        
        # Empfehlung
        diff = comparison['leasing']['total_cost'] - comparison['loan']['total_cost']
        if diff > 0:
            pdf.set_text_color(0, 100, 0)
            pdf.cell(0, 7, f"Empfehlung: Kredit (Ersparnis: {diff:,.2f} €)", ln=True)
        else:
            pdf.set_text_color(0, 100, 0)
            pdf.cell(0, 7, f"Empfehlung: Leasing (Ersparnis: {abs(diff):,.2f} €)", ln=True)
        pdf.set_text_color(0, 0, 0)
        pdf.ln(5)
    ...
    if 'capital_gains_tax' in financial_tools_results:
        ...
        pdf.set_font("Arial", "", 11)
        pdf.cell(0, 6, f"Steuer: {capital_gains['tax_amount']:,.2f} €", ln=True)
        pdf.cell(0, 6, f"Netto-Ertrag: {capital_gains['net_profit']:,.2f} €", ln=True)
        pdf.ln(5)
    ...
    if 'contracting' in financial_tools_results:
        ...
        pdf.set_font("Arial", "", 11)
        pdf.cell(0, 6, f"Jährliche Kosten: {contracting['annual_cost']:,.2f} €", ln=True)
        pdf.cell(0, 6, f"Gesamtkosten: {contracting['total_cost']:,.2f} €", ln=True)
        pdf.ln(5)
...
def get_chart_statistics(
    ...
    available = sum(1 for key in selected_charts if analysis_results.get(key) is not None)
    ...
    for chart_key in selected_charts:
        ...
        if any(keyword in chart_key for keyword in ['cost', 'roi', 'financing', 'investment', 'cashflow', 'amort', 'break_even']):
