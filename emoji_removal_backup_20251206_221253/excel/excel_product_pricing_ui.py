...
def render_product_price_config_ui(
    ...
    if pricing_mode == "einzelpreis":
        ...
        if not matrices:
            st.warning("Keine Preismatrizen verfügbar. Bitte erstellen Sie zuerst eine Matrix im 'Preis Matrix' Tab.")
            return result
        ...
        if matrix_data:
                    ...
                    with st.expander("📋 Berechnungs-Details", expanded=False):
                        ...
                        with col1:
                            ...
                            if calc_result.row_floor_source:
                                st.info(f"Floor-Matching: Zeile '{selected_row}' → '{calc_result.row_floor_source}'")
            ...
            if 'error' in preview:
                ...
                if preview['truncated']:
                    st.info("Tabelle wurde gekürzt. Vollständige Matrix im 'Preis Matrix' Tab anzeigen.")
            else:
...
def render_product_price_config_inline(
    ...
    if pricing_mode == "einzelpreis":
        ...
        if not matrices:
            st.warning("Keine Preismatrizen verfügbar.")
