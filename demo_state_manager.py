...
with st.sidebar:
    ...
    if selected_theme != st.session_state.current_theme:
        ...
        if success_count == total_count:
            ...
            st.warning(f"⚠️ Saved to {success_count}/{total_count} backends")
        else:
...
with tab2:
    ...
    with col1:
        if st.button("💾 Test Save", use_container_width=True):
            with st.spinner("Saving..."):
                ...
                st.info(f"⏱️ Duration: {duration:.2f}ms")
    ...
    with col4:
        if st.button("🗑️ Test Delete", use_container_width=True):
    ...
    with col2:
        if st.button("🔄 Sync Across Backends", use_container_width=True):
            if len(test_backends) < 2:
                st.warning("⚠️ Select at least 2 backends")
            else:
