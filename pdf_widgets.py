...
class PDFSectionManager:
    ...
    def initialize_session_state(self) -> None:
        if "pdf_section_order" not in st.session_state:
            st.session_state.pdf_section_order = [key for key, section in sorted(
                self.available_sections.items(), key=lambda x: x[1]["default_order"])]
        ...
        if SORTABLES_AVAILABLE:
            ...
            try:
                ...
                st.session_state.pdf_section_order = [
                    label_to_key.get(lbl, lbl) for lbl in sorted_labels]
            except Exception as e:
                st.warning(
                    f"Drag & Drop nicht verfügbar ({e}). Fallback-Steuerung aktiv.")
    ...
    def _render_pdf_structure_fallback(self) -> None:
        ...
        for idx, section_key in enumerate(st.session_state.pdf_section_order):
            ...
            with col2:
                st.markdown(
                    f"{section.get('icon', '')} {section.get('name', section_key)}")
            with col3:
                if idx > 0 and st.button(
                        " Nach oben", key=f"up_{section_key}"):
                    ...
                    st.rerun()
                if idx < len(st.session_state.pdf_section_order) - \
                        1 and st.button(" Nach unten", key=f"down_{section_key}"):
                    st.session_state.pdf_section_order[idx + 1], st.session_state.pdf_section_order[idx] = (
                        st.session_state.pdf_section_order[idx],
                        st.session_state.pdf_section_order[idx + 1],
                    )
    ...
    def _render_section_editor(
            self, section_key: str, section_info: dict[str, Any]) -> None:
        ...
        for idx, content in enumerate(contents):
            ...
            with col1:
                ...
                st.markdown(
                    f"{template.get('icon', '')} {template.get('name', '')}")
            with col2:
                self._render_content_editor(section_key, idx, content)
            with col3:
                if st.button(" Löschen",
                             key=f"delete_content_{section_key}_{idx}"):
                    contents.pop(idx)
                    st.rerun()
        ...
        with col2:
            if st.button(" Hinzufügen", key=f"add_content_{section_key}"):
                new_content = {
                    ...
                    "data": self.content_templates.get(
                        new_content_type,
                        {}).get("default_content"),
                }
    ...
    def _render_content_editor(self,
                               section_key: str,
                               content_idx: int,
                               content: dict[str,
                                             Any]) -> None:
        ...
        if content_type == "text":
            content["data"] = st.text_area("Text", value=content.get(
                "data", ""), key=f"text_{section_key}_{content_idx}", height=100)
    ...
    def _render_custom_section_creator(self) -> None:
        ...
        with col2:
            section_icon = st.text_input(
                "Icon", value="", max_chars=2, key="new_section_icon")
        content_types = st.multiselect(
            "Erlaubte Inhaltstypen", options=list(
                self.content_templates.keys()), default=[
                "text", "image"], format_func=lambda x: f"{
                self.content_templates[x]['icon']} {
                    self.content_templates[x]['name']}", )
        ...
        if st.button(" Sektion erstellen", key="create_custom_section"):
            if section_name and content_types:
                ...
                base = "".join(
                    ch.lower() if (
                        ch.isalnum() or ch in "-_") else "-" for ch in section_name.strip())
                ...
                st.error(
                    "Bitte geben Sie einen Namen ein und wählen Sie mindestens einen Inhaltstyp.")
...
def render_pdf_structure_manager(texts: dict[str, str]):
    ...
    with tab3:
        ...
        selected_template = st.selectbox("Vorlage auswählen", options=list(
            templates.keys()), format_func=lambda x: templates[x]["name"])
...
# Änderungshistorie
# 2025-06-21, Gemini Ultra: PDF Drag & Drop Manager mit
# Reihenfolgen-Editor implementiert
