...
def get_selected_services_for_pdf(project_data: dict[str, Any]) -> list[dict[str, Any]]:
    """
    ...
    try:
        ...
        if hasattr(st, 'session_state') and 'selected_services' in st.session_state:
    ...
    if services_data and isinstance(services_data, list):
        for service in services_data:
            if isinstance(service, dict) and 'name' in service and 'price' in service:
...
def format_services_for_pdf_display(services: list[dict[str, Any]]) -> dict[str, str]:
    """
    ...
    total_str = f"{total_cost:,.2f} €".replace(',', 'X').replace('.', ',').replace('X', '.')
...
def integrate_services_into_placeholders(result: dict[str, str], project_data: dict[str, Any]) -> dict[str, str]:
    """
    ...
    print(f"DEBUG: Services Integration - {len(selected_services)} Dienstleistungen gefunden")
