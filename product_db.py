...
def calculate_price_by_method(base_price: float,
    ...
    """
    if product_specs:
        product_category = str(product_specs.get('category', '')).lower()
        product_name = str(product_specs.get('model_name', '')).lower()
        product_type = str(product_specs.get('component_type', '')).lower()
        storage_markers = (
            'batteriespeicher',
            'speicher',
            'battery',
        ...
        if any(marker in f"{product_category} {product_name} {product_type}" for marker in storage_markers):
            # Batteriespeicher sind im Solar-Calculator eine gewählte
            # Produktposition. Alte/dynamische Felder wie
            # `selected_storage_storage_power_kw` dürfen nie als Menge oder
            # kWh/kWp-Faktor in den Preis einfließen.
            quantity = 1
            calculate_per = "Stück"
