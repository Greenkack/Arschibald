...
def _validate_path(path: str) -> tuple[bool, str, Optional[str]]:
    ...
    try:
        ...
        full_path = os.path.normpath(
            os.path.join(AGENT_WORKSPACE, path)
        )
        workspace_abs = os.path.abspath(AGENT_WORKSPACE)
        full_path_abs = os.path.abspath(full_path)

        # Check if the resolved path is within the workspace
        if not full_path_abs.startswith(workspace_abs):
            error_msg = (
                "Fehler: Der Versuch, außerhalb des "
                "Arbeitsverzeichnisses zuzugreifen, ist nicht erlaubt."
            )
        ...
        if ".." in path or path.startswith("/") or path.startswith("\\"):
            error_msg = (
                ...
                "Verwenden Sie relative Pfade ohne '..'."
            )
            return False, "", error_msg
...
def write_file(path: str, content: str) -> str:
    ...
    try:
        ...
        return (
            f"Fehler: Keine Berechtigung zum "
            f"Schreiben der Datei '{path}'."
        )
    except Exception as e:
...
def read_file(path: str) -> str:
    ...
    try:
        ...
        return (
            f"Fehler: Keine Berechtigung zum "
            f"Lesen der Datei '{path}'."
        )
    except UnicodeDecodeError:
        return (
            f"Fehler: Datei '{path}' konnte nicht "
            f"als UTF-8 dekodiert werden."
        )
    except Exception as e:
...
def list_files(path: str = ".") -> str:
    ...
    try:
        ...
        return (
            f"Fehler: Keine Berechtigung zum Auflisten "
            f"des Verzeichnisses '{path}'."
        )
    except Exception as e:
...
agent_workspace/{safe_project_name}/ Verzeichnis.
"""
