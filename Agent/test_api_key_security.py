...
def test_env_file_security():
    ...
    print(f"Security status: {'SECURE' if is_secure else 'WARNINGS'}")
    if warnings:
        print("Warnings:")
...
def test_sensitive_data_masking():
    ...
    print("\nNote: Phone number masking is optional and not critical for API key security")
...
def test_startup_security_validation():
    ...
    print(
        f"Validation status: {
            'PASSED' if is_valid else 'ISSUES FOUND'}")
...
def test_env_example_exists():
    ...
    if 'sk-' in content and 'sk-...' not in content:
        print("WARNING: .env.example may contain actual API keys!")
        all_present = False
