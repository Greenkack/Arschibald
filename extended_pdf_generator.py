...
class ChartCache:
    ...
    def invalidate(self, chart_key: str | None = None, chart_data: Any = None) -> None:
        ...
        if chart_key is None:
            ...
            if chart_data is not None:
                ...
                # If no chart_data provided, remove all entries (fallback to clear all)
                self._cache.clear()
                self._access_count.clear()
...
class ExtendedPDFLogger:
    ...
    def log_error(self, component: str, message: str, exception: Exception | None = None) -> None:
...
class ExtendedPDFGenerator:
    ...
    def generate_extended_pages(self) -> bytes:
        ...
        """
        self.logger.log_info('ExtendedPDFGenerator', 'Starting extended PDF generation with efficient merging')
        writer = PdfWriter()
        ...
        try:
            ...
            for section_name, generator_func in page_generators:
                self.logger.log_info('ExtendedPDFGenerator', f'Processing {section_name} section')
                ...
                try:
                    ...
                    if section_bytes:
                        ...
                        self.logger.log_info('ExtendedPDFGenerator', f'Added {pages_added} pages from {section_name}')
                    else:
                        self.logger.log_warning('ExtendedPDFGenerator', f'No pages generated for {section_name}')
                except Exception as e:
                    self.logger.log_error('ExtendedPDFGenerator', f'Error processing {section_name}', e)
                    continue
            ...
            if total_pages > 0:
                ...
                self.logger.log_info('ExtendedPDFGenerator', f'Successfully generated extended PDF with {total_pages} pages ({len(result)} bytes)')
                return result
            else:
                self.logger.log_warning('ExtendedPDFGenerator', 'No pages generated for extended PDF')
                return b''
        
        except Exception as e:
            self.logger.log_error('ExtendedPDFGenerator', 'Critical error generating extended pages', e)
    ...
    def _generate_financing_pages(self) -> bytes:
        ...
        try:
            ...
            self.logger.log_error('ExtendedPDFGenerator', 'Failed to generate financing pages', e)
            return b''
    ...
    def _merge_product_datasheets(self) -> bytes:
        ...
        try:
            ...
            self.logger.log_error('ExtendedPDFGenerator', 'Failed to merge product datasheets', e)
            return b''
    ...
    def _merge_company_documents(self) -> bytes:
        ...
        try:
            ...
            self.logger.log_error('ExtendedPDFGenerator', 'Failed to merge company documents', e)
            return b''
    ...
    def _add_pages_to_writer(
        ...
        """
        if not pdf_bytes:
            self.logger.log_warning('ExtendedPDFGenerator', 'Attempted to add empty PDF bytes to writer')
            return
        ...
        try:
            ...
            self.logger.log_info('ExtendedPDFGenerator', f'Added {page_count} pages to writer')
        except Exception as e:
            self.logger.log_error('ExtendedPDFGenerator', 'Error adding pages to writer', e)
...
class FinancingPageGenerator:
    ...
    def generate(self) -> bytes:
        ...
        """
        self.logger.log_info('FinancingPageGenerator', 'Starting comprehensive financing page generation (Task 9)')
        ...
        try:
            ...
            if final_end_preis <= 0:
                self.logger.log_error('FinancingPageGenerator', 'final_end_preis not available or zero, cannot generate financing pages')
                return b''
            
            self.logger.log_info('FinancingPageGenerator', f'Using final_end_preis: {final_end_preis:,.2f} €')
            ...
            y_pos = self._draw_credit_financing_section(c, final_end_preis, y_pos)
            ...
            # Subtask 9.3: Leasingfinanzierung berechnen und darstellen
            y_pos = self._draw_leasing_financing_section(c, final_end_preis, y_pos)
            ...
            self.logger.log_info('FinancingPageGenerator', f'Successfully generated comprehensive financing pages ({len(result)} bytes)')
            return result
        except Exception as e:
            self.logger.log_error('FinancingPageGenerator', 'Error generating comprehensive financing pages', e)
            import traceback
            traceback.print_exc()
            return b''
    ...
    def _get_final_price(self) -> float:
        ...
        try:
            ...
            if isinstance(self.analysis_results, dict):
                ...
                if final_price and isinstance(final_price, (int, float)) and final_price > 0:
                ...
                final_price = self.analysis_results.get('total_investment_brutto')
                if final_price and isinstance(final_price, (int, float)) and final_price > 0:
                    return float(final_price)
            ...
            if grand_total and isinstance(grand_total, (int, float)) and grand_total > 0:
                self.logger.log_warning('FinancingPageGenerator', 'final_end_preis not found, using grand_total as fallback')
                return float(grand_total)
            
            self.logger.log_error('FinancingPageGenerator', 'final_end_preis not available in any expected location')
            return 0.0
            
        except Exception as e:
            self.logger.log_error('FinancingPageGenerator', 'Error extracting final_end_preis', e)
            return 0.0
    ...
    def _get_financing_options(self) -> list[dict]:
        ...
        try:
            ...
            # Load comprehensive payment config which contains financing options
            comprehensive_config = load_admin_setting('comprehensive_payment_config', {})
            ...
            if not financing_opts:
                self.logger.log_info('FinancingPageGenerator', 'No financing options in comprehensive config, trying legacy payment_terms')
            ...
            if not financing_opts:
                self.logger.log_warning('FinancingPageGenerator', 'No enabled financing options found in database')
            
            return financing_opts
        except Exception as e:
            self.logger.log_error('FinancingPageGenerator', 'Error loading financing options from database', e)
            return []
    ...
    def _draw_financing_section_header(self, c: canvas.Canvas) -> None:
        ...
        c.drawString(2*cm, self.height - 3*cm, "Finanzierungsinformationen")
        
        c.setFont("Helvetica", 11)
        c.setFillColor(HexColor("#666666"))
        c.drawString(2*cm, self.height - 4*cm, "Vollständige Finanzierungsanalyse mit Kredit, Leasing und Amortisationsplan")
    
    def _draw_credit_financing_section(self, c: canvas.Canvas, final_end_preis: float, y_pos: float) -> float:
        ...
        try:
            ...
            # Get global constants for interest rate and years (Requirements 9.6, 9.7, 9.8)
            interest_rate = self._get_global_constant('loan_interest_rate_percent', 4.0)
            duration_years = self._get_global_constant('simulation_period_years', 20)
            
            # Calculate annuity (Requirement 9.5)
            credit_result = calculate_annuity(final_end_preis, interest_rate, duration_years)
            
            if 'error' in credit_result:
                self.logger.log_error('FinancingPageGenerator', f'Error calculating credit: {credit_result["error"]}')
                return y_pos
            ...
            table_data = [
                ...
                ['Zinssatz', f'{interest_rate:.2f}% p.a.'],
                ['Laufzeit', f'{duration_years} Jahre ({credit_result.get("laufzeit_monate", 0)} Monate)'],
                ['Monatliche Rate', self._format_currency(credit_result.get('monatliche_rate', 0))],
                ['Gesamtkosten', self._format_currency(credit_result.get('gesamtkosten', 0))],
                ['Zinskosten gesamt', self._format_currency(credit_result.get('gesamtzinsen', 0))]
            ]
            
            # Draw table with blue header and grey gridlines (Requirements 9.10, 9.11, 9.12)
            y_pos = self._draw_financing_table(c, table_data, y_pos)
            
            return y_pos - 1*cm
            
        except ImportError:
            self.logger.log_error('FinancingPageGenerator', 'financial_tools module not available')
            return y_pos
        except Exception as e:
            self.logger.log_error('FinancingPageGenerator', 'Error drawing credit financing section', e)
            return y_pos
    
    def _draw_leasing_financing_section(self, c: canvas.Canvas, final_end_preis: float, y_pos: float) -> float:
        ...
        """
        try:
            # Import financial_tools for calculate_leasing_costs (Requirement 9.13)
            from financial_tools import calculate_leasing_costs
            
            # Get global constants (Requirements 9.14, 9.15)
            leasing_factor = self._get_global_constant('leasing_factor_percent', 1.2)
            residual_value_percent = self._get_global_constant('residual_value_percent', 1.0)
            duration_years = self._get_global_constant('simulation_period_years', 20)
            ...
            if 'error' in leasing_result:
                self.logger.log_error('FinancingPageGenerator', f'Error calculating leasing: {leasing_result["error"]}')
                return y_pos
            ...
            self.logger.log_error('FinancingPageGenerator', 'Error drawing leasing financing section', e)
            return y_pos
    
    def _draw_amortization_plan(self, c: canvas.Canvas, final_end_preis: float, y_pos: float) -> float:
        ...
        try:
            ...
            c.setFillColor(HexColor(self.theme['colors']['primary']))
            c.rect(2*cm, y_pos - 0.5*cm, self.width - 4*cm, 0.6*cm, fill=1)
            c.setFillColor(HexColor("#FFFFFF"))
            
            # Columns: Jahr, Einsparungen, Kosten, Netto-Cashflow, kumulierter Cashflow (Requirement 9.19)
            col_widths = [2*cm, 3*cm, 3*cm, 3.5*cm, 3.5*cm]
            col_x = [2*cm, 4*cm, 7*cm, 10*cm, 13.5*cm]
            headers = ['Jahr', 'Einsparungen', 'Kosten', 'Netto-Cashflow', 'Kumuliert']
            ...
            for year in range(1, min(years + 1, 11)):
                ...
                if cumulative_cashflow >= 0 and amortization_year is None:
                    ...
                    c.rect(2*cm, y_pos - 0.4*cm, self.width - 4*cm, 0.5*cm, fill=1)
                    c.setFillColor(HexColor("#000000"))
                    c.setFont("Helvetica-Bold", 8)
                
                c.drawString(col_x[0] + 0.2*cm, y_pos, str(year))
                c.drawString(col_x[1] + 0.2*cm, y_pos, self._format_currency(annual_savings))
                c.drawString(col_x[2] + 0.2*cm, y_pos, self._format_currency(annual_costs))
                c.drawString(col_x[3] + 0.2*cm, y_pos, self._format_currency(net_cashflow))
                c.drawString(col_x[4] + 0.2*cm, y_pos, self._format_currency(cumulative_cashflow))
            ...
            if amortization_year:
                ...
                c.setFillColor(HexColor(self.theme['colors']['primary']))
                y_pos -= 0.5*cm
                c.drawString(2*cm, y_pos, f"Amortisationszeit: {amortization_year} Jahre")
            
            return y_pos - 1*cm
            
        except Exception as e:
            self.logger.log_error('FinancingPageGenerator', 'Error drawing amortization plan', e)
            return y_pos
    
    def _draw_financing_comparison(self, c: canvas.Canvas, final_end_preis: float, y_pos: float) -> float:
        ...
        try:
            ...
            c.drawString(2.5*cm, y_pos, f"Investition: {self._format_currency(cash_data.get('investition', 0))}")
            y_pos -= 0.5*cm
            c.drawString(2.5*cm, y_pos, f"Opportunitätskosten: {self._format_currency(cash_data.get('opportunitaetskosten', 0))}")
            y_pos -= 0.5*cm
            c.drawString(2.5*cm, y_pos, f"Gesamtkosten: {self._format_currency(cash_data.get('gesamtkosten', 0))}")
            y_pos -= 1*cm
            ...
            c.drawString(2.5*cm, y_pos, f"Monatliche Rate: {self._format_currency(credit_data.get('monatliche_rate', 0))}")
            y_pos -= 0.5*cm
            c.drawString(2.5*cm, y_pos, f"Gesamtkosten: {self._format_currency(credit_data.get('gesamtkosten', 0))}")
            y_pos -= 1*cm
            ...
            c.drawString(2.5*cm, y_pos, f"Monatliche Rate: {self._format_currency(leasing_data.get('monatliche_rate', 0))}")
            y_pos -= 0.5*cm
            c.drawString(2.5*cm, y_pos, f"Effektive Kosten: {self._format_currency(leasing_data.get('effektive_kosten', 0))}")
            y_pos -= 1*cm
            ...
            self.logger.log_error('FinancingPageGenerator', 'Error drawing financing comparison', e)
            return y_pos
    
    def _draw_financing_table(self, c: canvas.Canvas, data: list, y_pos: float) -> float:
    ...
    def _get_annual_savings(self) -> float:
        ...
        try:
            ...
            annual_production = self.analysis_results.get('annual_pv_production_kwh', 0)
            electricity_price = self._get_global_constant('electricity_price_eur_per_kwh', 0.30)
    ...
    def _get_annual_costs(self) -> float:
        ...
        try:
            ...
            maintenance_per_kwp = self._get_global_constant('maintenance_variable_eur_per_kwp_pa', 5.0)
            maintenance_fixed = self._get_global_constant('maintenance_fixed_eur_pa', 50.0)
            
            return float(anlage_kwp) * float(maintenance_per_kwp) + float(maintenance_fixed)
        except:
    ...
    def _draw_financing_overview(
        ...
        for idx, option in enumerate(options):
            ...
            if y_pos < 3*cm:
                ...
                c.setFillColor(HexColor(self.theme['colors']['primary']))
                c.drawString(2*cm, self.height - 3*cm, "Finanzierungsoptionen (Fortsetzung)")
...
class ProductDatasheetMerger:
    ...
    def merge(self, datasheet_ids: list[int]) -> bytes:
        ...
        if not datasheet_ids:
            self.logger.log_warning('ProductDatasheetMerger', 'No datasheet IDs provided')
            return b''
        
        self.logger.log_info('ProductDatasheetMerger', f'Starting efficient merge of {len(datasheet_ids)} datasheets')
        ...
        for product_id in datasheet_ids:
            try:
                ...
                if datasheet_bytes:
                    ...
                    self.logger.log_warning('ProductDatasheetMerger', f'No datasheet bytes returned for product {product_id}')
            except Exception as e:
                self.logger.log_error('ProductDatasheetMerger', f'Error loading datasheet for product {product_id}', e)
                continue
        
        if not datasheet_bytes_list:
            self.logger.log_warning('ProductDatasheetMerger', 'No datasheets were successfully loaded')
            return b''
        ...
        for product_id, datasheet_bytes in datasheet_bytes_list:
            try:
                ...
                successful_merges += 1
                self.logger.log_info('ProductDatasheetMerger', f'Successfully merged datasheet for product {product_id} ({page_count} pages)')
            except Exception as pdf_error:
                self.logger.log_error('ProductDatasheetMerger', f'Error reading PDF for product {product_id}', pdf_error)
                continue
        
        # Only return PDF bytes if we successfully merged at least one datasheet
        if successful_merges == 0:
            self.logger.log_warning('ProductDatasheetMerger', 'No datasheets were successfully merged')
            return b''
        ...
        writer.write(output)
        result = output.getvalue()
        self.logger.log_info('ProductDatasheetMerger', f'Successfully merged {successful_merges} datasheet(s) ({len(result)} bytes)')
        return result
    ...
    def _load_datasheet(self, product_id: int) -> bytes | None:
        ...
        try:
            ...
            if not product:
                self.logger.log_warning('ProductDatasheetMerger', f'Product {product_id} not found in database')
                return None
            
            # Check if product has datasheet - use correct field name from schema
            datasheet_path = product.get('datasheet_link_db_path')
            if not datasheet_path:
                self.logger.log_warning('ProductDatasheetMerger', f'Product {product_id} has no datasheet_link_db_path')
                return None
            ...
            if not os.path.isabs(datasheet_path):
                ...
                datasheet_path = os.path.join('data', 'product_datasheets', datasheet_path)
            
            # Load file from filesystem
            if not os.path.exists(datasheet_path):
                self.logger.log_warning('ProductDatasheetMerger', f'Datasheet file not found at {datasheet_path}')
                return None
            ...
            if not file_bytes:
                self.logger.log_warning('ProductDatasheetMerger', f'Empty file at {datasheet_path}')
                return None
            ...
            if file_ext == '.pdf':
                ...
                self.logger.log_info('ProductDatasheetMerger', f'Loaded PDF datasheet for product {product_id}')
                return file_bytes
            elif file_ext in ['.png', '.jpg', '.jpeg', '.gif', '.bmp', '.tiff']:
                # Convert image to PDF
                self.logger.log_info('ProductDatasheetMerger', f'Converting image datasheet ({file_ext}) to PDF for product {product_id}')
                return self._convert_image_to_pdf(file_bytes)
            else:
                self.logger.log_warning('ProductDatasheetMerger', f'Unsupported file format {file_ext} for product {product_id}')
                return None
    ...
    def _convert_image_to_pdf(self, image_bytes: bytes) -> bytes:
        ...
        try:
            ...
            self.logger.log_info('ProductDatasheetMerger', f'Original image size: {img_width}x{img_height}')
            
            # Calculate scaling to fit within margins (2cm on each side, 3cm top/bottom)
            ...
            if img_width > target_width_px or img_height > target_height_px:
                self.logger.log_info('ProductDatasheetMerger', f'Scaling image down to {target_width_px}x{target_height_px} for 300 DPI')
                ...
                self.logger.log_info('ProductDatasheetMerger', f'Optimized image size: {len(optimized_bytes)} bytes (original: {len(image_bytes)} bytes)')
                
                # Use optimized image
                img = ImageReader(io.BytesIO(optimized_bytes))
            else:
                # Image is already small enough, use original
                self.logger.log_info('ProductDatasheetMerger', 'Image already at optimal size, using original')
            ...
            self.logger.log_info('ProductDatasheetMerger', 'Successfully converted image to PDF')
            return buffer.getvalue()
            
        except Exception as e:
            self.logger.log_error('ProductDatasheetMerger', 'Error converting image to PDF', e)
            return b''
...
class CompanyDocumentMerger:
    ...
    def merge(self, document_ids: list[int]) -> bytes:
        ...
        """
        if not document_ids:
            self.logger.log_warning('CompanyDocumentMerger', 'No document IDs provided')
            return b''
        
        self.logger.log_info('CompanyDocumentMerger', f'Starting efficient merge of {len(document_ids)} documents')
        ...
        for doc_id in document_ids:
            try:
                ...
                if doc_bytes:
                    ...
                    self.logger.log_warning('CompanyDocumentMerger', f'No document bytes returned for document {doc_id}')
            except Exception as e:
                self.logger.log_error('CompanyDocumentMerger', f'Error loading document {doc_id}', e)
                continue
        
        if not document_bytes_list:
            self.logger.log_warning('CompanyDocumentMerger', 'No documents were successfully loaded')
            return b''
        ...
        for doc_id, doc_bytes in document_bytes_list:
            try:
                ...
                successful_merges += 1
                self.logger.log_info('CompanyDocumentMerger', f'Successfully merged document {doc_id} ({page_count} pages)')
            except Exception as pdf_error:
                self.logger.log_error('CompanyDocumentMerger', f'Error reading PDF for document {doc_id}', pdf_error)
                continue
        
        # Only return PDF bytes if we successfully merged at least one document
        if successful_merges == 0:
            self.logger.log_warning('CompanyDocumentMerger', 'No documents were successfully merged')
            return b''
        ...
        writer.write(output)
        result = output.getvalue()
        self.logger.log_info('CompanyDocumentMerger', f'Successfully merged {successful_merges} document(s) ({len(result)} bytes)')
        return result
    ...
    def _load_document(self, doc_id: int) -> bytes | None:
        ...
        try:
            ...
            if not conn:
                self.logger.log_error('CompanyDocumentMerger', f'Could not connect to database for document {doc_id}', None)
                return None
            ...
            try:
                ...
                if not row:
                    self.logger.log_warning('CompanyDocumentMerger', f'Document {doc_id} not found in database')
                    return None
                ...
                if not relative_path:
                    self.logger.log_warning('CompanyDocumentMerger', f'Document {doc_id} has no file path')
                    return None
                ...
                if not os.path.exists(absolute_path):
                    self.logger.log_warning('CompanyDocumentMerger', f'Document file not found at {absolute_path}')
                    return None
                ...
                if not file_bytes:
                    self.logger.log_warning('CompanyDocumentMerger', f'Empty file at {absolute_path}')
                    return None
                ...
                if file_ext == '.pdf':
                    # Return PDF bytes directly
                    self.logger.log_info('CompanyDocumentMerger', f'Loaded PDF document {doc_id}')
                    return file_bytes
                else:
                    self.logger.log_warning('CompanyDocumentMerger', f'Unsupported file format {file_ext} for document {doc_id}')
                    return None
...
class ChartPageGenerator:
    ...
    def generate(self, chart_keys: list[str]) -> bytes:
        ...
        """
        if not chart_keys:
            self.logger.log_warning('ChartPageGenerator', 'No chart keys provided')
            return b''
        
        self.logger.log_info('ChartPageGenerator', f'Generating {len(chart_keys)} charts with layout: {self.layout}')
        ...
        try:
            ...
            if result:
                self.logger.log_info('ChartPageGenerator', f'Successfully generated chart pages ({len(result)} bytes)')
            else:
                self.logger.log_warning('ChartPageGenerator', 'Generated empty chart pages')
            
            return result
        except Exception as e:
            self.logger.log_error('ChartPageGenerator', 'Error generating chart pages', e)
            return b''
    ...
    def _generate_one_per_page(self, chart_keys: list[str]) -> bytes:
        ...
        for chart_key in chart_keys:
            ...
            if not chart_bytes:
                self.logger.log_warning('ChartPageGenerator', f'Chart {chart_key} not found in analysis results')
                continue
            ...
            try:
                ...
                self.logger.log_error('ChartPageGenerator', f'Error drawing chart {chart_key}', e)
                c.drawString(
                    2*cm,
                    self.height / 2,
    ...
    def _get_chart_bytes(self, chart_key: str) -> bytes | None:
        ...
        if cached_bytes:
            self.logger.log_info('ChartPageGenerator', f'Cache hit for chart {chart_key}')
            return cached_bytes
        
        # Cache miss - store in cache and return
        self.logger.log_info('ChartPageGenerator', f'Cache miss for chart {chart_key}')
    ...
    def _draw_chart_in_area(
        ...
        if not chart_bytes:
            ...
            c.setFont("Helvetica", 9)
            c.drawString(x + 0.5*cm, y + height/2, f"Chart nicht verfügbar: {chart_key}")
            return
        ...
        try:
            ...
            optimized_bytes = self._optimize_chart_image(chart_bytes, width, height)
            ...
            self.logger.log_error('ChartPageGenerator', f'Error drawing chart {chart_key}', e)
            c.setFont("Helvetica", 8)
            c.drawString(x + 0.5*cm, y + height/2, f"Fehler beim Laden: {chart_name}")
    
    def _optimize_chart_image(self, image_bytes: bytes, target_width: float, target_height: float) -> bytes:
        ...
        try:
            ...
            if img_width > target_width_px * 1.2 or img_height > target_height_px * 1.2:
                ...
                self.logger.log_info('ChartPageGenerator', f'Scaling chart from {img_width}x{img_height} to {new_width}x{new_height}')
            ...
            self.logger.log_warning('ChartPageGenerator', f'Could not optimize chart image: {e}')
