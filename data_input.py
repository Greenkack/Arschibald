...
try:
    ...
    from product_db import get_product_by_model_name as real_get_product_by_model_name
    ...
    # print(f"data_input.py: FEHLER Import DB/Produkt: {e}. Dummies bleiben
    # aktiv.") # Logging
...
def parse_full_address_string(
        full_address: str, texts: dict[str, str]) -> dict[str, str]:
    ...
    if street_hn_match:
        ...
        if len(
                potential_street.split()) > 0 and re.search(
                r'\d',
                potential_hn):
            ...
            # st.warning(get_text_di(texts, "parse_street_hnr_warning_detail",
            # "Straße und Hausnummer konnten nicht eindeutig getrennt werden.
            # Bitte manuell prüfen.")) # st.warning hier nur wenn absolut nötig
    elif address_part:
        parsed_data["street"] = address_part
        # st.warning(get_text_di(texts, "parse_street_hnr_not_found", "Keine
        # Hausnummer in der Adresse gefunden. Bitte manuell prüfen.")) #
        # st.warning hier nur wenn absolut nötig
    return parsed_data


def get_coordinates_from_address_google(address: str,
                                        city: str,
                                        zip_code: str,
                                        api_key: str | None,
                                        texts: dict[str,
    ...
    if not api_key or api_key == "" or api_key == "PLATZHALTER_HIER_IHREN_KEY_EINFUEGEN":
        ...
        # st.warning(get_text_di(texts,
        # "geocode_google_api_key_missing_or_placeholder_ui", "Google API Key
        # nicht konfiguriert. Geocoding deaktiviert.")) # Nur bei Bedarf im UI
        return None
    if not address or not city:
        # st.warning(get_text_di(texts, "geocode_missing_address_city", "Für
        # Geocoding werden Straße und Ort benötigt.")) # Nur bei Bedarf im UI
        return None
    ...
    try:
        ...
        if data.get("status") == "OK" and data.get("results"):
            location = data["results"][0].get(
                "geometry", {}).get(
                "location", {})
            lat, lng = location.get("lat"), location.get("lng")
            if lat is not None and lng is not None:
                # st.success(get_text_di(texts,
                # "geolocation_success_google_api", f"Koordinaten via Google
                # API: Lat {lat:.6f}, Lon {lng:.6f}")) # Nur bei Bedarf im UI
        ...
        # else: st.warning(get_text_di(texts,
        # "geolocation_google_api_status_error", f"Google API Fehler:
        # {data.get('status')} - {data.get('error_message', '')}")) # Nur bei
        # Bedarf
        return None
    except requests.exceptions.Timeout:
        # st.error(get_text_di(texts, "geolocation_google_api_timeout", "Google
        # Geocoding API Zeitüberschreitung.")) # Nur bei Bedarf
        return None
    except requests.exceptions.RequestException:
        # st.error(get_text_di(texts, "geolocation_google_api_request_error",
        # f"Google Geocoding API Anfragefehler: {e}")) # Nur bei Bedarf
        return None
    except Exception:
        # st.error(f"{get_text_di(texts, 'geolocation_api_unknown_error',
        # 'Unbekannter Fehler beim Geocoding:')} {e}") # Nur bei Bedarf
        return None


def get_Maps_satellite_image_url(latitude: float,
                                 longitude: float,
                                 api_key: str | None,
                                 texts: dict[str,
                                             str],
    ...
    if not api_key or api_key == "PLATZHALTER_HIER_IHREN_KEY_EINFUEGEN":
        ...
        # st.warning(get_text_di(texts,
        # "maps_api_key_missing_or_placeholder_ui", "Google Maps API Key nicht
        # konfiguriert. Satellitenbild kann nicht geladen werden.")) # Nur bei
        # Bedarf
        return None
    if abs(latitude) < 1e-9 and abs(longitude) < 1e-9 and not st.session_state.get(
            "allow_zero_coords_map_di", False):
        # st.info(get_text_di(texts, "map_default_coords_info",
        # "Standardkoordinaten (0,0) werden verwendet. Bitte gültige
        # Koordinaten eingeben oder Adresse parsen, um ein spezifisches
        # Satellitenbild zu laden.")) # Nur bei Bedarf
        return None
    base_url = "https://maps.googleapis.com/maps/api/staticmap?"
    params = {"center": f"{latitude},{longitude}", "zoom": str(
        zoom), "size": f"{width}x{height}", "maptype": "satellite", "key": api_key}
    url_parts = [f"{k}={v}" for k, v in params.items()]
    full_url = base_url + "&".join(url_parts)
    return full_url

# KORREKTUR: `render_data_input` modifiziert
# `st.session_state.project_data` direkt und gibt es nicht mehr zurück.
...
def render_data_input(texts: dict[str, str]) -> None:
    ...
    TITLE_OPTIONS = [str(t) if t is not None else get_text_di(
        texts, "none_option", "(Kein)") for t in TITLE_OPTIONS_RAW]
    ...
    MODULE_LIST_MODELS = [
        p.get(
            ...
            f"ID:{
                p.get(
                    'id',
                    'N/A')}") for p in list_products_safe(
            category='Modul')] or [
        get_text_di(
            texts,
            "no_modules_in_db",
        ...
        p.get(
            ...
            f"ID:{
                p.get(
                    'id',
                    'N/A')}") for p in list_products_safe(
            category='Batteriespeicher')] or [
        get_text_di(
            texts,
            "no_storages_in_db",
            "Keine Speicher in DB")]
    WALLBOX_LIST_MODELS = [
        p.get(
            'model_name',
            f"ID:{
        ...
        p.get(
            ...
            f"ID:{
                p.get(
                    'id',
                    'N/A')}") for p in list_products_safe(
            category='Energiemanagementsystem')] or [
        get_text_di(
            texts,
            "no_ems_in_db",
            "Keine EMS in DB")]
        ...
        p.get(
            ...
            f"ID:{
                p.get(
                    'id',
                    'N/A')}") for p in list_products_safe(
            category='Leistungsoptimierer')] or [
        get_text_di(
            texts,
            "no_optimizers_in_db",
            "Keine Optimierer in DB")]
    CARPORT_LIST_MODELS = [
        p.get(
            'model_name',
            f"ID:{
        ...
        p.get(
            ...
            f"ID:{
                p.get(
                    'id',
                    'N/A')}") for p in list_products_safe(
            category='Notstromversorgung')] or [
        get_text_di(
            texts,
            "no_notstrom_in_db",
            "Keine Notstrom in DB")]
    ...
    register_options = {
        "pv_only": "Nur Photovoltaik",
        "wp_only": "Nur Wärmepumpe",
        "pv_wp_combined": "Photovoltaik + Wärmepumpe"}

    # Get current value from session state
    current_mode = st.session_state.get(
        "demand_mode_selection", "pv_wp_combined")

    # Handle legacy mode values
    legacy_map_modes = {
    ...
    if selected_mode_code != current_mode:
        ...
        current_mode = selected_mode_code

    st.info(
        f"**Aktueller Modus:** {register_options.get(current_mode, current_mode)}")

    mode_info = []
    if current_mode in ['pv_only', 'pv_wp_combined']:
        mode_info.append(
            "Konfigurieren Sie die Photovoltaik-Anlage in den Projektdetails unten")
    if current_mode in ['wp_only', 'pv_wp_combined']:
        mode_info.append(
            "Konfigurieren Sie die Wärmepumpe in den entsprechenden Bereichen")
    if mode_info:
        st.info("**Nächste Schritte:** " + " | ".join(mode_info))
    ...
    with st.expander(get_text_di(texts, "customer_data_header", "Kundendaten eingeben"), expanded=st.session_state.customer_data_expanded_di):
            ...
            inputs['project_details']['anlage_type'] = st.selectbox(
                get_text_di(
                    ...
                    inputs['project_details'].get(
                        'anlage_type', 'Neuanlage')), key='anlage_type_di_v6_exp_stable')
        with col2:
            inputs['project_details']['feed_in_type'] = st.selectbox(
                get_text_di(
                    ...
                    'Teileinspeisung', 'Volleinspeisung'].index(
                    inputs['project_details'].get(
                        'feed_in_type', 'Teileinspeisung')), key='feed_in_type_di_v6_exp_stable')
        with col3:
            inputs['customer_data']['type'] = st.selectbox(
                get_text_di(
                    texts, "customer_type_label", "Kundentyp"), options=[
                    'Privat', 'Gewerblich'], index=[
                    'Privat', 'Gewerblich'].index(
                    inputs['customer_data'].get(
                        'type', 'Privat')), key='customer_type_di_v6_exp_stable')
        ...
        # Die Aktualisierung der `inputs` (und damit
        # `st.session_state.project_data`) reicht aus.
        if st.button(
                get_text_di(
                    texts,
                    "parse_address_button",
            ...
            if full_address_input_val:
                ...
                if 'customer_data' not in st.session_state.project_data:
                    st.session_state.project_data['customer_data'] = {}
                
                st.session_state.project_data['customer_data']['address'] = parsed_data.get("street", "")
                st.session_state.project_data['customer_data']['house_number'] = parsed_data.get("house_number", "")
                st.session_state.project_data['customer_data']['zip_code'] = parsed_data.get("zip_code", "")
                st.session_state.project_data['customer_data']['city'] = parsed_data.get("city", "")
                
                # Update inputs dict (für lokale Konsistenz)
                inputs['customer_data']['address'] = parsed_data.get("street", "")
                inputs['customer_data']['house_number'] = parsed_data.get("house_number", "")
        ...
        inputs['customer_data']['state'] = session_selectbox(
            ...
            index=state_options_with_ps.index(default_state)if default_state in state_options_with_ps else 0)
        if inputs['customer_data']['state'] == please_select_text:
            inputs['customer_data']['state'] = None
        st.markdown("---")
        st.markdown(
            f"**{get_text_di(texts, 'coordinates_header', 'Koordinaten')}**")
        Maps_API_KEY_FROM_ENV = os.environ.get("Maps_API_KEY")
        EFFECTIVE_GOOGLE_API_KEY = None
        if Maps_API_KEY_FROM_ENV and Maps_API_KEY_FROM_ENV.strip(
            ...
            if api_key_from_db and api_key_from_db.strip(
        ...
        with col_geocode_btn:
            ...
            if st.button(
                    get_text_di(
                        texts,
                        "get_coordinates_button",
                        "Koordinaten abrufen"),
                    key="geocode_btn_di_v6_exp_stable",
                    disabled=not EFFECTIVE_GOOGLE_API_KEY):
                addr_geo, city_geo, zip_geo = inputs['customer_data'].get(
                    'address', ''), inputs['customer_data'].get(
                    'city', ''), inputs['customer_data'].get(
                    'zip_code', '')
        ...
        if not (abs(current_lat) < 1e-9 and abs(current_lon) < 1e-9):
            st.map(pd.DataFrame(
                {'lat': [current_lat], 'lon': [current_lon]}), zoom=13)
        # elif EFFECTIVE_GOOGLE_API_KEY: st.info(get_text_di(texts,
        # "map_no_coordinates_info", "Keine Koordinaten für Kartenanzeige.
        # Bitte Adresse parsen oder Koordinaten manuell eingeben.")) # Optional
        st.markdown("---")
        st.markdown(f"**{get_text_di(texts,
                                     'satellite_image_header',
        ...
        if not (abs(current_lat) < 1e-9 and abs(current_lon) < 1e-9):
            if EFFECTIVE_GOOGLE_API_KEY:
                if st.button(
                    ...
                    st.session_state.satellite_image_url_di = get_Maps_satellite_image_url(
                        current_lat, current_lon, EFFECTIVE_GOOGLE_API_KEY, texts)
                    if st.session_state.satellite_image_url_di:
                        # st.success(get_text_di(texts,
                        # "satellite_image_url_generated", "URL für
                        # Satellitenbild generiert.")) # Optional
            ...
            if st.session_state.get('satellite_image_url_di'):
                # st.markdown(f"Generierte Bild-URL (für Vorschau):");
                # st.code(st.session_state.satellite_image_url_di) # Für
                # Debugging
                try:
                    st.image(
                    ...
                    if inputs['project_details'].get(
                        ...
                        if not inputs['project_details'].get('satellite_image_base64_data') or inputs['project_details'].get(
                                'satellite_image_for_pdf_url_source') != st.session_state.satellite_image_url_di:
                            try:
                                # Spinner ist gut für UI-Feedback
                                with st.spinner("Lade Satellitenbild für PDF..."):
                                    response = requests.get(
                                        st.session_state.satellite_image_url_di, timeout=15)
                                    response.raise_for_status()
                                    inputs['project_details']['satellite_image_base64_data'] = base64.b64encode(
                                        response.content).decode('utf-8')
                                    inputs['project_details']['satellite_image_for_pdf_url_source'] = st.session_state.satellite_image_url_di
                                    # st.success("Satellitenbild für PDF
                                    # vorbereitet.") # Optional
                            except Exception:
                                # st.warning(f"Satellitenbild konnte nicht für
                                # PDF heruntergeladen werden:
                                # {e_sat_download}") # Optional
                                inputs['project_details']['satellite_image_base64_data'] = None
                                inputs['project_details']['satellite_image_for_pdf_url_source'] = None
                except Exception:
                    # st.error(f"{get_text_di(texts,
                    # 'satellite_image_display_error', 'Fehler Anzeige
                    # Satellitenbild:')} {e_img}") # Optional
        ...
        # else: st.info(get_text_di(texts, "satellite_image_no_coords_info",
    ...
    with st.expander(get_text_di(texts, "consumption_data_header", "Verbrauche und Kosten eingeben"), expanded=st.session_state.consumption_data_expanded_di):
        ...
        inputs['project_details']['consumption_heating_kwh_yr'] = col_cons_heat.number_input(
            label=get_text_di(
                texts,
        ...
        total_consumption_kwh_yr_display = (
            inputs['project_details'].get(
                'annual_consumption_kwh_yr', 0) or 0) + (
            inputs['project_details'].get(
                'consumption_heating_kwh_yr', 0) or 0)
        st.info(
            f"{
                get_text_di(
                    texts,
                    'total_annual_consumption_label',
                    'Gesamtjahresverbrauch (Haushalt + Heizung)')}: {
                total_consumption_kwh_yr_display:.2f} kWh")
        ...
        if use_calculated_price:
            ...
            inputs['project_details']['costs_household_euro_mo'] = col_costs_hh.number_input(
                label=get_text_di(
                    texts,
            ...
            inputs['project_details']['costs_heating_euro_mo'] = col_costs_heat.number_input(
                label=get_text_di(
                    texts,
            ...
            st.info(
                f"{
                    get_text_di(
                        texts,
                        'total_annual_costs_display_label',
                        'Gesamte jährliche Stromkosten (berechnet)')}: {
                    total_annual_costs_calc:.2f} €")
            calculated_price_kwh = (
                total_annual_costs_calc /
                total_consumption_kwh_yr_display)if total_consumption_kwh_yr_display > 0 else 0.0
            inputs['project_details']['electricity_price_kwh'] = calculated_price_kwh
            st.info(
                f"{
                    get_text_di(
                        texts,
                        'calculated_electricity_price_info',
                        'Daraus resultierender Strompreis')}: {
                    calculated_price_kwh:.4f} €/kWh")
        else:
            inputs['project_details']['electricity_price_kwh'] = col_price_direct.number_input(
                label=get_text_di(
                    texts,
    ...
    with st.expander(get_text_di(texts, "building_data_header", "Daten des Gebäudes eingeben"), expanded=st.session_state.building_data_expanded_di):
        ...
        with col_build1:
            ...
            # else:st.success(get_text_di(texts,"build_year_success_new","
            # Zählerschrank/Hauselektrik OK.")) # Optional
        with col_build2:
            ...
            inputs['project_details']['roof_type'] = st.selectbox(
                ...
                index=roof_type_options_with_ps.index(default_roof_type)if default_roof_type in roof_type_options_with_ps else 0,
        ...
        with col_build3:
            ...
            roof_covering_options_with_ps = [
                please_select_text] + DACHDECKUNG_OPTIONS
            inputs['project_details']['roof_covering_type'] = st.selectbox(
                ...
                index=roof_covering_options_with_ps.index(default_roof_covering)if default_roof_covering in roof_covering_options_with_ps else 0,
            ...
            # elif
            # inputs['project_details']['roof_covering_type']:st.success(get_text_di(texts,"roof_covering_info","
            # Dachbelegung problemlos.")) # Optional
        with col_build4:
        ...
        with col_build5:
            orientation_options = [please_select_text] + ['Süd',
                                                          'Südost',
                                                          'Ost',
                                                          'Südwest',
                                                          'West',
            ...
            inputs['project_details']['roof_orientation'] = st.selectbox(
                get_text_di(
                    texts,
                    "roof_orientation_label",
                    "Dachausrichtung"),
                options=orientation_options,
                index=orientation_options.index(default_orientation)if default_orientation in orientation_options else 0,
        ...
        inputs['project_details']['building_height_gt_7m'] = st.checkbox(
            ...
            value=inputs['project_details'].get(
    ...
    inputs['project_details']['future_ev'] = st.checkbox(
        get_text_di(
            texts,
            "future_ev_checkbox_label",
            "Zukünftiges E-Auto einplanen"),
        value=inputs['project_details'].get(
            'future_ev',
            False),
    ...
    inputs['project_details']['future_hp'] = st.checkbox(
        get_text_di(
            texts,
            "future_hp_checkbox_label",
            "Zukünftige Wärmepumpe einplanen"),
        value=inputs['project_details'].get(
            'future_hp',
            False),
    ...
    #         # Keine manuelle Speicher-kWh-Eingabe mehr: Speicherkapazität
    #         # kommt ausschließlich aus dem ausgewählten Produktmodell.
    #     else:inputs['project_details']['selected_storage_name'],inputs['project_details']['selected_storage_id']=None,None
    ...
    if inputs['customer_data']['financing_requested']:
        ...
        with st.expander(" **Persönliche Daten (Hauptantragsteller)**", expanded=False):
            ...
            with col_resident_since:
                inputs['customer_data']['resident_since'] = st.date_input(
                    "Wohnhaft an aktueller Adresse seit", value=inputs['customer_data'].get(
                        'resident_since', datetime(
                            2020, 1, 1).date()), key='resident_since_financing')
            with col_marital:
                marital_options = [
                    'Ledig',
            ...
            if inputs['customer_data']['marital_status'] in [
                    'Verheiratet/Verpartnert']:
        ...
        with st.expander(" **Berufs- & Einkommenssituation**", expanded=False):
            ...
            if inputs['customer_data']['employment_type'] == 'Befristet':
                inputs['customer_data']['employment_end_date'] = st.date_input(
                    "Befristet bis", value=inputs['customer_data'].get(
                        'employment_end_date', datetime(
                            2025, 12, 31).date()), key='employment_end_date_financing')
            ...
            if inputs['customer_data']['other_monthly_income'] > 0:
                inputs['customer_data']['other_income_description'] = st.text_input(
                    "Beschreibung der sonstigen Einkünfte", value=inputs['customer_data'].get(
                        'other_income_description', ''), key='other_income_description_financing')
        ...
        with st.expander(" **Finanzierungsdetails zum Photovoltaik-Vorhaben**", expanded=False):
            # Gesamtkosten werden aus der Konfiguration übernommen - hier als
            # Platzhalter
            estimated_total_costs = 25000.0  # Wird später aus calculations übernommen
            st.info(
                f" Geschätzte Gesamtkosten der Photovoltaik-Anlage: {estimated_total_costs:,.2f} €")
            ...
            if financing_type == "Bankkredit (Annuität)":
                ...
                with col_loan_term:
                    inputs['customer_data']['loan_term_years'] = st.number_input(
                        "Gewünschte Laufzeit (Jahre)", min_value=5, max_value=30, value=int(
                            inputs['customer_data'].get(
                                'loan_term_years', 15)), key='loan_term_years_financing')
                ...
                with col_leasing_term:
                    inputs['customer_data']['leasing_term_months'] = st.number_input(
                        "Leasinglaufzeit (Monate)", min_value=24, max_value=240, value=int(
                            inputs['customer_data'].get(
                                'leasing_term_months', 120)), step=12, key='leasing_term_months_financing')
    ...
    with st.expander(get_text_di(texts, "economic_data_header", "Wirtschaftliche Parameter eingeben"), expanded=st.session_state.economic_data_expanded_di):
        ...
        inputs['economic_data']['electricity_price_increase_annual_percent'] = st.number_input(
            label=get_text_di(
                texts,
...
# Die st.warning und st.info Meldungen im UI wurden auskommentiert, um die
# Ausgabe bei normalem Betrieb sauberer zu halten. Sie können für
# Debugging bei Bedarf wieder aktiviert werden.
