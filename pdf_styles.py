...
class PDFThemeManager:
    ...
    def _create_enhanced_generation_chart(
            self, ax, data, theme_config, **kwargs):
    ...
    def _draw_circle(self, x, y, radius):
        ...
        return Circle((x, y), radius, facecolor='lightcoral',
                      edgecolor='darkred')
        ...
        with tab1:
            ...
            chart_type = st.selectbox(
                ...
                format_func=lambda x: {
                    ...
                    'roi_dashboard': 'ROI Dashboard'}.get(
                    x,
                    x))
    ...
    def render_theme_selector(self):
        ...
        if selected_theme in all_themes:
            ...
            with st.expander(" Theme-Details", expanded=False):
                ...
                cols = st.columns(4)
                for idx, (color_name, color_value) in enumerate(
                        colors.items()):
                    with cols[idx % 4]:
                        st.color_picker(
        ...
        with tabs[3]:  # Effekte
            ...
            effects = {
                ...
                'page_numbers': st.checkbox(
                    "Seitenzahlen",
                    value=True),
                'header_line': st.checkbox(
                    "Kopfzeilen-Linie",
                    value=True),
                'footer_line': st.checkbox(
                    "Fußzeilen-Linie",
                    value=True),
                'section_borders': st.checkbox(
                    "Abschnitts-Rahmen",
                    value=False),
                'highlight_important': st.checkbox(
    ...
    def _generate_color_palette(self, base_color: str) -> list[str]:
        ...
        for i in range(5):
            ...
            hex_color = f'#{int(new_rgb[0] *
                                255):02x}{int(new_rgb[1] *
                                              255):02x}{int(new_rgb[2] *
                                                            255):02x}'
            ...
            if st.button(" Aktuelles Theme exportieren"):
                ...
                if current_theme in all_themes:
                    ...
                    st.download_button(
                        ...
                        data=theme_json,
                        file_name=f"pdf_theme_{current_theme}_{
...
def get_pdf_stylesheet(color_palette: dict[str, colors.Color]) -> StyleSheet1:
    ...
    stylesheet.add(
        ParagraphStyle(
            name='HeaderFooterText',
            parent=stylesheet['Normal'],
            fontSize=8,
            textColor=color_palette['darkgrey']))
    stylesheet.add(ParagraphStyle(name='PageInfo',
                   parent=stylesheet['HeaderFooterText'], alignment=TA_CENTER))
    stylesheet.add(
        ParagraphStyle(
            name='DocumentTitleHeader',
            parent=stylesheet['HeaderFooterText'],
            fontName=FONT_FAMILY_BOLD,
            alignment=TA_RIGHT,
            fontSize=9))
    stylesheet.add(
        ParagraphStyle(
            name='CompanyAddressFooter',
            parent=stylesheet['HeaderFooterText'],
            fontSize=7,
            alignment=TA_LEFT))
    stylesheet.add(
    ...
    stylesheet.add(
        ParagraphStyle(
            name='ComponentTitle',
            parent=stylesheet['SubSectionTitle'],
            fontSize=11,
            spaceBefore=0.6 * cm,
            spaceAfter=0.3 * cm,
            alignment=TA_LEFT,
    ...
    stylesheet.add(
        ParagraphStyle(
            ...
            parent=stylesheet['NormalCenter'],
            fontSize=9,
            leading=11,
            spaceAfter=0.8 * cm,
            textColor=color_palette['text']))
    stylesheet.add(
        ParagraphStyle(
            name='CustomerAddressDeckblatt',
            parent=stylesheet['NormalLeft'],
            fontSize=11,
            leading=14,
            spaceBefore=2 * cm,
            spaceAfter=1 * cm,
    ...
    stylesheet.add(
        ParagraphStyle(
            name='OfferDetailsDeckblatt',
            parent=stylesheet['NormalRight'],
            fontSize=10,
            leading=12,
            spaceBefore=0.5 * cm))
    stylesheet.add(
        ParagraphStyle(
            name='CoverLetter',
            parent=stylesheet['NormalJustify'],
            fontSize=11,
            leading=15,
            spaceBefore=0.5 * cm,
            spaceAfter=0.5 * cm,
    ...
    stylesheet.add(
        ParagraphStyle(
            ...
            backColor=color_palette['primary']))
    stylesheet.add(
        ParagraphStyle(
            name='TableBoldRight',
            parent=stylesheet['TableNumber'],
            fontName=FONT_FAMILY_BOLD))
    stylesheet.add(
        ParagraphStyle(
            name='ImageCaption',
...
def get_base_table_style(color_palette: dict[str, colors.Color]) -> TableStyle:
    return TableStyle([('TEXTCOLOR', (0, 0), (-1, -1), color_palette['text']), ('FONTNAME', (0, 0), (-1, -1), FONT_FAMILY_NORMAL), ('FONTSIZE', (0, 0), (-1, -1), 9), ('LEADING', (0, 0), (-1, -1), 11), ('GRID', (0, 0), (-1, -1), 0.5,
                      color_palette['separator']), ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'), ('LEFTPADDING', (0, 0), (-1, -1), 3 * mm), ('RIGHTPADDING', (0, 0), (-1, -1), 3 * mm), ('TOPPADDING', (0, 0), (-1, -1), 2 * mm), ('BOTTOMPADDING', (0, 0), (-1, -1), 2 * mm)])


def get_data_table_style(color_palette: dict[str, colors.Color]) -> TableStyle:
    return TableStyle([('BACKGROUND', (0, 0), (-1, 0), color_palette['primary']), ('TEXTCOLOR', (0, 0), (-1, 0), color_palette['white']), ('FONTNAME', (0, 0), (-1, 0), FONT_FAMILY_BOLD), ('ALIGN', (0, 0), (-1, 0), 'CENTER'), ('GRID', (0, 0), (-1, -1), 0.5, color_palette['separator']), ('VALIGN', (0, 0), (-1, -1), 'MIDDLE'), ('FONTNAME', (0, 1), (-1, -1), FONT_FAMILY_NORMAL),
                      ('FONTSIZE', (0, 1), (-1, -1), 9), ('ALIGN', (0, 1), (0, -1), 'LEFT'), ('ALIGN', (1, 1), (-1, -1), 'RIGHT'), ('LEFTPADDING', (0, 0), (-1, -1), 2 * mm), ('RIGHTPADDING', (0, 0), (-1, -1), 2 * mm), ('TOPPADDING', (0, 0), (-1, -1), 1.5 * mm), ('BOTTOMPADDING', (0, 0), (-1, -1), 1.5 * mm), ('TEXTCOLOR', (0, 1), (-1, -1), color_palette['text'])])


def get_product_table_style(
        color_palette: dict[str, colors.Color]) -> TableStyle:
    return TableStyle([('TEXTCOLOR', (0, 0), (-1, -1), color_palette['text']), ('FONTNAME', (0, 0), (0, -1), FONT_FAMILY_BOLD), ('ALIGN', (0, 0), (0, -1), 'LEFT'), ('FONTNAME', (1, 0), (1, -1), FONT_FAMILY_NORMAL), ('ALIGN', (1, 0), (1, -1),
                      'LEFT'), ('VALIGN', (0, 0), (-1, -1), 'TOP'), ('LEFTPADDING', (0, 0), (-1, -1), 2 * mm), ('RIGHTPADDING', (0, 0), (-1, -1), 2 * mm), ('TOPPADDING', (0, 0), (-1, -1), 1.5 * mm), ('BOTTOMPADDING', (0, 0), (-1, -1), 1.5 * mm)])


def get_main_product_table_style() -> TableStyle:
    return TableStyle([('VALIGN', (0, 0), (-1, -1), 'TOP'), ('LEFTPADDING', (0, 0), (-1, -1), 0), ('RIGHTPADDING',
                      (0, 0), (-1, -1), 0), ('TOPPADDING', (0, 0), (-1, -1), 0), ('BOTTOMPADDING', (0, 0), (-1, -1), 0)])
# Änderungshistorie
