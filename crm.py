...
def render_customer_management(texts: dict[str, str], conn):
    ...
    if view_mode == 'customer_list':
        ...
        with st.form("project_form", clear_on_submit=False):
            ...
            selected_storage_capacity_kwh = 0.0
            ...
            if include_storage:
                ...
                selected_storage_id = next(
                    ...
                    None)
                selected_storage_product = next(
                    (p for p in storage_products if p.get('id') == selected_storage_id),
                    None)
                for cap_key in (
                    'capacity_kwh',
                    'usable_capacity_kwh',
                    ...
                    try:
                        selected_storage_capacity_kwh = float(
                            (selected_storage_product or {}).get(cap_key) or 0.0)
                    except (TypeError, ValueError):
                        selected_storage_capacity_kwh = 0.0
                    if selected_storage_capacity_kwh > 0:
                        break
                st.metric(
                    get_text_crm(texts, "storage_capacity_product_label", "Kapazität Modell (kWh)"),
                    f"{selected_storage_capacity_kwh:.2f}".replace('.', ','))
        ...
        if project_details.get('include_storage'):
            ...
            st.write(
                f"**{
                    get_text_crm(
                        texts,
                        'storage_capacity_product_label',
                        'Speicherkapazität Modell')}:** {
                    project_details.get(
