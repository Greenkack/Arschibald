...
class PDFSystemManager:
    ...
    def _sanitize_xml_content(self, text: str) -> str:
        ...
        # Entferne problematische Emoji-Kombinationen die XML-Probleme
        # verursachen
    ...
    def _create_safe_inclusion_options(
            self, original_options: dict[str, Any]) -> dict[str, Any]:
    ...
    def _initialize_systems(self):
        ...
        # TOM-90 System
        try:
            # Verwende den RICHTIGEN TOM-90 Renderer mit Business-Sections
            # Support
        ...
        try:
            ...
            # Fallback: Preview als verfügbar markieren auch ohne spezielles
            # Modul
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    if company_info:
                        st.info(
                            " Verwende Business-Sections PDF als TOM-90 Fallback...")
                        return generate_business_sections_pdf(company_info)
                except BaseException:
    ...
    def _dummy_mega_hybrid_pdf(self, *args, **kwargs):
        ...
        try:
            ...
            # Falls das System Debug unterstützt
            debug_kwargs['debug_mode'] = True
        ...
        for system_name, system_func in fallback_systems:
            try:
                ...
                if result:
                    st.success(
                        f" {system_name} erfolgreich als Mega Hybrid Ersatz verwendet!")
                    return result
            except Exception as e:
    ...
    def _dummy_preview(self, *args, **kwargs):
        st.info(
            " PDF-Vorschau-System (Fallback) - Verwende beste verfügbare PDF-Generierung")
    ...
    def _try_standard_pdf_generator(self, *args, **kwargs):
        ...
        with debug_timer(
            ...
            try:
                from pdf_generator import generate_offer_pdf
                result = generate_offer_pdf(*args, **kwargs)
            ...
            if "Parse error" in error_msg and (
                    "paragraph text" in error_msg or "saw </para>" in error_msg):
                st.warning(
                    " XML-Parsing-Problem erkannt - verwende abgesicherte Parameter...")
                debug_log(
                    "central_pdf._try_standard_pdf_generator",
                ...
                safe_inclusion_options = safe_kwargs.get(
                    'inclusion_options', {}).copy()

                # Deaktiviere alle Business-Sektionen (können XML-Probleme
                # verursachen)
                business_keys = [key for key in safe_inclusion_options.keys(
                ) if 'business' in key.lower() or 'warranty' in key.lower()]
                for key in business_keys:
                    safe_inclusion_options[key] = False

                # Deaktiviere erweiterte Sektionen die XML-Probleme verursachen
                # können
                problematic_keys = [
                    'include_business_warranty',
                    'include_warranty_tom90',
                ...
                try:
                    ...
                    st.warning(
                        f" Auch abgesicherte Parameter fehlgeschlagen: {e2}")
                    debug_log(
                        "central_pdf._try_standard_pdf_generator",
                    ...
                    try:
                        ...
                        result = generate_offer_pdf(**minimal_kwargs)
                        if result and debug_enabled():
                            log_payload(
                                "central_pdf._try_standard_pdf_generator",
                        ...
                        st.error(
                            f" Alle Standard-PDF Versuche fehlgeschlagen: {e3}")
                        raise e3
            else:
            ...
            if company_info:
                ...
                business_params = {
                    'include_business_company_profile': inclusion_options.get(
                        'include_business_company_profile', False), 'include_business_certifications': inclusion_options.get(
                        'include_business_certifications', False), 'include_business_references': inclusion_options.get(
                        'include_business_references', False), 'include_business_installation': inclusion_options.get(
                        'include_business_installation', False), 'include_business_maintenance': inclusion_options.get(
                        'include_business_maintenance', False), 'include_business_financing': inclusion_options.get(
                            'include_business_financing', False), 'include_business_insurance': inclusion_options.get(
                                'include_business_insurance', False), 'include_business_warranty': inclusion_options.get(
                                    'include_business_warranty', False)}

                # Erste Versuch mit allen Business-Parametern
                try:
                    return generate_business_sections_pdf(
                        company_info, **business_params)
                except Exception as e:
                    # Falls XML-Fehler, deaktiviere problematische Sektionen
                    if "Parse error" in str(e):
                        st.warning(
                            " Business-Sections XML-Problem - deaktiviere problematische Bereiche...")
                        ...
                        try:
                            return generate_business_sections_pdf(
                                company_info, **safe_business_params)
                        except Exception:
                            # Letzter Versuch: Nur sichere Sektionen
                            st.info(
                                " Verwende nur sichere Business-Sektionen...")
                            ...
                            return generate_business_sections_pdf(
                                company_info, **minimal_business_params)
                    else:
                        raise e
            return None
        except ImportError:
        ...
        try:
            ...
            if project_data and analysis_results and company_info:
                try:
                    ...
                    if "Parse error" in str(e):
                        st.warning(
                            " TOM-90 XML-Problem - verwende sichere Parameter...")

                        safe_inclusion_options = kwargs.get(
                            'inclusion_options', {}).copy()
    ...
    def _try_emergency_fallback(self, *args, **kwargs):
        ...
        if project_data:
            ...
            p.drawString(
                70,
                y_position,
                f"• Batteriekapazität: {battery_capacity} kWh")
        ...
        if analysis_results:
            ...
            p.drawString(
                70,
                y_position,
                f"• Gesamtinvestition: {total_investment} €")
            y_position -= 20
            p.drawString(
                70,
                y_position,
                f"• Amortisationszeit: {payback_period} Jahre")
            y_position -= 20
            p.drawString(
                70,
                y_position,
                f"• Jährliche Einsparung: {annual_savings} €")
        ...
        p.drawString(
            50,
            y_position,
            "Dieses PDF wurde mit dem Notfall-System erstellt, da die")
        y_position -= 15
        p.drawString(
            50,
            y_position,
            "Standard-PDF-Generatoren nicht verfügbar waren.")
        y_position -= 15
        p.drawString(
            50,
            y_position,
            "Für ein vollständiges Angebot wenden Sie sich bitte an den Administrator.")
        ...
        p.drawString(
            50, y_position, f"Generiert von: Zentrale PDF-System Architektur | {
    ...
    def generate_pdf(
        ...
        with debug_timer(
            ...
            if chosen_layout == "auto":
                chosen_layout = self.get_best_available_system()
                debug_log(
                    "central_pdf.generate_pdf",
    ...
    def get_system_status(self) -> dict[str, bool]:
        ...
        # Preview prüfen
        try:
            status['preview'] = st.session_state.get(
...
class CentralPDFInterface:
    ...
    def _initialize_session_state(self):
        ...
        defaults = {
            # Standard: Mega Hybrid für vollständige PDFs
            f"{self.session_prefix}layout_choice": "mega_hybrid",
    ...
    def render_content_options(self,
        ...
        with tab1:
            ...
            with quick_col3:
                if st.button(
                    " Vorlagen",
                    key="show_templates_basic",
                        help="Vorlagen verwalten"):
                    st.session_state['show_template_manager'] = not st.session_state.get(
                ...
                with st.expander(" Vorlagen-Manager", expanded=True):
                    ...
                    with template_col1:
                        ...
                        if st.button(" Speichern",
                                     key="save_new_template") and template_name:
                            ...
                            st.session_state["pdf_templates"].append(
                                template_config)
                            st.success(
                                f" Vorlage '{template_name}' gespeichert!")
                            request_rerun()

                    with template_col2:
                        st.markdown("**Gespeicherte Vorlagen:**")
                        saved_templates = st.session_state.get(
                            "pdf_templates", [])
                            ...
                            for i, template in enumerate(saved_templates):
                                ...
                                with template_sub_col1:
                                    if st.button(
                                        ...
                                        st.session_state[f"{self.session_prefix}selected_main_sections"] = template.get(
                                            'selected_sections', [])
                                        st.session_state[f"{self.session_prefix}theme_name"] = template.get(
                                ...
                                with template_sub_col2:
                                    if st.button(
                                            "", key=f"delete_template_{i}", help="Vorlage löschen"):
            ...
            with col1:
                current_options["include_company_logo"] = st.checkbox(
                    " Firmenlogo", value=current_options.get("include_company_logo", True))
                current_options["include_product_images"] = st.checkbox(
                    " Produktbilder", value=current_options.get("include_product_images", True))
            ...
            with col2:
                current_options["include_all_documents"] = st.checkbox(
                    " Alle Datenblätter", value=current_options.get(
                        "include_all_documents", True))
                current_options["include_enhanced_charts"] = st.checkbox(
                    " Erweiterte Diagramme", value=current_options.get(
                        "include_enhanced_charts", True))
            ...
            st.caption(
                "Zusätzliche professionelle Geschäftsbereiche für die PDF")
            ...
            with business_col2:
                ...
                current_options["include_business_warranty"] = st.checkbox(
                    " Garantieleistungen", value=current_options.get(
                        "include_business_warranty", False), help="Garantien und Gewährleistungsumfang")
            ...
            if active_company_id and callable(db_list_company_documents_func):
                ...
                st.info(
                    "Keine Firmendaten verfügbar - Dokumentauswahl nicht möglich")

            current_options["company_document_ids_to_include"] = selected_doc_ids

            st.markdown("####  Firmenspezifische Vorlagen")
            st.caption(
                "Nutzen Sie gespeicherte Text- und Bildvorlagen Ihrer Firma")
                    ...
                    if company_text_templates:
                        ...
                        for i, template in enumerate(company_text_templates):
                            with text_template_cols[i % 2]:
                                ...
                                template_preview = template.get('content', '')[
                                    :50] + "..." if len(template.get('content', '')) > 50 else template.get('content', '')
                                template_type = template.get(
                                    'template_type', 'Standard')

                                if st.checkbox(
                                        f" {template_name} ({template_type})",
                                        key=f"company_text_template_{
                                            template.get('id')}",
                        ...
                        st.info(
                            " Keine firmenspezifischen Textvorlagen verfügbar")
                        st.caption(
            ...
            if active_company_id:
                try:
                    ...
                    if company_image_templates:
                        ...
                        st.info(
                            " Keine firmenspezifischen Bildvorlagen verfügbar")
                        st.caption(
            ...
            # Aktiviere alle verfügbaren Firmenvorlagen automatisch wenn "Alles
            # auswählen" geklickt wird
            ...
            content_add_col1, content_add_col2, content_add_col3 = st.columns(
                3)
            ...
            if custom_items:
                ...
                for i, item in enumerate(custom_items):
                    with st.expander(f"{'' if item['type'] == 'text' else ''} {item['type'].title()} #{i + 1}: {item.get('title', 'Ohne Titel')}", expanded=True):
                        ...
                        with item_col3:
                            ...
                            if st.button(
                                "", key=f"move_up_{i}", disabled=(
                                    i == 0), help="Nach oben verschieben"):
                                if i > 0:
                                    custom_items[i], custom_items[i -
                                                                  1] = custom_items[i -
                                                                                    1], custom_items[i]
                ...
                enabled_items = [
                    item for item in custom_items if item.get(
                        "enabled", False)]
                if enabled_items:
                ...
                # Speichere die Items in current_options für die
                # PDF-Generierung
            ...
            section_quick_col1, section_quick_col2, section_quick_col3, section_quick_col4 = st.columns(
                4)
            ...
            with section_quick_col2:
                if st.button(
                    ...
                    core_sections = [s[0]
                                     for s in section_groups["Kern-Bereiche"]]
            ...
            for group_name, group_sections in section_groups.items():
                with st.expander(f" {group_name}", expanded=True):
                    ...
                    for i, (section_id, section_name,
                            section_desc) in enumerate(group_sections):
                        with group_cols[i % 2]:
                            if st.checkbox(
                                section_name,
            ...
            if updated_sections:
                st.markdown(
                    "** Ausgewählte Sektionen - Reihenfolge anpassen:**")
                ...
                # Nur die tatsächlich ausgewählten Sektionen in der richtigen
                # Reihenfolge zeigen
                ordered_selected = [
                    s for s in current_order if s in updated_sections]
                ...
                with reorder_container:
                    for i, section in enumerate(ordered_selected):
                        ...
                        reorder_col1, reorder_col2, reorder_col3, reorder_col4 = st.columns(
                            [0.1, 0.6, 0.15, 0.15])
                        ...
                        with reorder_col3:
                            if st.button(
                                "", key=f"up_{section}_{i}", disabled=(
                                    i == 0), help="Nach oben verschieben"):
                                if i > 0:
                                    ordered_selected[i], ordered_selected[i - \
                                        1] = ordered_selected[i - 1], ordered_selected[i]
        ...
        with tab2:
            ...
            if analysis_results:
                ...
                for i, (chart_key, chart_name) in enumerate(
                        chart_mapping.items()):
                    with chart_cols[i % 2]:
                        # Prüfe ob Chart in den Analyseergebnissen vorhanden
                        # ist
            ...
            with chart_enh_cols[0]:
                current_options["include_chart_descriptions"] = st.checkbox(
                    " Beschreibungen", value=current_options.get(
                        "include_chart_descriptions", True), help="Erklärende Texte unter Diagrammen")
        ...
        with tab6:
            ...
            if st.button(" Als Vorlage speichern") and template_name:
                ...
                # Speichere in Session State (könnte später in DB gespeichert
                # werden)
                if "pdf_templates" not in st.session_state:
                    st.session_state["pdf_templates"] = []
                ...
                st.success(
                    f" Vorlage '{template_name}' erfolgreich gespeichert!")
            ...
            if saved_templates:
                ...
                for i, template in enumerate(saved_templates):
                    with st.expander(f" {template.get('name', f'Vorlage {i + 1}')}"):
                        ...
                        st.write(
                            f"**Theme:** {template.get('theme_name', 'Standard')}")
                        ...
                        with col1:
                            if st.button(
                                " Vorlage laden",
                                    key=f"load_template_{i}"):
                                # Lade Vorlage
                                st.session_state[f"{self.session_prefix}inclusion_options"] = template.get(
                                    'inclusion_options', {})
                                st.session_state[f"{self.session_prefix}selected_main_sections"] = template.get(
                                    'selected_sections', [])
                                st.session_state[f"{self.session_prefix}theme_name"] = template.get(
                        ...
                        with col2:
                            if st.button(
                                    " Löschen", key=f"delete_template_{i}"):
                                saved_templates.pop(i)
                                st.session_state["pdf_templates"] = saved_templates
                                st.success(" Vorlage gelöscht!")
                                request_rerun()
    ...
    def render_template_selection(self,
        ...
        # Templates laden
        try:
            title_templates = load_admin_setting_func(
                'pdf_title_image_templates', [])
            offer_templates = load_admin_setting_func(
                'pdf_offer_title_templates', [])
            letter_templates = load_admin_setting_func(
                ...
                selected_title = st.selectbox(
                    ...
                    options=[
                        t.get(
                            ...
                            f"Bild {
                                i + 1}") for i,
                        t in enumerate(title_templates)],
                ...
                selected_offer = st.selectbox(
                    ...
                    options=[
                        t.get(
                            ...
                            f"Titel {
                                i + 1}") for i,
                        t in enumerate(offer_templates)],
                ...
                selected_letter = st.selectbox(
                    ...
                    options=[
                        t.get(
                            ...
                            f"Brief {
                                i + 1}") for i,
                        t in enumerate(letter_templates)],
    ...
    def generate_pdf_central(self,
        ...
        try:
            ...
            pdf_bytes = PDF_MANAGER.generate_pdf(
                ...
                selected_offer_title_text=template_data.get(
                    'offer_title_text'),
                selected_cover_letter_text=template_data.get(
                    'cover_letter_text'),
                sections_to_include=inclusion_options.get(
                    'selected_sections', []),
                texts=texts,
                inclusion_options=inclusion_options,
                # Übergebe ALLE inclusion_options an das System
                ...
                include_chart_descriptions=inclusion_options.get(
                    'include_chart_descriptions', True),
                include_chart_kpis=inclusion_options.get(
                    'include_chart_kpis', True),
            ...
            if pdf_bytes:
                st.success(
                    f" PDF erfolgreich generiert! Layout: {layout_choice}")
                return pdf_bytes
            st.error(
                f" PDF-Generierung fehlgeschlagen für Layout: {layout_choice}")
            return None
    ...
    def render_main_interface(self,
                              ...
                              get_active_company_details_func: Callable[[], dict[str, Any] | None],
        ...
        st.markdown(
            "Alle PDF-Systeme vereint an einem Ort - keine Duplikation mehr!")
        ...
        template_data = self.render_template_selection(
        ...
        if generate_button:
            ...
            try:
                ...
                if pdf_bytes:
                    ...
                    customer_name = project_data.get(
                        'customer_data', {}).get(
                        'last_name', 'Kunde')
                    ...
                    st.success(
                        f" PDF mit {
                            selected_layout.upper()} Layout erfolgreich erstellt!")
...
def cleanup_old_pdf_session_state():
    ...
    if cleaned_count > 0:
        st.info(
            f"🧹 {cleaned_count} alte PDF Session State Variablen bereinigt")
...
def show_pdf_system_status():
    ...
    with col1:
        ...
        st.write(
            " Initialisiert" if status['manager_initialized'] else " Fehler")
