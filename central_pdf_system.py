...
class PDFSystemManager:
    ...
    def generate_pdf(self, layout_choice: str, *args, **kwargs) -> bytes | None:
            ...
            if saved_templates:
                ...
                for i, template in enumerate(saved_templates):
                    with st.expander(f" {template.get('name', f'Vorlage {i+1}')}"):
                        ...
                        col1, col2 = st.columns(2)
                        with col1:
                            if st.button(" Vorlage laden", key=f"load_template_{i}"):
                                # Lade Vorlage
                        ...
                        with col2:
                            if st.button(" Löschen", key=f"delete_template_{i}"):
                                saved_templates.pop(i)
                                st.session_state["pdf_templates"] = saved_templates
                                st.success(" Vorlage gelöscht!")
                                request_rerun()
