...
def render_multi_pdf_customer_input() -> bool:
    ...
    if auto_customer_data and any(auto_customer_data.values()):
        ...
        with col1:
            ...
            st.text_input(
                "Straße", value=auto_customer_data.get(
                    "street", ""), disabled=True)
        with col2:
            st.text_input(
                "PLZ", value=auto_customer_data.get(
                    "zip_code", ""), disabled=True)
            st.text_input(
                "Ort", value=auto_customer_data.get(
                    "city", ""), disabled=True)
            ...
            with col_individual:
                for cid in selected_ids:
                    ...
                    st.session_state.multi_offer_company_extended[cid] = st.checkbox(
                        f"📄 {company_name}", value=st.session_state.multi_offer_company_extended.get(
                            cid, False), key=f"ext_{cid}")
        ...
        with col_mode:
            ...
            if settings["rotation_mode"] == "kategorie-spezifisch":
                ...
                settings["module_rotation_step"] = st.slider(
                    "Module", 1, 10, settings.get("module_rotation_step", 1))
                settings["inverter_rotation_step"] = st.slider(
                    "Wechselrichter", 1, 10, settings.get(
                        "inverter_rotation_step", 1))
                settings["storage_rotation_step"] = st.slider(
                    "Speicher", 1, 10, settings.get("storage_rotation_step", 1))
...
def render_heatpump_integration_toggle() -> bool:
    ...
    if enabled:
        ...
        if st.session_state.get("heatpump_calculation_data"):
            ...
            st.info(
                "ℹ️ Keine Wärmepumpen-Daten verfügbar. Bitte erst Bedarfsanalyse durchführen.")
            return False
    ...
    try:
        ...
        for category in categories:
            ...
            try:
                ...
                logging.warning(
                    f"Produktrotation {category}: Fehler beim Laden: {e}")
                continue
            ...
            new_index = (base_index + (company_index * rotation_step)
...
def prepare_offer_data(
    ...
    return {
        ...
        "module_quantity": rotated_settings.get(
            "module_quantity",
            20),
        ...
        "include_storage": rotated_settings.get(
            "include_storage",
            True),
        "extended_output": st.session_state.multi_offer_company_extended.get(
            company["id"],
            False),
        "heatpump_enabled": st.session_state.multi_offer_heatpump_enabled,
        "heatpump_data": st.session_state.get(
            "heatpump_calculation_data",
            {}),
    }


def generate_company_pdf(offer_data: dict) -> bytes | None:
    """
...
def batch_generate_offers():
    ...
    if st.button(
        "🎯 Angebote für alle Firmen erstellen",
        type="primary",
        ...
        try:
            ...
            for i, company_id in enumerate(selected_companies):
                ...
                try:
                    ...
                    status_text.text(
                        f"🔄 Erstelle Angebot für {company_name} ({
                            i + 1}/{total_companies})...")

                    # Angebotsdaten vorbereiten mit Rotation & Preisstaffelung
                    offer_data = prepare_offer_data(
                        customer_data, company, settings, project_data, i)

                    # PDF generieren
                    pdf_content = generate_company_pdf(offer_data)

                    if pdf_content:
                        filename = f"Angebot_{company_name}_{
                            customer_data.get(
                                'last_name', 'Kunde')}.pdf"
                    ...
                    logging.error(
                        f"Fehler bei PDF-Generierung für {company_name}: {e}")
                    continue
            ...
            if generated_pdfs:
                ...
                render_crm_integration(
                    customer_data, generated_pdfs, project_data)
...
def render_multi_pdf_generator():
    ...
    selected_company_ids = render_multi_pdf_company_selection(
        available_companies)
