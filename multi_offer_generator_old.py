...
try:
    ...
    from pdf_generator import create_offer_pdf, generate_offer_pdf, merge_pdfs
            ...
            with zipfile.ZipFile(
                ...
                for company_id in selected_company_ids:
                    try:
                        ...
                        st.write(
                            f" Erstelle Angebot für Firma: {company_name}")

                        project_data = self._prepare_project_data_for_pdf(
                            company_id)
                        analysis_results = {
                            "anlage_kwp": 8.5,
                        ...
                        if pdf_bytes:
                            zip_file.writestr(
                                f"Angebot_{company_name}.pdf", pdf_bytes)
                            st.write(
                                f"   PDF für {company_name} erfolgreich erstellt")
                            successful_offers += 1
                        else:
                            st.warning(
                        ...
                        st.error(
                            f"   Fehler bei PDF-Erstellung für {company_name}: {e}")
                        failed_offers += 1
                        traceback.print_exc()
...
def render_multi_offer_generator(
    ...
    if project_data and not st.session_state.multi_offer_customer_data.get(
            "last_name"):
        # Versuche Kundendaten aus project_data zu extrahieren
        customer_data = {}
        customer_data["first_name"] = project_data.get(
            "customer_first_name", "")
        ...
        customer_data["salutation"] = project_data.get(
...
def ensure_output_directory_exists():
    ...
    try:
        ...
        logging.info(
            f"Ausgabeverzeichnis '{PDF_OUTPUT_DIRECTORY}' ist bereit.")
    except OSError as e:
        logging.error(
            f"Fehler beim Erstellen des Verzeichnisses {PDF_OUTPUT_DIRECTORY}: {e}")
        # Wenn das Verzeichnis nicht erstellt werden kann, ist ein
        # Weiterarbeiten sinnlos.
        raise
...
def generate_filename(offer_data: dict[str, Any]) -> str:
    ...
    customer_name = offer_data.get(
        "customer", {}).get(
        "name", "Unbekannter_Kunde")
...
def process_multiple_offers():
    ...
    """
    logging.info(
        "Starte Prozess zur Erstellung von kompletten Angebotsmappen.")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            for customer in tqdm(
                    customers,
                    desc="Erstelle Angebotsmappen",
                    unit=" Mappe"):
                try:
                    # Schritt 1: Daten berechnen
                    offer_data = calculate_offer_details(
                        customer_id=get_all_active_customers["id"])

                    # Schritt 2: Temporäres PDF für das Hauptangebot erstellen
                    temp_offer_filename = os.path.join(
                        temp_dir, f"temp_offer_{offer_data['offer_id']}.pdf")
                    ...
                    company_docs = get_company_documents(
                        company_id=offer_data["company_id"])
                    attachment_paths.extend(
                        [doc.get('path', '') for doc in company_docs if doc.get('path')])
                    # Produktdatenblätter (Fallback: leere Liste, wenn Funktion
                    # nicht verfügbar)
                    ...
                    try:
                        ...
                        attachment_paths.extend(
                            [sheet['path'] for sheet in datasheets if sheet.get('path')])
                    ...
                    logging.info(
                        f"Angebotsmappe für {
                            customer['name']} erfolgreich erstellt: {final_filename}")
                    successful_maps += 1

                except Exception as e:
                    logging.error(
                        f"Fehler bei Mappe für {
                            customer.get(
                                'name',
                                'Unbekannt')}: {e}",
                        exc_info=True)
        ...
        logging.critical(
            f"Ein kritischer Fehler hat den Prozess gestoppt: {e}",
            exc_info=True)
...
def render_multi_offer_generator(texts, project_data, calc_results):
    ...
    if st.button("Generiere Angebote für alle aktiven Kunden"):
        if not customers:
            st.warning(
                "Keine aktiven Kunden gefunden. Bitte erstellen Sie zuerst Kunden im CRM-Modul.")
            return
        ...
        with st.spinner("Generiere Angebote..."):
            ...
            for customer in customers:
                try:
                    ...
                    # Projekt-Daten aus dem Kunden laden oder Fallback
                    # verwenden
                    ...
                    results = perform_calculations(
                        customer_project_data, texts, errors_list)

                    if results and not errors_list:
                        amortization = results.get(
                            'amortization_time_years', 'N/A')
                        annual_savings = results.get(
                            'annual_financial_benefit_year1', 'N/A')
                        st.write(
                            f"   Angebot für {customer_name} berechnet. Amortisation: {amortization} Jahre, Ersparnis: {annual_savings}€/Jahr")
                        successful_offers += 1
                    else:
                        st.write(
                            f"   Fehler bei Berechnung für {customer_name}: {
                                ', '.join(errors_list) if errors_list else 'Unbekannter Fehler'}")
                        failed_offers += 1
            ...
            if successful_offers > 0:
                st.success(
                    f" Multi-Angebots-Prozess abgeschlossen: {successful_offers} erfolgreich, {failed_offers} fehlgeschlagen")
            else:
                st.error(
                    f" Alle Angebote fehlgeschlagen: {failed_offers} Fehler")
    ...
    with col2:
        if st.button(" Test-Kunden hinzufügen"):
            # Hier könnte eine Funktion aufgerufen werden, um Test-Kunden
            # hinzuzufügen
            st.info(
                "Test-Kunden-Erstellung über das CRM-Modul oder die Datenbank-Administration.")
...
try:
    from database import get_active_company, get_company_by_id, list_all_companies
...
class EnhancedMultiOfferGenerator:
    ...
    def render_company_specific_pdf_config(
            self, company_id: int, company_name: str, texts: dict[str, str]):
        ...
        with tabs[1]:  # Preise
            ...
            if 'base_price' in st.session_state:
                ...
                discount = base * \
                    config['custom_pricing']['discount_percent'] / 100
                surcharge = base * \
                    config['custom_pricing']['surcharge_percent'] / 100
        ...
        with tabs[3]:  # Visualisierungen
            ...
            if config['inclusion_options']['include_charts']:
                ...
                config['inclusion_options']['selected_charts_for_pdf'] = st.multiselect(
                    ...
                    default=config['inclusion_options'].get(
                        'selected_charts_for_pdf',
                        []),
    ...
    def generate_pdf_for_company(
        ...
        try:
            ...
            config = st.session_state.multi_offer_pdf_configs.get(
                company_id, {})
            ...
            if 'custom_pricing' in config and analysis_results:
                ...
                base_price = custom_analysis.get(
                    'total_investment_cost_netto', 0)
                ...
                custom_analysis['total_investment_cost_netto'] = base_price - \
                    discount + surcharge
            ...
            pdf_bytes = generate_offer_pdf(
                ...
                selected_title_image_b64=None,
                selected_offer_title_text=config.get(
                    'custom_texts',
                    {}).get(
                    'offer_title',
                    'Photovoltaik-Angebot'),
                selected_cover_letter_text=config.get(
                    'custom_texts',
                    {}).get(
                    'cover_letter',
                    ''),
                sections_to_include=config.get(
                    'sections_to_include',
                        []),
                inclusion_options=config.get(
                    'inclusion_options',
                    {}),
    ...
    ) -> dict[int, tuple[bool, bytes | None, str]]:
    ...
    def create_zip_with_all_pdfs(
        self,
        pdf_results: dict[int, tuple[bool, bytes | None, str]],
        customer_name: str
    ) -> bytes | None:
        ...
        try:
            ...
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
                for company_id, (success, pdf_bytes,
                                 message) in pdf_results.items():
                    if success and pdf_bytes:
                        company_info = get_company_by_id(company_id)
                        company_name = company_info.get(
                            'name', f'Firma_{company_id}') if company_info else f'Firma_{company_id}'
                        ...
                        safe_company_name = "".join(
                            c for c in company_name if c.isalnum() or c in (
                                ' ', '-', '_')).rstrip()
                        safe_customer_name = "".join(
                            c for c in customer_name if c.isalnum() or c in (
                                ' ', '-', '_')).rstrip()

                        filename = f"Angebot_{safe_customer_name}_{safe_company_name}_{
...
def render_enhanced_multi_offer_page(texts: dict[str, str]):
    ...
    if not PDF_GENERATOR_AVAILABLE:
        st.error(
            "PDF-Generator nicht verfügbar. Bitte stellen Sie sicher, dass alle Module installiert sind.")
        return
    ...
    with st.container():
        ...
        # Kundenname anzeigen
        customer_data = st.session_state.multi_offer_project_data.get(
            'customer_data', {})
        customer_name = f"{
            customer_data.get(
                'first_name',
    ...
    if selected_companies:
        ...
        if st.session_state.multi_offer_settings['use_same_products']:
            ...
            tabs = st.tabs(
                [f"Firma {i + 1}" for i in range(len(selected_companies))])

            for idx, (tab, company_id) in enumerate(
                    zip(tabs, selected_companies, strict=False)):
                with tab:
                    company = next(
                        (c for c in all_companies if c['id'] == company_id), None)
                    if company:
    ...
    if selected_companies:
            ...
            with st.expander(" PDF-Vorschau", expanded=False):
                preview_company = st.selectbox(
                    ...
                    format_func=lambda x: next(
                        (c['name'] for c in all_companies if c['id'] == x),
                        f"Firma {x}"))
        ...
        if st.button(
            ...
            with st.spinner("Generiere PDFs..."):
                ...
                if st.session_state.multi_offer_settings['parallel_generation']:
                    ...
                    for idx, company_id in enumerate(selected_companies):
                        ...
                        status_container.info(
                            f"Generiere PDF für {company_name}...")
                ...
                success_count = sum(
                    1 for success, _, _ in results.values() if success)
                total_count = len(results)

                if success_count == total_count:
                    st.success(
                        f" Alle {total_count} PDFs erfolgreich generiert!")
                elif success_count > 0:
                    st.warning(
                        f" {success_count} von {total_count} PDFs generiert.")
                else:
                ...
                if success_count > 0:
                    zip_bytes = generator.create_zip_with_all_pdfs(
