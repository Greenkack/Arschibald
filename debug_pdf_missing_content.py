...
def test_complete_pdf_flow():
    ...
    print("✅ pv_details_pdf extrahiert:")
    ...
    # Zubehör
    include_additional = pv_details_pdf.get(
        'include_additional_components', True)
    ...
    if len(product_ids) < 3:
        print(
            f"⚠️  WARNUNG: Nur {
                len(product_ids)} Produkte gefunden, erwartet 3+ (Modul, Inverter, Speicher)")
    ...
    include_all_docs = mock_inclusion_options.get(
    ...
    if not include_all_docs:
        ...
        print(
            "   LÖSUNG: Prüfen Sie, ob die Checkbox 'Alle Dokumente anhängen' aktiviert ist")
    else:
