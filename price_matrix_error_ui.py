...
def display_error_message(
    ...
    if error_info.severity == ErrorSeverity.CRITICAL:
        ...
        container.warning(error_info.user_message, icon="⚠️")
    else:
        container.info(error_info.user_message, icon="ℹ️")
...
def display_error_with_fallback(
    ...
    if error_result.get('fallback_used'):
        ...
        container.warning(
            f"⚠️ **Automatischer Fallback aktiviert**\n\n"
...
def display_validation_results(
    ...
    if validation_result.get('valid'):
        ...
        if info:
            with container.expander("ℹ️ Matrix-Informationen"):
        ...
        if warnings:
            with container.expander("⚠️ Warnungen"):
...
def display_admin_notification_banner(
    ...
    # Zeige Warnungen
    if warnings:
        with container.expander(f"⚠️ {len(warnings)} Warnungen"):
            for notif in warnings:
                st.warning(notif.get('message', ''), icon="⚠️")
                st.divider()
