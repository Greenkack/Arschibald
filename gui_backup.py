...
def _apply_active_app_theme(*, inject_css: bool = True) -> None:
    ...
    if not overrides and database_module and callable(
        getattr(database_module, "load_admin_setting", None)):
        try:
            raw_overrides = database_module.load_admin_setting(
                "app_theme_overrides", None)
        except Exception:
            raw_overrides = None
    ...
    getattr(
        ...
        "load_admin_setting",
         None)):
            try:
                stored_key = database_module.load_admin_setting(
                    "app_theme_key", None)
            except Exception:
                stored_key = None
...
def inject_custom_context_menu(nav_lock_enabled: bool) -> Any:
    ...
    menu_items = [
     ...
     "label": "↩Zurück zur letzten Ansicht"},
                {
                    "action": "clear_cache",
                    "label": "Cache leeren",
        ...
        if (!window.customContextMenuEventsBound) {
            ...
            menu.addEventListener('click', (event) => {
                ...
                if (['goto_pdf_output_standard', 'goto_pdf_output_multi', 'open_pdf_preview'].includes(action)) {
                    try {
                        const win = parentDoc.defaultView || window;
                        const targetTab = action === 'goto_pdf_output_multi' ? 'multi' : (action === 'open_pdf_preview' ? 'preview' : 'standard');
                        win.sessionStorage?.setItem('contextMenuNavigateTab', targetTab);
                    } catch (error) {
                        console.warn(
    'Unable to persist tab preference', error);
                    }
                }
    ...
    html_payload = html_template.replace("__MENU_ITEMS__", menu_items_json)
...
def _set_doc_output_target(target: str, message: str) -> None:
    ...
    st.toast(message)
    already_on_doc = st.session_state.get(
        "selected_page_key_sui") == "doc_output"
...
def _reset_calculations() -> None:
    ...
    already_on_analysis = st.session_state.get(
        "selected_page_key_sui") == "analysis"
...
def handle_context_menu_action(action: str) -> None:
    ...
    if action == "clear_cache":
        ...
        st.toast(
    "Cache geleert." if cleared else "Keine Cache-Daten zum Löschen gefunden.")
        request_rerun()
        return

    if action == "toggle_debug":
        new_value = not st.session_state.get(
            "context_menu_debug_enabled", False)
        st.session_state.context_menu_debug_enabled = new_value
        st.toast(
    "Debug-Modus aktiviert." if new_value else "Debug-Modus deaktiviert.")
        return
    ...
    if action == "switch_company":
        ...
        if active_id is None and database_module is not None and callable(
            getattr(database_module, "load_admin_setting", None)):
            try:
                loaded = database_module.load_admin_setting(
    ...
    if action == "goto_pdf_output_standard":
        _set_doc_output_target("standard",
     "Zur PDF-Ausgabe (Standard) gewechselt.")
        return
...
def main():
    ...
    if isinstance(loaded_translations, dict) and loaded_translations:
        ...
        if loaded_translations is not None:
            if 'import_errors' in globals() and isinstance(
                globals()['import_errors'], list):
                globals()['import_errors'].append(
    f"WARNUNG: Übersetzungsdaten sind kein gültiges Dictionary {
        ...
        if isinstance(_texts_initial, dict):
            ...
            if 'import_errors' in globals() and isinstance(
                globals()['import_errors'], list):
                globals()['import_errors'].append(
                    "KRITISCH: ist kein Dictionary! Minimale Fallback-Texte verwendet.")
    ...
    try:
        if isinstance(theme_payload, dict) and theme_payload:
            st.set_page_config(
    page_title=get_text_gui("app_title"),
    layout="wide",
     theme=theme_payload)
        else:
            st.set_page_config(
    ...
    modern_theme_active = False

    # ============================================================================
    # MODERNE UI-TEMPLATES - TEMPORÄR DEAKTIVIERT (schwarzer Bildschirm Fix)
    # Läd CSS-Templates mit maximaler Priorität. Fällt bei Fehler auf Legacy-Theme zurück.
    # ============================================================================
    try:
        # DEAKTIVIERT: CSS-Template-System verursacht schwarzen Bildschirm
        ...
        st.session_state["modern_ui_template_count"] = 0
        import traceback
        print(f"CSS-Templates Fehler: {e_css_template}")
        print(traceback.format_exc())

    # Legacy-Theme nur anwenden, wenn modernes Theme nicht aktiv ist
    if not modern_theme_active:
   ...
   /* ========== ENDE GLOBALE CHECKBOX-EFFEKTE ========== */
    ...
    # nur EINMAL pro Session passieren und nicht bei jedem Rerun überschrieben
    # werden
    ...
    # Prüfe ob kritische States noch existieren (können durch Cleanup gelöscht
    # werden)
    if 'project_data' not in st.session_state:
        st.session_state.project_data = {
    'customer_data': {},
    ...
    # Stelle sicher dass Lists auch wirklich Lists sind (nicht None oder
    # andere Typen)
    ...
    # Context Menu (Rechtsklick) nur im Legacy-Theme laden, um doppelte Menüs
    # zu vermeiden
    menu_event: Any | None = None
    if not st.session_state.get('modern_ui_active', False):
        menu_event = inject_custom_context_menu(
            st.session_state.get('nav_lock_enabled', True))
    else:
        # Reset Legacy-Nonce damit es bei Rückfall wieder funktioniert
        st.session_state.pop('_context_menu_last_nonce', None)
    ...
    (function() {
        ...
        drawer.innerHTML = `
            ...
            <button class="drawer-btn" data-action="action3"> Button 3</button>
    ...
    drawer_action = components.html(drawer_script, height=0, width=0)
    ...
    with st.sidebar:
        ...
        # Hauptmenü Sektion
        st.markdown(
    '<div style="color: rgba(255,255,255,0.4); font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 1.5px; padding: 20px 0 8px 0; margin-top: 10px;">HAUPTMENÜ</div>',
     unsafe_allow_html=True)
    ...
    use_container_width=True,
     type=button_type):
        ...
        # Business Sektion
        st.markdown(
    '<div style="color: rgba(255,255,255,0.4); font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 1.5px; padding: 20px 0 8px 0;">BUSINESS</div>',
     unsafe_allow_html=True)
    ...
    use_container_width=True,
     type=button_type):
        ...
        # Tools Sektion
        st.markdown(
    '<div style="color: rgba(255,255,255,0.4); font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 1.5px; padding: 20px 0 8px 0;">TOOLS</div>',
     unsafe_allow_html=True)
    ...
    getattr(
        ...
        'list_companies',
         None)):
            ...
            if active_company_id_val is not None and active_company_id_val not in name_lookup and database_module and callable(
                getattr(database_module, 'get_company', None)):
                try:
                    active_company = database_module.get_company(
                        active_company_id_val)
            ...
            current_selection = st.session_state.get(
                'context_menu_company_selection')
            ...
            if can_select_company:
                ...
                selected_index = st.selectbox(
                    ...
                    index=current_index if 0 <= current_index < len(
                        labels) else 0,
                ...
                st.info(
                    "Keine Unternehmen gefunden. Bitte im Admin-Bereich anlegen.")
    ...
    f"Aktuell aktiv: {
        name_lookup.get(
            active_company_id_val,
             f'ID {active_company_id_val}')}")

            apply_col, cancel_col = st.columns(2)
            if apply_col.button(
    "Übernehmen",
    key="context_menu_company_apply",
     disabled=not can_select_company):
    ...
    getattr(
        ...
        "save_admin_setting",
         None)):
                            database_module.save_admin_setting(
                                'active_company_id', selected_id)
                        st.session_state.active_company_id = selected_id
                        st.session_state.context_menu_company_switch_open = False
                        st.toast(
    f"Aktives Unternehmen: {
        name_lookup.get(
            selected_id,
             f'ID {selected_id}')}")
                        request_rerun()
                        ...
                        st.error(
    f"Unternehmen konnte nicht gesetzt werden: {err}")
            if cancel_col.button(
    "Abbrechen",
     key="context_menu_company_cancel"):
                st.session_state.context_menu_company_switch_open = False
                request_rerun()
    ...
    "Notiz", key="context_menu_note_input", height=140)
            save_col, cancel_col = st.columns(2)
            if save_col.button(
    "Speichern",
    key="context_menu_note_save",
    disabled=not bool(
        note_text.strip())):
        ...
        if st.session_state.get('context_notes'):
            with st.expander("Gespeicherte Notizen", expanded=False):
                for idx, note in enumerate(
                    reversed(st.session_state.context_notes[-5:])):
                    if idx:
                        st.divider()
                    timestamp = (
    note.get('created_at') or '').replace(
        'T', ' ')[
            :19]
                    if timestamp:
    ...
    if drawer_position is None:
        if hasattr(st, "query_params"):
            ...
            query_params = dict(getattr(st, "query_params"))
        else:
            try:
                ...
                query_params = {}
        candidate = query_params.get(
            "drawer") or query_params.get("drawer_pos")
                ...
                if not PDF_PREVIEW_AVAILABLE:
                    ...
                    if not project_data_preview or not calc_results_preview:
                        ...
                        with demo_image_placeholder:
                            ...
                            # Einfacher Platzhalter für die Vorschau (ASCII-safe)
                            demo_text = (
                                "```")
                            demo_text += "\n"
                            demo_text += "[Ihr Firmenlogo]\n\n"
                            demo_text += "Photovoltaik-Angebot\n\n"
                            demo_text += "Kunde: [Kundenname]\n"
                            demo_text += "Datum: [Heute]\n\n"
                            demo_text += "Anlagenleistung: XX kWp\n"
                            demo_text += "Investition: XX.XXX EUR\n"
                            demo_text += "Ertrag: XX.XXX kWh/Jahr\n\n"
                            demo_text += "[Diagramme und Tabellen]\n"
                            demo_text += "```"
