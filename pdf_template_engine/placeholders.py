...
DEFAULT_SPECIFIC_YIELDS_BY_ORIENTATION_TILT = (
    _DEFAULT_SPECIFIC_YIELDS_BY_ORIENTATION_TILT
)
...
def resolve_feed_in_tariff_eur_per_kwh(
    ...
    default_parts_under_10_eur_per_kwh: float = 0.0786) -> float:
    ...
    try:
        ...
        try:
            ...
            tariffs_data = {}
        tariffs_list = (
            tariffs_data.get(mode_l, []) if isinstance(tariffs_data, dict) else []
        )
        chosen = None
        for trf in tariffs_list or []:
            try:
                if (
                    _fit_to_float(trf.get("kwp_min", 0))
                    <= anlage_kwp
                    <= _fit_to_float(trf.get("kwp_max", 999))
                ):
...
def get_feed_in_tariff_eur_per_kwh(
    anlage_kwp: float, mode: str, load_admin_setting_func
) -> float:  # noqa: D401
...
def fmt_number(
    value: Any, decimal_places: int = 2, suffix: str = "", force_german: bool = True
) -> str:
    ...
    try:
        ...
        if force_german:
            ...
            if decimal_places == 0:
                formatted = (
                    f"{num:,.0f}".replace(",", "#").replace(".", ",").replace("#", ".")
                )
            else:
                formatted = (
                    f"{num:,.{decimal_places}f}".replace(",", "#")
                    .replace(".", ",")
                    .replace("#", ".")
                )
        else:
...
def build_dynamic_data(
    ...
    customer = (
        project_data.get("customer_data", {}) if isinstance(project_data, dict) else {}
    )
    project_details = (
    ...
    result: dict[str, str] = {
        ...
        "customer_phone": as_str(
            customer.get("phone_mobile") or customer.get("phone_landline")
        ),
    ...
    phone = as_str(
        customer.get("phone_mobile") or customer.get("phone_landline") or ""
    ).strip()
    ...
    result: dict[str, str] = {
        ...
        "company_city_zip": as_str(
            (
                f"{company_info.get('zip_code', '')} {company_info.get('city', '')}"
            ).strip()
        ),
    ...
    if anlage_kwp is None:
        ...
        try:
            ...
            anlage_kwp_calc = (
                (mod_qty * mod_wp) / 1000.0
                if mod_qty > 0 and mod_wp > 0
                else project_details.get("anlage_kwp")
            )
    ...
    try:
        ...
        if mods_qty is not None:
            ...
            result["pv_modules_count_with_unit"] = (
                f"{
    ...
    # Batteriegröße (kWh): ausschließlich aus Produkt-/Berechnungsdaten.
    # Die frühere manuelle UI-Eingabe `selected_storage_storage_power_kw`
    # (alte manuelle Speicher-kWh-Eingabe) wird bewusst ignoriert: sie ist kein
    # valider Produktwert und darf weder PDF-Anzeige noch Speicherladung treiben.
    bat_kwh = None

    try:
        from product_db import (
            get_product_by_id as _get_prod_by_id_cap,
            get_product_by_model_name as _get_prod_model_cap,
        )
    except Exception:
        _get_prod_by_id_cap = None  # type: ignore
        _get_prod_model_cap = None  # type: ignore

    storage_product_for_capacity = {}
    storage_id_pref = project_details.get("selected_storage_id")
    if _get_prod_by_id_cap and storage_id_pref not in (None, ""):
        try:
            storage_product_for_capacity = _get_prod_by_id_cap(
                int(storage_id_pref)) or {}
        except Exception:
            storage_product_for_capacity = {}
    storage_model_name_pref = as_str(
        project_details.get("selected_storage_name") or ""
    ).strip()
    if not storage_product_for_capacity and _get_prod_model_cap and storage_model_name_pref:
        try:
            storage_product_for_capacity = _get_prod_model_cap(
                storage_model_name_pref) or {}
        except Exception:
            storage_product_for_capacity = {}

    cand_db = [
        storage_product_for_capacity.get("capacity_kwh"),
        storage_product_for_capacity.get("usable_capacity_kwh"),
        storage_product_for_capacity.get("nominal_capacity_kwh"),
        storage_product_for_capacity.get("max_kwh_capacity"),
        storage_product_for_capacity.get("storage_power_kw"),
    ]
    for cand in cand_db:
    ...
    # Fallbacks auf Analyse/weitere Projektfelder, aber NICHT auf die alte
    # manuelle Gesamtkapazität.
    ...
    try:
        inv_total_kw = project_details.get(
        ...
        if inv_total_kw is not None:
            ...
            result["inverter_section_title"] = f"WECHSELRICHTER – {watt_formatted} W"
        else:
    ...
    candidate_sources: list[Any] = [
        analysis_results,
        project_details,
        project_data if isinstance(project_data, dict) else None,
        (
            project_data.get("analysis_results")
            if isinstance(project_data, dict)
            else None
        ),
    ...
    def extract_numeric_from_sources(
        keys: list[str], sources: list[Any]
    ) -> float | None:
        visited: set[int] = set()
        normalized_targets = {
            re.sub(r"[^a-z0-9]", "", (key or "").lower()) for key in keys
        }
    ...
    def extract_string_from_sources(keys: list[str], sources: list[Any]) -> str | None:
        normalized_targets = {
            re.sub(r"[^a-z0-9]", "", (key or "").lower()) for key in keys if key
        }
        visited: set[int] = set()
        ...
        def _extract(source: Any) -> str | None:
            ...
            if isinstance(source, dict):
                for raw_key, raw_value in source.items():
                    ...
                    if norm_key in normalized_targets and raw_value not in (None, ""):
    ...
    amort_years = extract_numeric_from_sources(amortization_keys, candidate_sources)
    amortization_years_value: float | None = None
    if amort_years is not None and amort_years > 0 and math.isfinite(amort_years):
        amortization_years_value = amort_years

    # Seite 2: Energieflüsse (Jahr 1)
    monthly_direct_sc = (
        analysis_results.get("monthly_direct_self_consumption_kwh", []) or []
    )
    monthly_storage_charge = (
        analysis_results.get("monthly_storage_charge_kwh", []) or []
    )
    monthly_storage_discharge_sc = (
        analysis_results.get("monthly_storage_discharge_for_sc_kwh", []) or []
    )
    feed_in_kwh = analysis_results.get("netzeinspeisung_kwh")
    grid_bezug_kwh = analysis_results.get("grid_bezug_kwh") or analysis_results.get(
        "grid_purchase_kwh"
    )
                ...
                if prod > 0:
                    val = max(
                        ...
                        min(
                            100.0,
                            100.0
                            * (float(direct_sc_sum) + float(discharge_sc_sum))
                            / prod))
                    result["self_consumption_percent"] = fmt_number(val, 0, "%")
            except Exception:
                pass
    ...
    if annual_prod_float and annual_prod_float > 0:
        try:
            # Rohanteile 0..1
            direct_raw = (
                max(0.0, min(direct_sc_sum, annual_prod_float)) / annual_prod_float
            )
            # Batterieanteil an Produktion basiert auf Ladung aus Produktion,
            # begrenzt durch Rest nach Direktverbrauch
            battery_raw = (
                max(0.0, min(charge_sum, max(0.0, annual_prod_float - direct_sc_sum)))
                / annual_prod_float
            )
    ...
    if cons_total and cons_total > 0:
        try:
            battery_cover_pct = (
                100.0 * max(0.0, min(discharge_sc_sum, cons_total)) / cons_total
            )
            grid_rate_pct = (
                100.0 * max(0.0, min(grid_bezug_val or 0.0, cons_total)) / cons_total
            )
            direct_cover_pct = (
                100.0 * max(0.0, min(direct_sc_sum, cons_total)) / cons_total
            )
    ...
    try:
        ...
        battery_charge_pct = _parse_pct_str(
            result.get("battery_use_quote_prod_percent")
        )  # z.B. "41%"
        ...
        direct_consumption_pct = _parse_pct_str(
            result.get("feed_in_quote_prod_percent_number")
        )  # Zahl ohne % -> direkt
        feed_pct_swapped = _parse_pct_str(
            result.get("direct_consumption_quote_prod_percent")
        ...
        battery_cover_pct = _parse_pct_str(
            result.get("battery_cover_consumption_percent")
        )  # z.B. "Speichernutzung quote"
        direct_cover_pct = _parse_pct_str(
            result.get("direct_cover_consumption_percent_number")
        )  # Zahl ohne %
        grid_pct = _parse_pct_str(
            result.get("grid_consumption_rate_percent")
        )  # Stromnetz Quote
        ...
        try:
            ...
            result["self_supply_rate_percent"] = (
                f"{
                int(
                    round(lower_autarky))}%"
            )
    ...
    def _get_monthly_cost_eur() -> float:
        ...
        try:
            hh = (
                parse_float(project_details.get("stromkosten_haushalt_euro_monat"))
                or 0.0
            )
            hz = (
                parse_float(project_details.get("stromkosten_heizung_euro_monat"))
                or 0.0
            )
            if (hh + hz) > 0:
                return float(hh + hz)
        ...
        # verfügbar)
        try:
            cons_kwh = parse_float(
                analysis_results.get("jahresstromverbrauch_fuer_hochrechnung_kwh")
            )
            price_eur_kwh = parse_float(
    ...
    anlage_kwp_numeric = (
        parse_float(analysis_results.get("anlage_kwp"))
        or parse_float(project_data.get("anlage_kwp"))
        or 0.0
    )
    ...
    if _load_admin_setting_feed:
        eeg_eur_per_kwh = resolve_feed_in_tariff_eur_per_kwh(
            ...
            analysis_results_snapshot=(
                analysis_results.get("einspeiseverguetung_eur_per_kwh")))
    else:
        eeg_eur_per_kwh = (
            parse_float(analysis_results.get("einspeiseverguetung_eur_per_kwh"))
            or 0.0786
        )
    ...
    monthly_storage_discharge = (
        analysis_results.get("monthly_storage_discharge_for_sc_kwh") or []
    )
    monthly_feed_in_list = analysis_results.get("monthly_feed_in_kwh") or []

    direct_kwh = (
        sum(float(x or 0) for x in monthly_direct_sc) if monthly_direct_sc else 0.0
    )
    ...
    speicher_ladung_kwh = (
        sum(float(x or 0) for x in monthly_storage_charge)
        if monthly_storage_charge
        else 0.0
    )
    speicher_nutzung_kwh = (
        sum(float(x or 0) for x in monthly_storage_discharge)
        if monthly_storage_discharge
        else 0.0
    )
    ...
    val_speicher_nutzung_money = (speicher_nutzung_kwh or 0.0) * float(
        price_eur_per_kwh
    )
    val_speicher_ueberschuss_money = (speicher_ueberschuss_kwh or 0.0) * float(
        eeg_eur_per_kwh
    )
    ...
    einkommensteuer_satz = (
        parse_float(
            project_data.get("income_tax_rate")
            or project_details.get(
                # Fallback: 42% (Standard in Deutschland)
                "income_tax_rate"
            )
            or analysis_results.get("income_tax_rate")
        )
        or 0.42
    )
    val_steuerliche_vorteile = (
        val_feedin_money * (einkommensteuer_satz / 100.0)
        if einkommensteuer_satz > 1
        else val_feedin_money * einkommensteuer_satz
    )
    ...
    result["annual_feed_in_revenue_eur"] = fmt_number(
        val_feedin_money, 2, "€"
    )  # Alias für Einspeisung
    result["tax_benefits_eur"] = fmt_number(
        val_steuerliche_vorteile, 2, "€"
    ...
    if module_details or module_name:
        mod_brand = as_str(
            module_details.get("brand") or module_details.get("manufacturer") or ""
        )
        if mod_brand:
        ...
        perf_pct = module_details.get(
            "performance_warranty_percent"
        ) or module_details.get("efficiency_percent_end")
        if perf_years is not None and perf_pct is not None:
            ...
            try:
                prod_txt = (
                    fmt_number(float(prod_warranty_years), 0, "Jahre Produktgarantie")
                    if prod_warranty_years is not None
                    else ""
                )
            except Exception:
                prod_txt = (
                    f"{
                    as_str(prod_warranty_years)} Jahre Produktgarantie"
                    if prod_warranty_years is not None
                    else ""
                )
            if prod_txt:
                result["module_guarantee_combined"] = (
                    f"{prod_txt} | {years_str} Leistungsgarantie"
                )
        ...
        for out_key, src_key in [
            ...
            if val not in (None, ""):
                ...
                print(f"DEBUG: Set {out_key} = {val} from module_details.{src_key}")
            else:
                print(
        ...
        try:
            if not all(
                ...
                if not pid:
                    ...
                    if not pid:
                        mm_pd = as_str(
                            project_details.get("module_model") or ""
                        ).strip()
                        if mm_pd:
                            pid = _get_pid(mm_pd)
                ...
                if pid:
                    ...
                    try:
                        alias_map = (
                            _load_admin_setting("module_pdf_alias_map", {}) or {}
                        )
                    except Exception:
                        alias_map = None
                    rev: dict[str, list] = {}
    ...
    try:
        ...
        if (
            "solarfabrik"
            in str(project_details.get("selected_module_name", "")).lower()
        ):
            ...
            print(f"  module_details.version = {module_details.get('version')}")
    except Exception:
        pass
    ...
    if inverter_details or inverter_name:
        inv_brand = as_str(
            inverter_details.get("brand") or inverter_details.get("manufacturer") or ""
        )
        if inv_brand:
        ...
        try:
            ...
            cand = [
                (
                    "w",
                    inverter_details.get("power_watt")
                    or inverter_details.get("rated_power_w")
                    or inverter_details.get("power_w")),
                ("kw", inverter_details.get("power_kw")),
                (
                    "w",
                    project_details.get("selected_inverter_power_w")
            ...
            for unit, v in cand:
                ...
                if pf and pf > 0:
                    ...
                    watt_val = (pf if pf > 100 else pf * 1000.0) if unit == "kw" else pf
                    break
            # Fallback-Heuristik, falls Quelle unklar: Werte > 1000 als W
            # interpretieren, sonst kW
            if watt_val is None:
                v = inverter_details.get("power") or project_details.get(
                    "inverter_power"
                )
                pf = parse_float(v)
                if pf and pf > 0:
            ...
            try:
                if (
                    watt_val is not None and watt_val > 1000
                ):  # >100 kW ist unrealistisch für einzelne WR
            ...
            if watt_val is not None:
                ...
                try:
                    ...
                    watt_int = watt_val
                try:
                    result["inverter_power_watt"] = (
                        f"{watt_int:,.0f}".replace(",", ".") + " W"
                    )
                except Exception:
        ...
        if "hybrid" in name_l:
            ...
            try:
                has_storage = bool(
                    project_details.get("selected_storage_name")
                    or project_details.get("battery_capacity_kwh")
                    or analysis_results.get("battery_capacity_kwh")
                )
            except Exception:
                has_storage = False
            result["inverter_type"] = (
                "Hybrid-Wechselrichter" if has_storage else "String-Wechselrichter"
            )
        ...
        if inv_id:
            ...
            if aval:
                ...
                al = str(aval).lower()
                if any(
                    t in al for t in ["dreiphas", "drei", "einphas", "1phas", "3phas"]
                ):
    ...
    if storage_id not in (None, ""):
        # Bevorzugt per ID laden: Modellnamen können sich ändern oder mehrere
        # Varianten derselben Produktfamilie enthalten (z.B. Huawei LUNA2000
        # 7/14/21 kWh). Die ID ist der robusteste Bezug auf die UI-Auswahl.
        try:
            from product_db import get_product_by_id as _get_storage_by_id

            sd = _get_storage_by_id(int(storage_id))
            if isinstance(sd, dict):
                storage_details = sd
                storage_name = as_str(sd.get("model_name") or storage_name).strip()
        except Exception:
            pass
    if not storage_details and storage_name:
        storage_details = fetch_details(storage_name) or {}
    if storage_details or storage_name or project_details.get("include_storage"):
        sto_brand = as_str(
            storage_details.get("brand") or storage_details.get("manufacturer") or ""
        )
        if sto_brand:
        ...
        # Kapazität (kWh): Produktdaten zuerst; die alte manuelle UI-Eingabe
        # `selected_storage_storage_power_kw` bewusst ignorieren.
        cand_sto = [
            storage_details.get("capacity_kwh"),
            storage_details.get("usable_capacity_kwh"),
            storage_details.get("nominal_capacity_kwh"),
            storage_details.get("max_kwh_capacity"),
        ...
        # Leistung (kW)
        sto_kw = (
            storage_details.get("power_kw")
            or storage_details.get("storage_power_kw")
            or project_details.get("selected_storage_power_kw")
        )
        ...
        # Entladetiefe (DoD %)
        dod_pct = storage_details.get("dod_percent") or analysis_results.get(
            "storage_dod_percent"
        )
        ...
        cycles = storage_details.get("max_cycles") or analysis_results.get(
            "storage_cycles"
        )
        ...
        if not result.get("storage_cell_technology"):
            ...
            for k in (
                ...
                if val:
                    ...
                    chem = (
                        chem.replace("LiFePO5", "LiFePO4")
                        .replace("Lifepo5", "LiFePO4")
                        .replace("LiFePo5", "LiFePO4")
                    )
        ...
        if sto_id:
            ...
            if not result.get("storage_cycles"):
                ...
                try:
                    if aval:
                        result["storage_cycles"] = (
                            f"{int(float(parse_float(aval) or 0))} cycles"
                            if parse_float(aval)
                            else str(aval)
                        )
                except Exception:
        ...
        try:
            if (
                project_details.get("include_storage") or storage_name
            ) and not result.get("storage_extension_module_size_kwh"):
                result["storage_extension_module_size_kwh"] = "siehe Produktdatenblatt"
            if (
                project_details.get("include_storage") or storage_name
            ) and not result.get("storage_max_size_kwh"):
                result["storage_max_size_kwh"] = "siehe Produktdatenblatt"
        except Exception:
            pass

        # Huawei LUNA2000-Familie: Defaults für Felder, die NICHT bereits
        # ein gültiger Wert aus DB/UI haben. Hardcodierte 7-kWh-Werte werden
        # NUR noch verwendet, wenn das tatsächlich gewählte Modell ein
        # 7-kWh-Modell ist – sonst übernimmt der DB/UI-Wert.
        # BUGFIX 2026-01-03: Vorher wurden alle Huawei-Speicher (auch 21 kWh)
        # auf "LUNA2000-7-S1-7kWh" mit 7/7/21 kWh überschrieben.
        name_l = (storage_name or "").lower()
        brand_l = (sto_brand or "").lower()
        if ("huawei" in brand_l) or ("luna2000" in name_l):
            if not result.get("storage_manufacturer"):
                result["storage_manufacturer"] = "Huawei"
            # storage_model: nur setzen, wenn aus DB nicht bereits korrekt
            # geliefert. Sonst tatsächlichen Modellnamen verwenden.
            if not result.get("storage_model"):
                result["storage_model"] = storage_name or "LUNA2000"
            if not result.get("storage_cell_technology"):
                result["storage_cell_technology"] = (
                    "Lithium-Eisenphosphat (LiFePO4)"
                )
            # storage_size_battery_kwh_star: aus echter Kapazität ableiten
            if not result.get("storage_size_battery_kwh_star") and bat_kwh:
                result["storage_size_battery_kwh_star"] = fmt_number(
                    float(bat_kwh), 2, "kWh"
                )
            # storage_backup_text/outdoor: produkt-allgemeine Defaults
            if not result.get("storage_backup_text"):
                result["storage_backup_text"] = "ja, dreiphasig"
            if not result.get("storage_outdoor_capability"):
                result["storage_outdoor_capability"] = "Outdoorfähig"
    ...
    try:
        ...
        for comp_key in ("module", "inverter", "storage"):
            print(
                f"  {comp_key}_manufacturer_raw=",
                result.get(f"{comp_key}_manufacturer_raw"),
                "display=",
                result.get(f"{comp_key}_manufacturer"))
    except Exception:
        pass
    ...
    try:
        ...
        if project_details:
            ...
            if project_details.get("formatted_final_modified_vat_amount"):
                ...
                result["vat_amount_eur"] = project_details[
    ...
    try:
        ...
        provider = (
            project_data.get("energy_supplier")
            or project_data.get("stromanbieter")
            or analysis_results.get("energy_supplier")
        )
        if provider:
            result["basis_energy_supplier_name"] = str(provider)
        # Wartung Prozent (z.B. 1 % Invest p.a.)
        maint_pct = analysis_results.get(
            "maintenance_costs_percent"
        ) or analysis_results.get("maintenance_percent_invest_pa")
        if isinstance(maint_pct, int | float):
            result["basis_maintenance_percent_invest"] = (
                f"{
        ...
        # PV Lebensdauer
        lifetime = analysis_results.get("simulation_period_years") or project_data.get(
        ...
        inc_pct = analysis_results.get(
            "electricity_price_increase_annual_percent"
        ) or project_data.get("electricity_price_increase_annual_percent")
        if isinstance(inc_pct, int | float):
            result["basis_price_increase_percent_text"] = (
                f"{
                inc_pct:.2f} % jährlich"
            )
        # Eigenkapitalkosten
        cost_cap = (
            analysis_results.get("cost_of_capital_percent")
            or project_data.get("cost_of_capital_percent")
            or analysis_results.get("alternative_investment_interest_rate_percent")
        )
    ...
    try:
        ...
        if direct_kwh is None:
            try:
                ...
                batt_dis_tmp = (
                    _pf(analysis_results.get("battery_discharge_for_sc_kwh")) or 0.0
                )
        ...
        if direct_kwh is not None and curr_price > 0:
            result["annual_electricity_cost_savings_self_consumption_year1"] = (
                fmt_number(direct_kwh * curr_price, 2, "€")
            )
        feed_kwh = _pf(analysis_results.get("netzeinspeisung_kwh"))
        feed_tariff = (
            _pf(analysis_results.get("einspeiseverguetung_eur_per_kwh"))
            or _pf(analysis_results.get("feed_in_tariff_eur_per_kwh"))
            or _pf(analysis_results.get("feed_in_tariff_year1_eur_per_kwh"))
        )
    ...
    if not result.get("total_annual_savings_eur"):
        ...
        if not result.get("self_consumption_without_battery_eur"):
            direct_kwh_fs = (
                ...
                or sum(
                    _to_float(x)
                    for x in (
                        analysis_results.get("monthly_direct_self_consumption_kwh")
                        or []
                    )
                )
                or 0.0
        ...
        if not result.get("tax_benefits_eur"):
            ...
            feedin_value = _to_float(
                (
                    result.get("annual_feed_in_revenue_eur")
                    or result.get("direct_grid_feed_in_eur")
                    or "0"
                )
                .replace("€", "")
                .replace(".", "")
            ...
            einkommensteuer_satz = (
                _to_float(
                    project_data.get("income_tax_rate")
                    or project_details.get("income_tax_rate")
                    or analysis_results.get("income_tax_rate")
                )
                or 0.42
            )
            tax_benefits = feedin_value * (
                einkommensteuer_satz / 100.0
                if einkommensteuer_satz > 1
                else einkommensteuer_satz
            )
    ...
    try:
        anlage_kwp_val = (
            parse_float(
                analysis_results.get("anlage_kwp") or project_data.get("anlage_kwp")
            )
            or 0.0
        )
    ...
    try:
        ...
        def extract_brands_from_project_data(
            ...
            module_brand = pick(
                "module_manufacturer_raw", "module_manufacturer", "module_manufacturer"
            )
            if module_brand:
    ...
    try:
        ...
        if hp_offer and isinstance(hp_offer, dict):
            ...
            if beg_data:
                ...
                result["hp_subsidy_amount"] = (
                    fmt_number(subsidy_amount, 0, "€") if subsidy_amount else "0 €"
                )
            ...
            if financing:
                ...
                result["hp_financing_monthly"] = (
                    fmt_number(monthly_rate, 2, "€") if monthly_rate else "0,00 €"
                )
        ...
        for k, v in hp_ph.items():
            ...
            elif k.endswith(("_AMOUNT", "_NET", "_PRICE_NET")) or k in {
                "HP_MONTHLY_RATE",
                "HP_TOTAL_INTEREST",
            }:
        ...
        try:
            if analysis_results and isinstance(analysis_results, dict):
                ...
                if combined is not None:
                    ...
                    result["PV_TOTAL_NET"] = (
                        fmt_number(pv_total, 0, "€") if pv_total is not None else ""
                    )
                    result["HP_TOTAL_NET"] = (
                        fmt_number(hp_after, 0, "€") if hp_after is not None else ""
                    )
        except Exception:
            pass
    ...
    try:
        ...
        def _get_numeric_value(key: str) -> float:
            for container in (analysis_results, project_details, project_data, result):
        ...
        if prod_kwh <= 0:
            ...
            ) or result.get("monthly_pv_production_kwh")
        ...
        if export_kwh <= 0:
            monthly_feed = analysis_results.get("monthly_feed_in_kwh") or result.get(
                "monthly_feed_in_kwh"
            )
            if isinstance(monthly_feed, (list, tuple)) and monthly_feed:
                export_kwh = sum(max(_as_float(x), 0.0) for x in monthly_feed)
                if export_kwh > 0:
                    export_found = True
        ...
        export_kwh = max(min(export_kwh, prod_kwh) if prod_kwh > 0 else export_kwh, 0.0)
        ev_kwh = max(
            min(ev_kwh, prod_kwh - export_kwh) if prod_kwh > 0 else ev_kwh, 0.0
        )
        ...
        if annual_co2_existing > 0:
            if annual_co2_calculated <= 0:
                ...
                ratio = (
                    annual_co2_existing / annual_co2_calculated
                    if annual_co2_calculated
                    else 1.0
                )
                if 0.8 <= ratio <= 1.2:
                    annual_co2_kg = (annual_co2_existing + annual_co2_calculated) / 2.0
                else:
                    annual_co2_kg = annual_co2_calculated
        ...
        if (
            baseline_kg <= 0
            and (co2_ev + co2_export) > 0
            and (ev_found or export_found)
        ):
        ...
        if not isinstance(baseline_tons, (int, float)) or baseline_tons <= 0:
            baseline_tons = (
                baseline_kg / 1000.0 if baseline_kg > 0 else DEFAULT_BASELINE_CO2_TONS
            )
        ...
        result["sustainability_annual_co2_savings_kg_ellipsis"] = (
            f"{
            fmt_number(
                annual_co2_kg, 2)} kg"
        )
        result["sustainability_annual_co2_savings_kg_value"] = (
            f"{
            annual_co2_kg:.2f}"
        )
        result["sustainability_car_km_equivalent_long"] = (
            f"{
            fmt_number(
                car_km_equiv,
                2)} Kilometer"
        )
        result["sustainability_car_km_equivalent_short"] = (
            f"{
            fmt_number(
                car_km_equiv, 2)} km"
        )
        result["sustainability_co2_reduction_percent"] = (
            f"{
            fmt_number(
                co2_reduction_pct, 2)} %"
        )
        # Bäume: gerundete ganze Zahl
        result["sustainability_tree_equivalent_with_label"] = (
            f"{
            fmt_number(
                round(trees_equiv),
                0)} Bäume"
        ...
        result["sustainability_car_km_equivalent_value"] = (
            f"{
            car_km_equiv:.2f}"
        )
        result["sustainability_co2_reduction_percent_value"] = (
            f"{
            co2_reduction_pct:.2f}"
        )
        result["sustainability_emission_factor_grid"] = f"{grid_factor:.4f}"
        result["sustainability_emission_factor_export"] = (
            f"{
            export_factor:.4f}"
        )
        ...
        result["sustainability_annual_co2_savings_tons"] = (
            f"{
            annual_co2_tons:.3f}"
        )
    ...
    try:
        ...
        if "sustainability_annual_co2_savings_kg_ellipsis" in result:
            ...
            if m:
                result["sustainability_annual_co2_savings_kg_clean"] = (
                    f"{
                    m.group(1)} kg"
                )
            else:
        ...
        mod_manufacturer = (
            analysis_results.get("module_manufacturer")
            or result.get("module_manufacturer")
            or ""
        ).strip()
        mod_model = (
            analysis_results.get("module_model") or result.get("module_model") or ""
        ).strip()
        ...
        result["pv_modules_count_key"] = result.get("pv_modules_count_with_unit", "")

        # Wechselrichter: Hersteller + Modell (ohne Leistung)
        # Priorität: analysis_results > result (aus DB/project_details)
        inv_manufacturer = (
            analysis_results.get("inverter_manufacturer")
            or result.get("inverter_manufacturer")
            or ""
        ).strip()
        inv_model = (
            analysis_results.get("inverter_model") or result.get("inverter_model") or ""
        ).strip()
        ...
        # Priorität: result/project_details > analysis_results, damit Seite 7
        # die tatsächlich im UI gewählte Speicher-Variante zeigt und nicht eine
        # ältere/abgeleitete Analyse-Bezeichnung.
        stor_manufacturer = (
            result.get("storage_manufacturer")
            or project_details.get("selected_storage_manufacturer")
            or analysis_results.get("storage_manufacturer")
            or ""
        ).strip()
        stor_model = (
            result.get("storage_model")
            or project_details.get("selected_storage_name")
            or analysis_results.get("storage_model")
            or ""
        ).strip()

        stor_line_parts = []
        model_already_contains_brand = bool(
            stor_manufacturer
            and stor_model
            and _clean(stor_model).lower().startswith(_clean(stor_manufacturer).lower())
        )
        if stor_manufacturer and not model_already_contains_brand:
        ...
        storage_capacity = (
            analysis_results.get("storage_capacity_kwh")
            or result.get("storage_capacity_kwh")
            or ""
        ).strip()
    ...
    pdf_services_cfg = (
        project_data.get("pdf_services") if isinstance(project_data, dict) else None
    )
    ...
    # False
    design_cfg = (
        project_data.get("pdf_design_config")
    ...
    projektierung_enabled = pdf_services_cfg.get("service_project_management", True)
    optimierung_enabled = pdf_services_cfg.get("service_optimization", True)
    ...
    try:
        ...
        pv_count_raw = (
            analysis_results.get("pv_modules_count_with_unit")
            if analysis_results
            else None
        )
        if not pv_count_raw and project_data and "project_details" in project_data:
            pv_count_raw = project_data["project_details"].get("module_quantity")
        ...
        inverter_power = (
            analysis_results.get("inverter_total_power_kw")
            if analysis_results
            else None
        ) or result.get("inverter_total_power_kw")

        formatted_pricing = (
            project_details.get("formatted_final_pricing")
            if isinstance(project_details.get("formatted_final_pricing"), dict)
            else {}
        )
        ...
        project_simple_formatted = (
            project_simple_pricing.get("formatted", {})
            if isinstance(project_simple_pricing.get("formatted"), dict)
            else {}
        )
        ...
        if not inverter_power and project_data and "project_details" in project_data:
            ...
            if isinstance(project_details, dict):
                ...
                discount_val = _to_float(project_details.get("total_discounts"))
                if discount_val is not None:
                    complete_updates["total_discount"] = abs(discount_val)
                    complete_formatted_updates["total_discount"] = (
                        formatted_pricing_values.get("total_discounts")
                        or _format_currency_value(abs(discount_val))
                    )

                surcharge_val = _to_float(project_details.get("total_surcharges"))
                if surcharge_val is not None:
                    complete_updates["total_surcharge"] = surcharge_val
                    complete_formatted_updates["total_surcharge"] = (
                        formatted_pricing_values.get("total_surcharges")
                        or _format_currency_value(surcharge_val)
                    )
                ...
                if zwischensumme_val is not None:
                    ...
                    complete_formatted_updates["zwischensumme"] = (
                        formatted_pricing_values.get("zwischensumme_brutto")
                        or _format_currency_value(zwischensumme_val)
                    )

                final_net_val = _to_float(project_details.get("final_offer_price_net"))
                if final_net_val is not None:
                    complete_updates["finale_summe_netto"] = final_net_val
                    complete_formatted_updates["final_end_preis"] = (
                        formatted_pricing_values.get("final_offer_price_net")
                        or _format_currency_value(final_net_val)
                    )

                if complete_updates:
                    existing_complete_formatted = (
                        project_complete_pricing.get("formatted", {})
                        if isinstance(project_complete_pricing.get("formatted"), dict)
                        else {}
                    )
                    merged_complete_formatted = {
                        **existing_complete_formatted,
                        **{k: v for k, v in complete_formatted_updates.items() if v},
                    }
                    project_complete_pricing = {
                        **project_complete_pricing,
                        **complete_updates,
                    ...
                    if merged_complete_formatted:
                        project_complete_pricing["formatted"] = (
                            merged_complete_formatted
                        )
                ...
                if zubehor_val is not None:
                    ...
                    final_formatted_updates["zubehor"] = (
                        formatted_pricing_values.get("zubehor_total")
                        or formatted_pricing_values.get("zubehor")
                        or _format_currency_value(zubehor_val)
                    )
                ...
                if extras_val is not None:
                    ...
                    final_formatted_updates["extra_services"] = (
                        formatted_pricing_values.get("extras_total")
                        or formatted_pricing_values.get("extra_services")
                        or _format_currency_value(extras_val)
                    )

                zw_final_val = _to_float(project_details.get("zwischensumme_brutto"))
                if zw_final_val is not None:
                    final_updates["zwischensumme_final"] = zw_final_val
                    final_formatted_updates["zwischensumme_final"] = (
                        formatted_pricing_values.get("zwischensumme_brutto")
                        or _format_currency_value(zw_final_val)
                    )

                mwst_val = _to_float(project_details.get("minus_mehrwertsteuer"))
                if mwst_val is not None:
                    ...
                    ) or _format_currency_value(mwst_val)
                    final_formatted_updates["mwst_zwischensumme"] = (
                        formatted_pricing_values.get("minus_mehrwertsteuer")
                        or mwst_formatted
                    )
                    final_formatted_updates["ersparte_mwst"] = (
                        formatted_pricing_values.get("minus_mehrwertsteuer")
                        or mwst_formatted
                    )
                    final_formatted_updates["vat_savings"] = (
                        formatted_pricing_values.get("minus_mehrwertsteuer")
                        or mwst_formatted
                    )
                ...
                if final_price_val is not None:
                    ...
                    final_formatted_updates["final_end_preis"] = (
                        formatted_pricing_values.get("final_offer_price_net")
                        or _format_currency_value(final_price_val)
                    )

                kern_val = _to_float(project_details.get("component_base_price_net"))
                if kern_val is not None:
                    final_updates["kern_komponenten_total"] = kern_val
                    final_formatted_updates["kern_komponenten_total"] = (
                        formatted_pricing_values.get("component_base_price_net")
                        or _format_currency_value(kern_val)
                    )

                if final_updates or final_formatted_updates:
                    existing_final_formatted = (
                        project_final_pricing.get("formatted", {})
                        if isinstance(project_final_pricing.get("formatted"), dict)
                        else {}
                    )
                    merged_final_formatted = {
                        **existing_final_formatted,
                        **{k: v for k, v in final_formatted_updates.items() if v},
                    }
                    project_final_pricing = {**project_final_pricing, **final_updates}
            ...
            if formatted_entries:
                final_pricing_values = {**final_pricing_values, **formatted_entries}
            ...
            inverter_power = project_details.get(
                "selected_inverter_power_kw"
            ) or project_details.get("inverter_power_kw")
        ...
        storage_capacity = (
            analysis_results.get("storage_capacity_kwh") if analysis_results else None
        ) or result.get("storage_capacity_kwh")
        ...
        if not storage_capacity and project_data and "project_details" in project_data:
            ...
            storage_capacity = (
                project_details.get("selected_storage_capacity_kwh")
                or project_details.get("storage_capacity_kwh")
                or project_details.get("battery_capacity_kwh")
            )
        ...
        try:
            ...
            if analysis_results:
                ...
                if annual_yield:
                    with suppress(ValueError, TypeError):
                        annual_yield_val = float(str(annual_yield).replace(",", "."))
                        daily_production = annual_yield_val / 365

            # Fallback: Versuche aus result
            if daily_production == 0 and result.get("annual_pv_production_kwh"):
                with suppress(ValueError, TypeError):
                    annual_prod_val = float(
                        str(result["annual_pv_production_kwh"])
                        .replace(",", ".")
                        .replace(" kWh", "")
                        .replace("kWh", "")
                    )
            ...
            if storage_kwh > 0 and daily_consumption > 0:
                ...
                result["storage_consumption_ratio_percent"] = (
                    f"{
            ...
            if storage_kwh > 0 and daily_production > 0:
                ...
                result["storage_production_ratio_percent"] = (
                    f"{
                    production_ratio:.0f}"
                )
                print(
    ...
    try:
        ...
        result["optional_services_list"] = (
            "Keine optionalen Dienstleistungen ausgewählt"
        )
    ...
    try:
        ...
        if (
            ...
            and "project_details" in project_data
            and isinstance(project_details, dict)
        ):
            ...
            try:
                ...
                if isinstance(simple_candidate, dict):
                    ...
                    formatted_simple = (
                        simple_data.get("formatted", {})
                        if isinstance(simple_data.get("formatted"), dict)
                        else project_simple_formatted
                    )
                    result["simple_endergebnis_brutto_formatted"] = (
                        formatted_simple.get(
                            "endergebnis",
                            formatted_simple.get("endergebnis_brutto", "0,00 €"))
                    )
                    result["simple_mwst_formatted"] = formatted_simple.get(
                        "mwst",
                        formatted_simple.get(
                            "mwst_betrag",
                            simple_data.get("formatted", {}).get("mwst", "0,00 €")))
                    print(
                ...
                if isinstance(complete_candidate, dict):
                    ...
                    result["calc_total_discounts_formatted"] = (
                        complete_data.get("formatted", {}).get(
                            "total_discount", "0,00 €"
                        )
                        if "formatted" in complete_data
                        else "0,00 €"
                    )
                ...
                if isinstance(final_candidate, dict):
                    ...
                    result["zubehor_total"] = final_data.get("zubehor_betrag", 0.0)
                    ...
                    result["ersparte_mehrwertsteuer_formatted"] = final_data.get(
                        "formatted", {}
                    ).get("ersparte_mwst", "0,00 €")
                    result["vat_savings"] = final_data.get("vat_savings", 0.0)
                    result["vat_savings_formatted"] = final_data.get(
                        "formatted", {}
                    ).get("ersparte_mwst", "0,00 €")
                    vat_formatted = final_data.get("formatted", {}).get("ersparte_mwst")
                    if isinstance(vat_formatted, str) and vat_formatted.strip():
                        result["vat_amount_eur"] = vat_formatted
                    else:
                        vat_value = _to_float(
                            final_data.get("ersparte_mehrwertsteuer")
                            or final_data.get("vat_savings")
                        )
                        if vat_value is not None:
                            result["vat_amount_eur"] = _format_currency_value(vat_value)
                    result["kern_komponenten_total"] = final_data.get(
                        "kern_komponenten_total", 0.0
                    )
                    result["kern_komponenten_total_formatted"] = (
                        final_data.get("formatted", {}).get(
                            "kern_komponenten_total", "0,00 €"
                        )
                        if "formatted" in final_data
                        else "0,00 €"
                    )
                    print(
                ...
                # Werte erhalten
                def _alias_value(
                    *values: Any, default: str = "0,00 €", allow_zero: bool = False
                ) -> str:
                    zero_strings = {"0", "0 €", "0,00", "0.00", "0,00 €", "0.00 €"}
                    for val in values:
                        if val is None:
                            continue
                        if isinstance(val, int | float):
                ...
                result["preis_mit_mwst_formatted"] = _alias_value(
                    formatted_pricing.get("preis_mit_mwst"),          # Korrekt: finaler Gesamtpreis inkl. Extras/Rabatte
                    ...
                    result.get("simple_endergebnis_brutto_formatted"),  # Fallback: Basispreis (ohne Extras)
                ...
                result["zwischensumme_preis_formatted"] = _alias_value(
                    formatted_pricing.get("zwischensumme_brutto"),          # Korrekt: finaler Bruttopreis
                ...
                result["minus_mwst_formatted"] = _alias_value(
                    formatted_pricing.get("minus_mehrwertsteuer"),          # Korrekt: MwSt-Betrag aus finaler Kalkulation
                ...
                result["final_end_preis_formatted"] = _alias_value(
                    formatted_pricing.get("final_offer_price_net"),         # Korrekt: finaler Nettopreis
                    result.get("final_end_preis_formatted"),                # Bereits korrekt aus final_data gesetzt
                ...
                simple_vat_value = parse_float(result.get("simple_mwst_formatted"))
                if simple_vat_value is None:
                    simple_vat_value = parse_float(
                        final_pricing_values.get("SIMPLE_MWST_BETRAG")
                    )

                # BUGFIX 2026-01-03: Override-Block NUR als Fallback, wenn die
                # Werte aus final_pricing_data NICHT geliefert wurden.
                # Sonst werden die korrekt vorberechneten Werte (Gesamtsumme Brutto,
                # MwSt, Zwischensumme, Endpreis) mit unsinnigen Rekonstruktionen
                # überschrieben, weil hier `simple_vat` (MwSt der Basis) als
                # MwSt für `final_gross` verwendet wird – mathematisch falsch.
                _placeholder_zero = ("0,00 €", "0,00€", "0.00 €", "", None)

                def _needs_fallback(key: str) -> bool:
                    val = result.get(key)
                    if val is None:
                        return True
                    return str(val).strip() in _placeholder_zero

                if simple_brutto_value is not None and simple_vat_value is not None:
                    base_net_value = max(simple_brutto_value - simple_vat_value, 0.0)
                    ...
                    discount_net_value = (
                        parse_float(result.get("calc_total_discounts")) or 0.0
                    )
                    surcharge_net_value = (
                        parse_float(result.get("calc_total_surcharges")) or 0.0
                    )
                    ...
                    # Nur überschreiben, wenn aus final_pricing_data noch kein
                    # gültiger Wert geliefert wurde (Fallback-Verhalten).
                    if _needs_fallback("preis_mit_mwst_formatted"):
                        result["preis_mit_mwst_formatted"] = fmt_number(
                            simple_brutto_value, 2, "€"
                        )
                    if _needs_fallback("minus_mwst_formatted"):
                        result["minus_mwst_formatted"] = fmt_number(
                            simple_vat_value, 2, "€"
                        )
                    if _needs_fallback("zwischensumme_preis_formatted"):
                        result["zwischensumme_preis_formatted"] = fmt_number(
                            final_gross_value, 2, "€"
                        )
                    # final_end_preis: nur setzen, wenn noch leer
                    if _needs_fallback("final_end_preis_formatted"):
                        result["final_end_preis_formatted"] = fmt_number(
                            final_net_value, 2, "€"
                        )

                # Seite 8 nutzt im Template den Token VAT_SAVINGS_FORMATTED,
                # der intern auf vat_savings_formatted zeigt. Die konsolidierte
                # MwSt-Zeile liegt aber unter minus_mwst_formatted. Beide Keys
                # müssen identisch sein, sonst kann Seite 8 wieder eine alte
                # oder falsch abgeleitete VAT-SAVINGS-Zahl rendern.
                if not _needs_fallback("minus_mwst_formatted"):
                    result["vat_savings_formatted"] = result["minus_mwst_formatted"]
                    result["VAT_SAVINGS_FORMATTED"] = result[
                ...
                alias_for_pdf_bytes = {
                    ...
                    "zwischensumme_preis_formatted": result[
        ...
        print(
            "DEBUG: Seite 7 Preisberechnung - Alle Platzhalter gesetzt (OPTION A - nur UPPERCASE Keys!)"
        )
    ...
    # berechneten Werten.
    if (
        "anlage_kwp" in result
        and result["anlage_kwp"]
        and result["anlage_kwp"] != "0.00 kWp"
    ):
        # Kopiere die bereits berechnete Anlagengröße nach anlage_kwp_gesamt
        result["anlage_kwp_gesamt"] = result["anlage_kwp"]
    elif (
        "kWp_anlage_anlage" in result
        and result["kWp_anlage_anlage"]
        and result["kWp_anlage_anlage"] != "0.00 kWp"
    ):
    ...
    try:
        ...
        if calculated_amortization is not None:
            if (
                amortization_years_value is None
                or abs(amortization_years_value - calculated_amortization) > 0.05
            ):
        ...
        print(f"WARN: Fehler bei der Amortisationsberechnung (PDF): {amort_calc_error}")
    ...
    if (
        ...
        result["amortization_time"] = fmt_number(amortization_years_value, 2, "Jahre")
    ...
    try:
        ...
        def _normalize_month_labels(source: Any) -> list[str]:
            ...
            if isinstance(source, str):
                return [
                    part.strip() for part in re.split(r"[,;/|]", source) if part.strip()
                ]
            return []
        ...
        def _normalize_series(series: Any) -> list[float]:
                ...
                for item in series:
                    ...
                    try:
                        if isinstance(candidate, str):
                            token = re.sub(r"[^0-9,\.\-]", "", candidate).replace(
                                ",", "."
                            )
                            cleaned.append(
                                float(token) if token not in {"", "-", "."} else 0.0
                            )
                        else:
            ...
            if isinstance(series, str):
                ...
                if numbers and len(numbers) >= 12:
                    return [float(num.replace(",", ".")) for num in numbers[:12]]
            return []
        ...
        print(f"WARN: Monatsdaten für Seite 1 Chart nicht verfügbar: {chart_err}")
    ...
    try:
        ...
        if payment_data is None:
            ...
            if not selected_variant_key and isinstance(project_data, dict):
                selected_variant_key = project_data.get(
            ...
            if selected_variant_key:
                ...
                try:
                    ...
                    include_amounts_pref = session_get("payment_include_amounts", True)
        ...
        if isinstance(payment_data, dict):
            ...
            _assign_payment(
                "payment_anzahlung_percent_plain", "anzahlung_percent_plain"
            )
            _assign_payment("payment_anzahlung_amount", "anzahlung_amount")
            _assign_payment("payment_anzahlung_amount_plain", "anzahlung_amount_plain")

            _assign_payment("payment_nach_dc_percent", "nach_dc_percent")
            _assign_payment("payment_nach_dc_percent_plain", "nach_dc_percent_plain")
            _assign_payment("payment_nach_dc_amount", "nach_dc_amount")
            _assign_payment("payment_nach_dc_amount_plain", "nach_dc_amount_plain")

            _assign_payment("payment_nach_betrieb_percent", "nach_betrieb_percent")
            _assign_payment(
                "payment_nach_betrieb_percent_plain", "nach_betrieb_percent_plain"
            )
            _assign_payment("payment_nach_betrieb_amount", "nach_betrieb_amount")
            _assign_payment(
                "payment_nach_betrieb_amount_plain", "nach_betrieb_amount_plain"
            )
        ...
        print(f"WARN: Zahlungsdaten für Seite 8 nicht verfügbar: {payment_err}")
    ...
    def _assign_alias(
        target_key: str, source_keys: tuple[str, ...], formatter=None
    ) -> None:
