...
def build_dynamic_data(
        ...
        try:
            if not all(
                ...
                from database import (
                    load_admin_setting as _load_admin_setting,
                )  # optional
    ...
    try:
        ...
        def extract_brands_from_project_data(
            project_data_local: dict[str, Any],
        ) -> dict[str, str]:
    ...
    try:
        ...
        # Priorität: analysis_results > result (aus DB/project_details)
        stor_manufacturer = (
            analysis_results.get("storage_manufacturer")
            or result.get("storage_manufacturer")
            or ""
        ).strip()
        stor_model = (
            analysis_results.get("storage_model") or result.get("storage_model") or ""
        ).strip()
        ...
        if (
            ...
            try:
                ...
                if simple_brutto_value is not None and simple_vat_value is not None:
                    ...
                    if (
                        not result.get("final_end_preis_formatted")
                        or result.get("final_end_preis_formatted") == "0,00 €"
                    ):
        ...
        if payment_data is None:
            ...
            if selected_variant_key:
                ...
                try:
                    ...
                    try:
                        from database import (
                            load_admin_setting as _load_payment_setting,
                        )  # type: ignore
                    except Exception:
