...
class JobManager:
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        Returns:
            ...
            - dead_letter: Jobs in dead letter queue
    ...
    def get_job_history(self, limit: int = 100, status_filter: JobStatus | None = None) -> list[tuple[Job, JobResult]]:
        ...
        Returns:
            List of (Job, JobResult) tuples, sorted by completion time
        """
        history = []
        ...
        with self.lock:
            for job_id, result in self.job_results.items():
                ...
                # Try to find job in queue or running_jobs
                job = self.running_jobs.get(job_id)
                if not job:
                    # Try to reconstruct from result metadata
                    job = Job(
                        id=job_id,
                        name=result.metadata.get('job_name', 'Unknown'),
