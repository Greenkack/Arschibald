...
def card(
    ...
    **kwargs
    ...
    Args:
        ...
        variant: Color variant (red, green, yellow, orange, blue, purple, gray)
        **kwargs: Additional arguments
    """
    if SHADCN_UI_AVAILABLE:
        try:
    ...
    # Fallback to native container with custom styling
    # Map variant to orange accent color
    accent_colors = {
        "red": "#dc2626",
        "green": "#16a34a", 
        "yellow": "#ca8a04",
    ...
    accent_color = accent_colors.get(variant, "#ff8c00")  # Default to orange
    
    # Custom card styling with white background, shadow, and orange accent
    st.markdown(f"""
    <div style="
        background: white;
        padding: 1.5rem;
    ...
    ">
        ...
        <div style="color: #0f172a; font-size: 1.875rem; font-weight: 700; line-height: 1.2;">
            {description or ""}
        </div>
        {f'<div style="color: #475569; margin-top: 0.5rem;">{content}</div>' if content else ''}
    </div>
    """, unsafe_allow_html=True)
