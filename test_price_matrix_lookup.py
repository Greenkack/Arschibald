...
def test_find_module_count_row():
    ...
    if not matrix_id:
        print("❌ Fehler: Konnte Test-Matrix nicht erstellen")
        return False
    ...
    if row_label == "20" and row_id == row_20:
        print(f"✓ Exakte Übereinstimmung: {row_label} (ID: {row_id})")
    else:
        print(f"❌ Fehler: Erwartet '20' (ID: {row_20}), erhalten '{row_label}' (ID: {row_id})")
        return False
    ...
    if row_label == "15" and row_id == row_15:
        print(f"✓ Floor-Logik: {row_label} (ID: {row_id})")
    else:
        print(f"❌ Fehler: Erwartet '15' (ID: {row_15}), erhalten '{row_label}' (ID: {row_id})")
        return False
    ...
    if row_label is None and row_id is None:
        print("✓ Korrekt: Keine Zeile gefunden")
    else:
        print(f"❌ Fehler: Erwartet None, erhalten '{row_label}' (ID: {row_id})")
        return False
    ...
    if row_label == "25" and row_id == row_25:
        ...
        print(f"❌ Fehler: Erwartet '25' (ID: {row_25}), erhalten '{row_label}' (ID: {row_id})")
        return False
    
    # Cleanup
    price_matrix_store.delete_matrix(matrix_id)
    
    print("\n✓ Alle Tests für find_module_count_row bestanden")
    return True
...
def test_find_storage_column():
    ...
    if col_label == "15kWh" and col_id == col_15kwh:
        print(f"✓ Exakte Übereinstimmung: {col_label} (ID: {col_id})")
    else:
        print(f"❌ Fehler: Erwartet '15kWh' (ID: {col_15kwh}), erhalten '{col_label}' (ID: {col_id})")
        return False
    ...
    if col_label == "10kWh" and col_id == col_10kwh:
        print(f"✓ Case-insensitive: {col_label} (ID: {col_id})")
    else:
        print(f"❌ Fehler: Erwartet '10kWh' (ID: {col_10kwh}), erhalten '{col_label}' (ID: {col_id})")
        return False
    ...
    if col_label == "Kein Speicher" and col_id == col_none:
        print(f"✓ Kein Speicher: {col_label} (ID: {col_id})")
    else:
        print(f"❌ Fehler: Erwartet 'Kein Speicher' (ID: {col_none}), erhalten '{col_label}' (ID: {col_id})")
        return False
    ...
    if col_label is None and col_id is None:
        print("✓ Korrekt: Keine Spalte gefunden")
    else:
        print(f"❌ Fehler: Erwartet None, erhalten '{col_label}' (ID: {col_id})")
        return False
    
    # Cleanup
    price_matrix_store.delete_matrix(matrix_id)
    
    print("\n✓ Alle Tests für find_storage_column bestanden")
    return True
...
def test_lookup_price_by_intersection():
    ...
    if price == 15000.0:
        ...
        print(f"❌ Fehler: Erwartet 15000.0, erhalten {price}")
        return False
    ...
    if price == 18000.0:
        ...
        print(f"❌ Fehler: Erwartet 18000.0, erhalten {price}")
        return False
    ...
    if price is None:
        print("✓ Korrekt: Keine Preis-Zelle gefunden")
    else:
        print(f"❌ Fehler: Erwartet None, erhalten {price}")
        return False
    ...
    if price is None:
        print("✓ Korrekt: Keine Zelle für ungültige IDs")
    else:
        print(f"❌ Fehler: Erwartet None, erhalten {price}")
        return False
    ...
    print("\n✓ Alle Tests für lookup_price_by_intersection bestanden")
    return True
...
def test_calculate_price_from_matrix():
    ...
    if not result['success'] and result['error_type'] == 'no_row':
        print(f"✓ Korrekt: {result['error']}")
    else:
        print(f"❌ Fehler: Erwarteter Fehlertyp 'no_row', erhalten '{result.get('error_type')}'")
        return False
    ...
    if not result['success'] and result['error_type'] == 'no_column':
        ...
        print(f"❌ Fehler: Erwarteter Fehlertyp 'no_column', erhalten '{result.get('error_type')}'")
        return False
    ...
    print("\n✓ Alle Tests für calculate_price_from_matrix bestanden")
    return True
