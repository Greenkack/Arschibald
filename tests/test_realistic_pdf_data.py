...
def test_realistic_pdf_data():
    ...
    try:
        ...
        print(
            f"  2. amortization_time: {
                result.get(
                    'amortization_time',
                    'FEHLT')}")
        print(
            f"  3. preis_mit_mwst_formatted: {
                result.get(
        ...
        # 3. Seite 7 Werte (sollten berechnet werden) - prüfe die gemappten
        # Keys
        seite7_mapped_keys = [
        ...
        seite7_ok = all(result.get(key) and result.get(key) !=
                        "0,00 €" for key in seite7_mapped_keys)
