...
def test_new_calculations():
    ...
    print(
        f"- Wechselrichterleistung: {analysis_results['inverter_total_power_kw']} kW")
    print(
        f"- Jährliche Einspeisung: {analysis_results['annual_grid_feed_in_kwh']} kWh")
    print(
        f"- EEG-Vergütung: {analysis_results['eeg_verguetung_cent_pro_kwh']} ct/kWh")
    print(
        f"- Strompreis: {analysis_results['strompreis_cent_pro_kwh']} ct/kWh")
    print()
    ...
    try:
        ...
        print(
            f"   - Einspeisung: {result.get('annual_feed_in_revenue_eur', 'N/A')}")
        print(
            f"   - Steuerliche Vorteile (platz1): {result.get('tax_benefits_eur', 'N/A')}")
        print()

        print("2. Gesamtersparnisse-Berechnung:")
        print(
            f"   - Direkt: {result.get('self_consumption_without_battery_eur', 'N/A')}")
        print(
            f"   - Einspeisung: {result.get('annual_feed_in_revenue_eur', 'N/A')}")
        print(f"   - Steuervorteile: {result.get('tax_benefits_eur', 'N/A')}")
        print(
            f"   - Speichernutzung: {result.get('battery_usage_savings_eur', 'N/A')}")
        print(
            f"   - Speicherüberschuss: {result.get('battery_surplus_feed_in_eur', 'N/A')}")
        print(f"   - GESAMT: {result.get('total_annual_savings_eur', 'N/A')}")
        print()

        print("3. Wechselrichterformat:")
        print(
            f"   - kW (für Seite 4): {result.get('inverter_total_power_kw', 'N/A')}")
        print(
            f"   - W (für Seite 1): {result.get('inverter_total_power_w', 'N/A')}")
        print()
        ...
        print(
            f"   'Gesamt' -> {result.get('total_annual_savings_eur', 'N/A')}")
        print(
            f"   'Warmwasser' -> {result.get('inverter_total_power_w', 'N/A')}")
