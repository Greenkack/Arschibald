...
def test_page_content_placement():
    ...
    try:
        ...
        assert len(
            pdf_reader.pages) == 8, f"Expected 8 pages, got {len(pdf_reader.pages)}"
        ...
        # Note: Text extraction from PDF may not be perfect, so we check for
        # existence
        ...
        if "ANGEBOT" in page2_text.upper() or "PERSÖNLICH" in page2_text.upper():
            ...
            print(
                f"   ⚠ Page 2 text extraction may be incomplete (this is common with PDF text extraction)")
        ...
        # The important thing is that the page exists and the function was
        # called
...
def test_renamed_functions_exist():
    ...
    try:
        ...
        if all_functions_exist:
            ...
            print(
                f"✗ TEST 9.4 FAILED: {
                    len(missing_functions)} functions are missing:")
...
def run_all_tests():
    ...
    if passed == total:
        ...
        print(
            f"\n⚠ {
                total -
                passed} test(s) failed. Please review the output above.")
        print("=" * 70 + "\n")
        return False
