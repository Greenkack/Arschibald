...
try:
    ...
    print("   [OK] task_manager.py erfolgreich importiert")
...
try:
    ...
    print("   [OK] task_ui.py erfolgreich importiert")
except ImportError as e:
    print(f"   [ERROR] Fehler beim Importieren: {e}")
    exit(1)
...
try:
    ...
    print(f"   [OK] Statistiken geladen: {stats.get('total', 0)} Tasks in DB")
...
try:
    ...
    if task_id:
        print(f"   [OK] Task #{task_id} erfolgreich erstellt")
    else:
        print("   [WARNING]  Task-Erstellung gab None zurück (möglicherweise DB-Problem)")
except Exception as e:
    print(f"   [ERROR] Fehler: {e}")
    task_id = None
...
if task_id:
    ...
    try:
        ...
        if task:
            print(f"   [OK] Task geladen: '{task.get('title')}'")
            print(f"      Status: {task.get('status')}")
            print(f"      Priorität: {task.get('priority')}")
        else:
            print("   [ERROR] Task nicht gefunden")
    except Exception as e:
        print(f"   [ERROR] Fehler: {e}")
    ...
    try:
        ...
        if mark_task_in_progress(task_id):
            print("   [OK] Status auf 'in_progress' gesetzt")
    ...
    try:
        if update_task(task_id, description="Aktualisierte Beschreibung"):
            print("   [OK] Task erfolgreich aktualisiert")
    except Exception as e:
        print(f"   [ERROR] Fehler: {e}")
    ...
    try:
        if delete_task(task_id):
            print("   [OK] Task erfolgreich gelöscht")
    except Exception as e:
        print(f"   [ERROR] Fehler: {e}")
...
try:
    ...
    print(f"   [OK] get_all_tasks(): {len(all_tasks)} Tasks")
    
    overdue = get_overdue_tasks()
    print(f"   [OK] get_overdue_tasks(): {len(overdue)} überfällige Tasks")
    
    due_soon = get_tasks_due_soon(days=7)
    print(f"   [OK] get_tasks_due_soon(7): {len(due_soon)} Tasks")
except Exception as e:
    print(f"   [ERROR] Fehler: {e}")
...
try:
    stats = get_task_statistics()
    print(f"   [OK] Gesamt: {stats.get('total', 0)}")
    print(f"   [OK] Überfällig: {stats.get('overdue', 0)}")
    print(f"   [OK] Heute fällig: {stats.get('due_today', 0)}")
    print(f"   [OK] Diese Woche: {stats.get('due_this_week', 0)}")
    
    by_status = stats.get('by_status', {})
    print(f"   [OK] Nach Status: {by_status}")
    
    by_priority = stats.get('by_priority', {})
    print(f"   [OK] Nach Priorität: {by_priority}")
except Exception as e:
    print(f"   [ERROR] Fehler: {e}")
...
print("=" * 70)
print("\n[OK] Alle Kern-Funktionen sind verfügbar und funktionieren.")
