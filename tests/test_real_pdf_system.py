...
def test_real_pdf_generation():
    ...
    print("Test-Daten:")
    print(
        f"  - PV-Anlage: {test_project_data['project_details']['anlage_kwp']} kWp")
    print(
        f"  - Wechselrichter: {test_project_data['project_details']['selected_inverter_power_kw']} kW")
    print(
        f"  - Einkommensteuersatz: {test_analysis_results['einkommensteuer_satz']}%")
    print(f"  - Einspeisung: {test_analysis_results['val_feedin_kwh']} kWh")
    print(
        f"  - EEG-Vergütung: {test_analysis_results['feed_in_tariff_eur_per_kwh']} €/kWh")
    ...
    try:
        ...
        print(
            f"  - Gesamt ohne Speicher: {result.get('total_savings_eur', 'N/A')}")
        print(
            f"  - Wechselrichter in W: {result.get('inverter_total_power_w', 'N/A')}")
        ...
        try:
            ...
            if "Warmwasser" in PLACEHOLDER_TO_DATA_KEY_S1:
                ...
                mapped_value = result.get(mapped_key, 'N/A')
                print(
                    f"  - 'Warmwasser' (Seite 1) -> '{mapped_key}' = {mapped_value}")
        except ImportError as e:
            print(f"  - Platzhalter-Import-Fehler: {e}")
            # Versuche direkte Prüfung der wichtigen Werte
            if 'tax_benefits_eur' in result:
                print(
                    f"  - tax_benefits_eur gefunden: {result['tax_benefits_eur']}")
            if 'total_savings_eur' in result:
                print(
                    f"  - total_savings_eur gefunden: {result['total_savings_eur']}")
            if 'inverter_total_power_w' in result:
                print(
                    f"  - inverter_total_power_w gefunden: {result['inverter_total_power_w']}")
