...
class PDFExtendedGenerationTester:
    ...
    def create_test_project_data(self, 
        ...
        try:
            ...
            # Get module
            cursor.execute("SELECT id FROM products WHERE category = 'Modul' LIMIT 1")
            module_row = cursor.fetchone()
            module_id = module_row[0] if module_row else 1
            
            # Get inverter
            cursor.execute("SELECT id FROM products WHERE category = 'Wechselrichter' LIMIT 1")
            inverter_row = cursor.fetchone()
            inverter_id = inverter_row[0] if inverter_row else 2
            
            # Get storage
            cursor.execute("SELECT id FROM products WHERE category = 'Speicher' LIMIT 1")
            storage_row = cursor.fetchone()
            storage_id = storage_row[0] if storage_row else 3
            
            # Get accessories
            cursor.execute("SELECT id FROM products WHERE category = 'Wallbox' LIMIT 1")
            wallbox_row = cursor.fetchone()
            wallbox_id = wallbox_row[0] if wallbox_row else None
            
            cursor.execute("SELECT id FROM products WHERE category = 'EMS' LIMIT 1")
            ems_row = cursor.fetchone()
            ems_id = ems_row[0] if ems_row else None
            
            cursor.execute("SELECT id FROM products WHERE category = 'Optimizer' LIMIT 1")
    ...
    def test_7_1_all_accessory_components(self) -> TestResult:
        ...
        """
        result = TestResult("7.1 - PDF generation with all accessory components")
        ...
        try:
            ...
            self.log(f"  - Module ID: {project_data['pv_details']['selected_module_id']}")
            self.log(f"  - Inverter ID: {project_data['pv_details']['selected_inverter_id']}")
            self.log(f"  - Storage ID: {project_data['pv_details']['selected_storage_id']}")
            self.log(f"  - Wallbox ID: {project_data['pv_details']['selected_wallbox_id']}")
            self.log(f"  - EMS ID: {project_data['pv_details']['selected_ems_id']}")
            self.log(f"  - Optimizer ID: {project_data['pv_details']['selected_optimizer_id']}")
            self.log(f"  - Include all documents: {inclusion_options['include_all_documents']}")
            ...
            test_output_path = os.path.join(self.test_data_dir, "pdf_output", "test_7_1_all_accessories.pdf")
    ...
    def test_7_2_company_documents(self) -> TestResult:
        ...
        try:
            ...
            result.passed = True
            result.message = (f"PDF generated with "
                            f"{result.details.get('page_count', '?')} pages, "
                            f"{len(doc_ids)} company docs requested")
