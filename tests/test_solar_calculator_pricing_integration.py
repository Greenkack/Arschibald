...
class TestSolarCalculatorPricingIntegration:
    ...
    def test_storage_pricing_calculation(
        ...
        # Regression: Dieser alte UI-Wert darf NICHT als Mengen-Multiplikator
        # verwendet werden. Früher wurde daraus 50 / 10 = 5 Speicher gemacht.
        sample_project_details["selected_storage_storage_power_kw"] = 50.0
        # Regression 2: Auch ein falsch gepflegtes calculate_per darf den
        # Speicherpreis nicht mit kWh/kWp/Kapazität multiplizieren.
        sample_product_data["storage"]["calculate_per"] = "kWp"
        sample_product_data["storage"]["capacity_w"] = 10000.0
        ...
        assert storage_pricing["original_calculate_per"] == "kWp"
        assert storage_pricing["total_price"] > 0
        assert storage_pricing["total_price"] == pytest.approx(
            storage_pricing["unit_price"])
        assert storage_pricing["desired_capacity"] == 10.0
        assert storage_pricing["unit_capacity"] == 10.0
