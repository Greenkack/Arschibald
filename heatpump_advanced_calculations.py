...
def calculate_tax_benefits(
    ...
    if is_old_building:
        ...
        tax_benefits['combination_note'] = (
            "WICHTIG: §35c (energetische Sanierung) schließt §35a (Handwerkerleistungen) aus! "
...
def _determine_optimal_location(noise_level: float, distance: float, limit: float) -> Dict[str, Any]:
    ...
    if distance < min_distance:
        recommendations.append(
            f"Aktueller Abstand ({distance}m) zu gering - mind. {min_distance:.1f}m empfohlen"
        )
    else:
        recommendations.append(
...
def _assess_noise_situation(compliant: bool, margin: float) -> str:
    ...
    if not compliant:
        ...
        return "GRENZWERTIG - Knapp unter Grenzwerten, Maßnahmen empfohlen"
    else:
        return "KRITISCH - Sehr knapp an Grenzwerten, Maßnahmen dringend empfohlen"
...
def _get_hybrid_recommendation(payback_years: float, additional_invest: float, backup: str) -> str:
    ...
    if payback_years < 10 and 'Gas' in backup:
        ...
        return f"GRENZWERTIG - Amortisation {payback_years:.1f} Jahre, nur bei bestehendem {backup} sinnvoll"
    elif additional_invest < 3000:
        return "ÜBERDENKEN - Geringe Mehrkosten, aber Wartungsaufwand für zweites System"
    else:
...
def _interpret_co2_savings(savings_kg: float, break_even_year: int) -> str:
    ...
    if break_even_year is None or break_even_year > 20:
        ...
        return f"AKZEPTABEL - Break-Even nach {break_even_year} Jahren, moderate Einsparung ({savings_kg/1000:.1f} t)"
    else:
        return f"GRENZWERTIG - Break-Even erst nach {break_even_year} Jahren"
...
def _assess_refrigerant(data: Dict, years_left: int) -> str:
    ...
    if data['gwp'] < 10:
        ...
        return f"AKZEPTABEL - Mittleres GWP ({data['gwp']}), noch {years_left} Jahre compliant"
    elif data['gwp'] < 700:
        return f"GRENZWERTIG - Mittleres GWP ({data['gwp']}), nur noch {years_left} Jahre compliant"
    else:
...
def _simulate_cold_wave(
    ...
    if coverage >= 1.0:
        ...
        assessment = f"WP UNTERDIMENSIONIERT - {round((1 - coverage) * 100, 0)}% Leistungsdefizit"
        recommendation = [
...
def _simulate_blackout(building_data: Dict, heatpump_data: Dict, heat_load_kw: float) -> Dict[str, Any]:
    ...
    if final_temp > 16:
        ...
        assessment = "UNBEQUEM - Temperatur sinkt deutlich, aber kein Frostschutz nötig"
    else:
...
def compare_multiple_heatpumps(
    ...
    if not winner['noise_compliant']:
        recommendation['general_advice'].append(
            'Lautstärke kritisch: Schallschutzmaßnahmen einplanen'
        )
