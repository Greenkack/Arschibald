...
class SolarCalculatorPricingIntegration:
    ...
    def calculate_component_pricing(
        ...
        try:
            ...
            # Process PV mounting (Unterkonstruktion)
            mounting_components = self._calculate_pv_mounting_pricing(project_details)
            ...
            # Services are handled entirely by update_pricing_with_services() in solar_calculator.py
            # Do NOT call _calculate_services_pricing() here to avoid double-counting.
    ...
    def _calculate_storage_pricing(
        ...
        try:
            ...
            def _product_capacity_kwh(product_data: dict[str, Any]) -> float:
                for key in (
                    "capacity_kwh",
                    "usable_capacity_kwh",
                    ...
                    try:
                        value = float(product_data.get(key, 0) or 0)
                    except Exception:
                        value = 0.0
                    if value > 0:
                        return value
            ...
            # WICHTIG: Das frühere manuelle Speicher-kWh-Feld
            # ist kein Mengenfeld. Ein ausgewählter Speicher ist genau eine
            # Angebotsposition. Sonst entstehen falsche Multiplikatoren wie
            # 5 × derselbe Speicher.
            quantity = 1
            unit_capacity = _product_capacity_kwh(product)
            desired_capacity = unit_capacity

            # Calculate pricing
            base_price = float(product.get("price_euro", 0.0))
            original_calculate_per = product.get("calculate_per", "Stück")
            # Speicher sind eine ausgewählte Produktposition. Der Preis darf
            # weder mit der alten Wunsch-kWh noch mit der Produktkapazität
            # multipliziert werden, selbst wenn `calculate_per` falsch als
            # kWp/kWh gepflegt wurde.
            calculate_per = "Stück"
            ...
            if self.key_manager:
                component_keys = self.key_manager.generate_keys({
                    ...
                    "STORAGE_ORIGINAL_CALCULATE_PER": original_calculate_per,
            ...
            return {
                ...
                "total_price": total_selling_price,
                "calculate_per": calculate_per,
                "original_calculate_per": original_calculate_per,
    ...
    def _calculate_pv_mounting_pricing(
            self, project_details: dict[str, Any]) -> list[dict[str, Any]]:
        """Calculate pricing for PV mounting components (Unterkonstruktion)"""
        components = []
        if not project_details.get("include_pv_mounting", False):
            return components

        # Each mounting sub-component stored as pv_mounting_{key}_name / pv_mounting_{key}_price
        ...
        for key in component_keys:
            name = project_details.get(f"pv_mounting_{key}_name")
            price = float(project_details.get(f"pv_mounting_{key}_price", 0.0) or 0.0)
            if name:
                quantity = int(project_details.get(f"pv_mounting_{key}_quantity", 1) or 1)
                unit_price = price  # price IS already the unit price per piece
                total_price = unit_price * quantity
                label = component_labels.get(key, key.replace("_", " ").title())
                components.append({
                    "component_type": f"pv_mounting_{key}",
                    "model_name": name,
                    "quantity": quantity,
                    ...
                    "display_name": label,
                    "product_specs": {"brand": project_details.get(f"pv_mounting_{key}_manufacturer", "")},
                    "margin_info": None,
                    "dynamic_keys": {},
                    "is_optional": False,
                    "pricing_rules": {},
        ...
        # Also handle a single lump-sum mounting price if no individual components
        if not components:
            lump_price = float(project_details.get("pv_mounting_total_price", 0.0) or 0.0)
            mounting_name = project_details.get("pv_mounting_system_name", "")
            if lump_price > 0:
                components.append({
                    "component_type": "pv_mounting",
                    "model_name": mounting_name or "Unterkonstruktion",
                    "quantity": 1,
                    "unit_price": lump_price,
                    "total_price": lump_price,
                    "calculate_per": "pauschal",
                    "category": "Unterkonstruktion",
                    "display_name": "Unterkonstruktion",
                    "product_specs": {},
                    "margin_info": None,
                    "dynamic_keys": {},
                    "is_optional": False,
    ...
    def _calculate_services_pricing(
            self, project_details: dict[str, Any]) -> list[dict[str, Any]]:
        """Calculate pricing for selected services (Dienstleistungen / Zusatzdienste)"""
        components = []
        try:
            from services_integration import calculate_services_pricing as _calc_services

            # Collect selected service IDs from session state
            import streamlit as st
            ...
            try:
                for key, value in st.session_state.items():
                    if key.startswith("service_select_") and value:
                        try:
                            selected_service_ids.append(int(key.replace("service_select_", "")))
            ...
            services_result = _calc_services(selected_service_ids, project_details)
            all_services = (
                services_result.get("standard_services", []) +
                services_result.get("optional_services", [])
            )
            for svc in all_services:
                total_price = float(svc.get("total_price", svc.get("price", 0.0)) or 0.0)
                if total_price <= 0:
                    continue
                components.append({
                    "component_type": "service",
                    "model_name": svc.get("name", "Dienstleistung"),
                    "quantity": int(svc.get("quantity", 1) or 1),
                    "unit_price": float(svc.get("price", total_price) or total_price),
                    "total_price": total_price,
                    "calculate_per": svc.get("calculate_per", "pauschal"),
                    "category": "Dienstleistungen",
                    "display_name": svc.get("name", "Dienstleistung"),
                    "product_specs": {},
                    "margin_info": None,
                    "dynamic_keys": {},
                    "is_optional": svc.get("is_optional", False),
                    "pricing_rules": {},
                })
        except ImportError:
            pass
        except Exception as e:
            self.logger.error(f"Error calculating services pricing: {e}")
        return components
