...
print(
    f"DATABASE.PY TOP LEVEL: DB_SCHEMA_VERSION ist auf {DB_SCHEMA_VERSION} gesetzt.")
...
def _create_company_templates_tables_v14(conn: sqlite3.Connection):
    ...
    """)
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_company_text_templates_company_id ON company_text_templates (company_id);")
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_company_text_templates_type ON company_text_templates (template_type);")
    ...
    """)
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_company_image_templates_company_id ON company_image_templates (company_id);")
    cursor.execute(
        "CREATE INDEX IF NOT EXISTS idx_company_image_templates_type ON company_image_templates (template_type);")
...
def _ensure_column_exists(
        ...
        table_name: str,
        column_name: str,
        column_type_for_alter: str,
        is_not_null_with_default_for_alter: bool = False,
        default_value_for_alter: str = "''"):
    cursor = conn.cursor()
    ...
    if column_name not in existing_columns:
        ...
        try:
            cursor.execute(
                f"ALTER TABLE {table_name} ADD COLUMN {column_name} {final_column_definition_for_alter};")
            print(
                f"DB: Spalte '{column_name}' ({final_column_definition_for_alter}) zu Tabelle '{table_name}' hinzugefügt via ALTER TABLE.")
            conn.commit()
        except sqlite3.OperationalError as e_alter:
            if "duplicate column name" in str(e_alter).lower():
                print(
                    f"DB: Spalte '{column_name}' in '{table_name}' existiert bereits.")
            else:
                print(
                    f"DB FEHLER beim Hinzufügen von Spalte '{column_name}' zu '{table_name}': {e_alter}")
...
def init_db():
    conn = get_db_connection()
    if conn is None:
        print("DB FEHLER: init_db() kann DB-Verbindung nicht herstellen.")
        return
    try:
        cursor = conn.cursor()
        cursor.execute("PRAGMA user_version;")
        ...
        print(
            f"DB: Aktuelle user_version: {current_db_version}, Ziel-Schema-Version: {DB_SCHEMA_VERSION}")

        if current_db_version < 1:
            _create_admin_settings_table_v1(conn)
            _ensure_column_exists(
                conn, "admin_settings", "last_modified", "TEXT")
            cursor.execute(
                "UPDATE admin_settings SET value = '1' WHERE key = 'schema_version' OR key IS NULL;")
            conn.commit()
        ...
        if current_db_version < 2:
            ...
            cursor.execute(
                "UPDATE admin_settings SET value = '2' WHERE key = 'schema_version';")
            conn.commit()
            try:
        ...
        if current_db_version < 12:
            ...
            if 'company_name' in companies_cols_info and 'name' not in companies_cols_info:
                try:
                    print(
                        "DB: Alte Spalte 'company_name' in 'companies' gefunden, 'name' fehlt. Versuche Umbenennung zu 'name'...")
                    cursor.execute(
                        "ALTER TABLE companies RENAME COLUMN company_name TO name;")
                    conn.commit()
                    print(
                        "DB: Spalte 'company_name' erfolgreich zu 'name' in 'companies' umbenannt.")
                    cursor.execute("PRAGMA table_info(companies);")  # Refresh
                    companies_cols_info = {
                        row[1]: row for row in cursor.fetchall()}
                except sqlite3.OperationalError as e_rename_comp:
                    print(
                        f"DB HINWEIS: Umbenennung von 'company_name' zu 'name' in 'companies' fehlgeschlagen: {e_rename_comp}.")
            ...
            _ensure_column_exists(
                ...
                "name",
                "TEXT",
                is_not_null_with_default_for_alter=True,
                default_value_for_alter="''")  # Sicherstellen, dass 'name' existiert
            _ensure_column_exists(
                conn,
                "companies",
                "is_default",
            ...
            if 'document_name' in company_doc_cols_info and 'display_name' not in company_doc_cols_info:
                try:
                    print(
                        "DB: Alte Spalte 'document_name' in 'company_documents' gefunden, 'display_name' fehlt. Versuche Umbenennung...")
                    cursor.execute(
                        "ALTER TABLE company_documents RENAME COLUMN document_name TO display_name;")
                    conn.commit()
                    print(
                        "DB: Spalte 'document_name' erfolgreich zu 'display_name' umbenannt.")
                    cursor.execute("PRAGMA table_info(company_documents);")
                    company_doc_cols_info = {
                        row[1]: row for row in cursor.fetchall()}
                except sqlite3.OperationalError as e_rename_doc:
                    print(
                        f"DB HINWEIS: Umbenennung von 'document_name' zu 'display_name' in 'company_documents' fehlgeschlagen: {e_rename_doc}.")
            ...
            _ensure_column_exists(
                ...
                "TEXT",
                is_not_null_with_default_for_alter=True,
                default_value_for_alter="''")
            _ensure_column_exists(
                conn, "company_documents", "file_name", "TEXT")
            _ensure_column_exists(
                conn,
                "company_documents",
                "absolute_file_path",
            ...
            cursor.execute(
                "UPDATE admin_settings SET value = '12' WHERE key = 'schema_version';")
            conn.commit()
            try:
        ...
        if current_db_version < 13:
            ...
            cursor.execute(
                "UPDATE admin_settings SET value = '13' WHERE key = 'schema_version';")
            conn.commit()
            try:
            ...
            print("DB: Schema v13 angewendet.")
        if current_db_version < 14:
            _create_company_templates_tables_v14(conn)
            cursor.execute(
                "UPDATE admin_settings SET value = '14' WHERE key = 'schema_version';")
            conn.commit()
            try:
            ...
            print("DB: Schema v14 angewendet (Firmenspezifische Angebotsvorlagen).")

        # Stelle sicher, dass die SQLite user_version am Ende exakt dem
        # Code-Schema entspricht
        try:
        ...
        if current_db_version == DB_SCHEMA_VERSION:
            print("DB: Schema ist aktuell.")
        else:
            print(
                f"DB WARNUNG: Diskrepanz user_version ({current_db_version}) vs Code ({DB_SCHEMA_VERSION}).")
        ...
        for key, default_value in INITIAL_ADMIN_SETTINGS.items():
            ...
            if cursor.fetchone() is None:
                value_insert = json.dumps(default_value) if isinstance(
                    ...
                    default_value, bool) else default_value
                if value_insert is None and key in [
        ...
        print(f"DB KRITISCHER FEHLER init_db: {e}")
        traceback.print_exc()
        conn.rollback()
    finally:
        if conn:
    ...
    try:
        ...
        if row and row['value'] is not None:
            ...
            if isinstance(value_str, str) and value_str.strip().startswith(
                    ('[', '{')) and value_str.strip().endswith((']', '}')):
                try:
                    return json.loads(value_str)
                except json.JSONDecodeError:
                    pass
            if key in INITIAL_ADMIN_SETTINGS and isinstance(
                    INITIAL_ADMIN_SETTINGS.get(key), bool):
                try:
            ...
            if key == 'active_company_id':
                try:
                    ...
                    return default
            return value_str
        if key == 'active_company_id' and row and row['value'] is None:
            return None
        return default
    except Exception as e:
        print(f"DB Fehler load_admin_setting '{key}': {e}")
        return default
    finally:
        if conn:
            conn.close()
...
def save_admin_setting(key: str, value: Any) -> bool:
    ...
    try:
        cursor = conn.cursor()
        value_to_save = json.dumps(value) if isinstance(
        ...
        if key == 'price_matrix_csv_data' and value_to_save is not None:
            print(
                f"DB DEBUG: save_admin_setting - Länge von value_to_save für '{key}': {len(str(value_to_save))} Zeichen.")
            # print(f"DB DEBUG: save_admin_setting - Inhalt für '{key}' (erste
            # 200 Zeichen): {str(value_to_save)[:200]}") # Bei Bedarf
            # einkommentieren
        ...
        params_for_sql = (
            key,
            None if (
                value_to_save is None and key in [
                    'active_company_id',
                    'price_matrix_csv_data']) else value_to_save)
        print(
            f"DB DEBUG: save_admin_setting - Versuche SQL auszuführen für Key '{key}'. Wert None? {
                params_for_sql[1] is None}")
        cursor.execute(sql_query, params_for_sql)
        conn.commit()
        print(
            f"DB ERFOLG: save_admin_setting - Einstellung '{key}' erfolgreich gespeichert.")
        return True
    except Exception as e:
...
def add_pdf_template(
        ...
        image_data: bytes | None = None) -> int | None:
    conn = get_db_connection()
    if not conn:
        return None
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO pdf_templates (template_type, name, content, image_data, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
            (template_type,
             name,
             content,
             image_data,
        ...
        print(f"DB Fehler add_pdf_template: {e}")
        conn.rollback()
        return None
    finally:
        if conn:
...
def list_pdf_templates(template_type: str |
    ...
    try:
        ...
        print(f"DB Fehler list_pdf_templates: {e}")
        return []
    finally:
        if conn:
            conn.close()
...
def get_pdf_template(template_id: int) -> dict[str, Any] | None:
    conn = get_db_connection()
    if not conn:
        return None
    try:
        ...
        print(f"DB Fehler get_pdf_template: {e}")
        return None
    finally:
        if conn:
            conn.close()
...
def update_pdf_template(
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE pdf_templates SET name=?, content=?, image_data=?, updated_at=? WHERE id=?",
            (name,
             content,
             image_data,
             now,
        ...
        print(f"DB Fehler update_pdf_template: {e}")
        conn.rollback()
        return False
    finally:
        if conn:
...
def delete_pdf_template(template_id: int) -> bool:
    ...
    try:
        ...
        print(f"DB Fehler delete_pdf_template: {e}")
        conn.rollback()
        return False
    finally:
        if conn:
...
if not os.path.exists(COMPANY_DOCS_BASE_DIR):
    try:
        os.makedirs(COMPANY_DOCS_BASE_DIR)
    except OSError as e:
        print(f"DB FEHLER Erstellen Firmen-Doc-Verzeichnis: {e}")


def add_company(company_data: dict[str, Any]) -> int | None:
    conn = get_db_connection()
    if not conn:
    ...
    # print(f"DB DEBUG: add_company - Empfangene company_data['name']:
    # '{company_name_to_add}'") # Bereits im Log

    if not company_name_to_add or not str(company_name_to_add).strip():
        print(
            f"DB FEHLER: Firmenname ist Pflicht und darf nicht leer sein. Empfangen: '{company_name_to_add}'")
        if conn:
            conn.close()
        return None

    company_name_to_add_stripped = str(company_name_to_add).strip()
    # print(f"DB DEBUG: add_company - Name nach strip():
    # '{company_name_to_add_stripped}'") # Bereits im Log

    cursor = conn.cursor()

    try:
        # print(f"DB DEBUG: add_company - Führe case-insensitive Vorab-Prüfung
        ...
        if existing_company_by_name_nocase:
            print(
                f"DB HINWEIS (Vorab-Prüfung): Firma ähnlich zu '{company_name_to_add_stripped}' (case-insensitive) existiert: ID {
                    existing_company_by_name_nocase['id']}, Name '{
                    existing_company_by_name_nocase['name']}'. Anlage abgebrochen.")
            if conn:
                conn.close()  # Schließe Verbindung hier, da Funktion beendet wird
            return None
        # else: # Bereits im Log
    ...
    # print(f"DB DEBUG: add_company - Versuche INSERT mit Name:
    # '{company_name_to_add_stripped}' und Werten: {values_to_insert}") #
    # Bereits im Log
    try:
        ...
        print(
            f"DB FEHLER (IntegrityError) beim INSERT: Firma '{company_name_to_add_stripped}' existiert bereits oder anderer UNIQUE Constraint verletzt. Fehler: {e_int}")
        return None
    except Exception as e:
        ...
        if conn:
            conn.close()  # Sicherstellen, dass die Verbindung immer geschlossen wird
...
def list_companies() -> list[dict[str, Any]]:
    ...
    try:
        ...
        print(f"DB Fehler list_companies: {e}")
        return []
    finally:
        if conn:
            conn.close()
...
def get_company(company_id: int) -> dict[str, Any] | None:
    ...
    try:
        ...
        print(f"DB Fehler get_company (ID: {company_id}): {e}")
        return None
    finally:
        if conn:
            conn.close()
...
def update_company(company_id: int, company_data: dict[str, Any]) -> bool:
    ...
    for key, val in update_data_db.items():
        if key in allowed_fields_for_update:
            ...
            if key == "is_default":
                values_for_set.append(int(val) if val is not None else 0)
            elif isinstance(val, str):
                values_for_set.append(val.strip() if val else None)
            else:
                values_for_set.append(val)

    if not fields_to_set_parts:
        return False
    values_for_set.append(company_id)
    stmt = f"UPDATE companies SET {
        ', '.join(fields_to_set_parts)} WHERE id = ?"
    ...
    try:
        ...
        print(
            f"DB Integritätsfehler update_company (ID {company_id}): {e_int}")
        conn.rollback()
        return False
    except Exception as e:
        print(f"DB Fehler update_company (ID {company_id}): {e}")
        conn.rollback()
        return False
    finally:
        if conn:
    ...
    try:
        ...
        if cursor.rowcount > 0:
            ...
            if active_id == company_id:
                save_admin_setting('active_company_id', None)
            return True
        return False
    except Exception as e:
        print(f"DB Fehler delete_company (ID: {company_id}): {e}")
        conn.rollback()
        return False
    finally:
        if conn:
...
def set_default_company(company_id: int) -> bool:
    ...
    try:
        ...
        print(f"DB Fehler set_default_company (ID: {company_id}): {e}")
        conn.rollback()
        return False
    finally:
        if conn:
...
def get_active_company() -> dict[str, Any] | None:
    ...
    if active_id_str is not None:
        try:
            ...
            print(
                f"DB Warnung: Ungültiger active_company_id Wert '{active_id_str}'.")
            active_id = None
    if active_id is not None:
        company_details = get_company(active_id)
        if company_details:
            return company_details
        print(f"DB Warnung: Aktive Firma ID {active_id} nicht in DB gefunden.")
        save_admin_setting('active_company_id', None)
        return None
    conn = get_db_connection()
    if not conn:
    ...
    try:
        ...
        print(f"DB Fehler get_active_company (Fallback): {e}")
    finally:
        if conn:
            conn.close()
    return None
...
def add_company_document(
        ...
        file_content_bytes: bytes) -> int | None:
    conn = get_db_connection()
    if not conn:
        return None
    if not display_name or not display_name.strip():
        ...
        return None
    company_specific_docs_dir = os.path.join(
        COMPANY_DOCS_BASE_DIR, str(company_id))
    if not os.path.exists(company_specific_docs_dir):
        try:
            os.makedirs(company_specific_docs_dir)
        except OSError as e_mkdir:
            print(
                f"DB: FEHLER Erstellen Verzeichnis {company_specific_docs_dir}: {e_mkdir}")
            return None
    safe_filename_base = "".join(
        c if c.isalnum() else "_" for c in os.path.splitext(original_filename)[0])
    ...
    absolute_path_on_disk = os.path.join(
        company_specific_docs_dir, final_safe_filename)
    try:
        ...
        print(
            f"DB: IOError beim Schreiben der Dokumentdatei {absolute_path_on_disk}: {e_io}")
        return None
    except sqlite3.Error as e_sql:
        print(f"DB: SQLite Fehler add_company_document: {e_sql}")
        conn.rollback()
        return None
    finally:
        if conn:
...
def add_company_text_template(
        company_id: int,
        name: str,
        content: str,
        template_type: str = "offer_text") -> int | None:
...
def add_company_image_template(
        company_id: int,
        name: str,
        image_data: bytes,
        template_type: str = "title_image",
        original_filename: str | None = None) -> int | None:
    ...
    company_images_dir = os.path.join(
        COMPANY_DOCS_BASE_DIR, str(company_id), "images")
    if not os.path.exists(company_images_dir):
        try:
            os.makedirs(company_images_dir)
        except OSError as e_mkdir:
            print(
                f"DB: FEHLER Erstellen Verzeichnis {company_images_dir}: {e_mkdir}")
            return None
    ...
    relative_path_for_db = os.path.join(
        str(company_id), "images", final_safe_filename)
    absolute_path_on_disk = os.path.join(
        company_images_dir, final_safe_filename)
    ...
    try:
        ...
        print(
            f"DB: IOError beim Schreiben der Bilddatei {absolute_path_on_disk}: {e_io}")
        return None
    except sqlite3.Error as e_sql:
...
def list_company_text_templates(
        company_id: int, template_type: str | None = None) -> list[dict[str, Any]]:
...
def list_company_image_templates(
        company_id: int, template_type: str | None = None) -> list[dict[str, Any]]:
    ...
    try:
        ...
        for row in cursor.fetchall():
            ...
            template_dict['absolute_file_path'] = os.path.join(
                COMPANY_DOCS_BASE_DIR, template_dict['file_path'])
...
def get_company_image_template_data(template_id: int) -> bytes | None:
        ...
        if row:
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    print(
                        f"DB Fehler beim Löschen der Bilddatei {file_path}: {e_os}")
...
def update_company_text_template(
        template_id: int,
        name: str,
        content: str) -> bool:
...
def list_company_documents(
    ...
    try:
        ...
        return [{**dict(row),
                 'absolute_file_path': os.path.join(COMPANY_DOCS_BASE_DIR,
                                                    row['relative_db_path'])} for row in rows]
    except Exception as e:
        print(f"DB Fehler list_company_documents (ID: {company_id}): {e}")
        return []
    finally:
        if conn:
            conn.close()
...
def delete_company_document(document_id: int) -> bool:
    ...
    actual_absolute_path_to_delete_on_disk = os.path.join(
        ...
        if os.path.exists(actual_absolute_path_to_delete_on_disk):
            try:
                ...
                parent_dir = os.path.dirname(
                    actual_absolute_path_to_delete_on_disk)
                if os.path.exists(parent_dir) and not os.listdir(parent_dir):
                    os.rmdir(parent_dir)
            except OSError as e_os:
                print(
                    f"DB Fehler Löschen Datei {actual_absolute_path_to_delete_on_disk}: {e_os}")
        conn.commit()
        return cursor.rowcount > 0
    except Exception as e:
        print(f"DB Fehler delete_company_document (ID: {document_id}): {e}")
        conn.rollback()
        return False
    finally:
        if conn:
...
def add_default_technique_for_company(company_id: int):
    ...
    try:
        ...
        default_technique = [
            ...
            (company_id,
             'Wechselrichter',
             'BT Serie GW5K-BT 5 kW Batteriewechselrichter',
             5000,
             1),
            (company_id,
             'Speicher',
             'Vitocharge VX3 A10 10,0 kWh Speicherturm',
             10,
             5)]
        ...
        conn.commit()
        print(
            f"Standardtechnik für Firma {company_id} erfolgreich hinzugefügt.")
        return True

    except sqlite3.Error as e:
        print(
            f"DB FEHLER: Fehler beim Hinzufügen der Standardtechnik für Firma {company_id}: {e}")
        conn.rollback()
        return False
...
def get_company_documents(company_id: int, doc_type: str |
                          None = None) -> list[dict[str, Any]]:
    """
