...
def main():
    ...
    for module_name, source_path in utils_modules.items():
        ...
        if source_path.exists():
            if not target_path.exists():
                ...
                print(f"  ℹ️  {module_name}.py bereits vorhanden")
        else:
    ...
    print(f"ℹ️  Vorhanden:   {len([m for m in utils_modules if (root / f'{m}.py').exists()]) - fixed} Module")
    print(f"❌ Nicht gefunden: {not_found} Module")
    print(f"⚠️  Deprecated:  {len(deprecated_modules)} Module (können ignoriert werden)")
