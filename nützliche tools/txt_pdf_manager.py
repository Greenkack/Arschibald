...
def get_page_files(page_num: int) -> dict[str, str]:
    ...
    for file_type in file_types:
        file_path = os.path.join(
            input_dir, f"seite_{page_num}_{file_type}.txt")
    ...
    with st.expander("📋 Seiten-Details", expanded=False):
        for page_num in pages[:10]:  # Zeige erste 10 Seiten
            ...
            with col1:
                ...
                txt_files = {
                    k: v for k,
                    v in files.items() if not k.startswith("bild_")}
            ...
            with col2:
                ...
                img_files = {
                    k: v for k,
                    v in files.items() if k.startswith("bild_")}
                if img_files:
...
def validate_page_structure(page_num: int) -> dict[str, bool]:
    ...
    if "details" in files:
        try:
            with open(files["details"], encoding='utf-8') as f:
                content = f.read()
...
if __name__ == "__main__":
    ...
    for page_num in pages[:5]:
        ...
        print(
            f"{status} Seite {page_num}: {sum(validation.values())}/{len(validation)} Checks")
