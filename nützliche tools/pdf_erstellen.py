...
anzahl_seiten = len([f for f in os.listdir(OUTPUT_DIR)
                    if f.endswith("_details.txt")])

for seite in range(1, anzahl_seiten + 1):
    detailfile = os.path.join(OUTPUT_DIR, f"seite_{seite}_details.txt")

    # Seitengröße laden
    with open(detailfile, encoding="utf-8") as df:
    ...
    with open(textfile, encoding="utf-8") as tf:
    ...
    bildpos_file = os.path.join(OUTPUT_DIR,
                                f"seite_{seite}_bilder_positionen.txt")
    if os.path.exists(bildpos_file):
        with open(bildpos_file, encoding="utf-8") as bf:
            for line in bf:
                parts = line.strip().split(":")
                bildname = f"seite_{seite}_bild_{
                    parts[0].split()[1].strip()}.png"
    ...
    if os.path.exists(formenfile):
        ...
        with open(formenfile, encoding="utf-8") as sf:
            for line in sf:
