    ...
    try:
        ...
        def _get_value(key: str) -> str | None:
            ...
            if key_lower == "customer_name":
                return (project_data or {}).get("customer_name") or (
                    project_data or {}).get("name") or "Max Mustermann"
            ...
            if key_lower == "customer_address_street":
                addr = (
                    project_data or {}).get("customer_address") or (
                    project_data or {}).get("address") or {}
                if isinstance(addr, dict):
                    street = addr.get("street", "")
                    number = addr.get("house_number") or addr.get(
                        "number") or ""
            ...
            if key_lower == "customer_address_city":
                addr = (
                    ...
                    project_data or {}).get("address") or {}
                if isinstance(addr, dict):
                    zip_code = addr.get("zip_code") or addr.get(
                        "postal_code") or ""
            ...
            if key_lower == "house_number":
                addr = (
                    ...
                    project_data or {}).get("address") or {}
                if isinstance(addr, dict):
                    return str(addr.get("house_number")
                               or addr.get("number") or "1")
                return "1"
            if key_lower == "zip_code":
                ...
                if isinstance(addr, dict):
                    return str(
                        addr.get("zip_code") or addr.get("postal_code") or "12345")
                return "12345"
            ...
            if key_lower == "company_email":
                return (project_data or {}).get(
                    "company_email") or "mail@tommatech.de"
            if key_lower == "company_phone":
                return (project_data or {}).get(
                    "company_phone") or "089244186540"
            if key_lower == "company_website":
                return (project_data or {}).get(
                    "company_website") or "www.tommatech.de"
            if key_lower == "company_address_street":
                return (project_data or {}).get(
                    "company_address_street") or "Maximilianstraße 35"
            if key_lower == "company_address_city":
                return (project_data or {}).get(
                    "company_address_city") or "80539 München"
            if key_lower == "company_tax_id":
                return (project_data or {}).get(
                    "company_tax_id") or "DE354973606"

            # === TECHNISCHE DATEN ===
            if key_lower == "anlage_kwp":
                value = (
                    analysis_results or {}).get("anlage_kwp") or (
                    project_data or {}).get("anlage_kwp")
            ...
            if key_lower == "battery_capacity_kwh":
                value = (
                    analysis_results or {}).get("battery_capacity_kwh") or (
                    project_data or {}).get("battery_capacity_kwh")
                if isinstance(value, (int, float)):
                    return f"{value:.1f}".replace(".", ",")
                return str(value) if value is not None else "6,1"
            if key_lower == "module_quantity":
                return str((analysis_results or {}).get("module_quantity") or (
                    project_data or {}).get("module_quantity") or "21")
            if key_lower == "roof_angle":
                return str(
                    (analysis_results or {}).get("roof_angle") or (
                        project_data or {}).get("roof_angle") or "30")
            if key_lower == "annual_consumption_kwh":
                value = (
                    analysis_results or {}).get("annual_consumption_kwh") or (
                    project_data or {}).get("annual_consumption_kwh")
                if isinstance(value, (int, float)):
                    return f"{value:,.0f}".replace(",", ".")
                return str(value) if value is not None else "6.000"
            if key_lower == "annual_pv_production_kwh":
                value = (analysis_results or {}).get("annual_pv_production_kwh") or (
                    project_data or {}).get("annual_pv_production_kwh")
                if isinstance(value, (int, float)):
            ...
            if key_lower == "final_cost":
                value = (
                    analysis_results or {}).get("final_cost") or (
                    project_data or {}).get("final_cost")
                if isinstance(value, (int, float)):
                    return f"{value:,.0f}".replace(",", ".")
            ...
            if key_lower == "total_savings_with_storage_eur":
                value = (analysis_results or {}).get(
                    "total_savings_with_storage_eur")
                if isinstance(value, (int, float)):
                    return f"{value:,.0f}".replace(",", ".")
            ...
            if key_lower == "total_savings_without_storage_eur":
                value = (analysis_results or {}).get(
                    "total_savings_without_storage_eur")
                if isinstance(value, (int, float)):
                    return f"{value:,.0f}".replace(",", ".")
            ...
            if key_lower == "independence_degree_percent":
                value = (
                    analysis_results or {}).get("independence_degree_percent") or (
                    analysis_results or {}).get("independence_degree")
                if isinstance(value, (int, float)):
                    return f"{value:.0f}%"
                return f"{value}%" if value is not None else "54%"
            if key_lower == "self_consumption_percent":
                value = (
                    analysis_results or {}).get("self_consumption_percent") or (
                    analysis_results or {}).get("self_consumption")
                if isinstance(value, (int, float)):
                    return f"{value:.0f}%"
            ...
            if key_lower == "project_id":
                return (project_data or {}).get("project_id") or (
                    project_data or {}).get("offer_number") or "AN2025-1454"
            ...
            if key_lower == "total_investment":
                return str((analysis_results or {}).get(
                    "total_investment") or "45.000")
            if key_lower == "annual_savings":
                return str((analysis_results or {}).get(
                    "annual_savings") or "2.400")
            if key_lower == "payback_period":
                return str((analysis_results or {}).get(
                    "payback_period") or "12 Jahre")
            if key_lower == "total_roi_25_years":
                return str((analysis_results or {}).get(
                    "total_roi_25_years") or "25%")
            if key_lower == "net_present_value":
                return str((analysis_results or {}).get(
                    "net_present_value") or "18.500")
            if key_lower == "monthly_savings":
                return str((analysis_results or {}).get(
                    "monthly_savings") or "200")
            if key_lower == "cumulative_savings":
                return str((analysis_results or {}).get(
                    "cumulative_savings") or "60.000")
            if key_lower == "financing_costs":
                return str((analysis_results or {}).get(
                    "financing_costs") or "2.500")
            if key_lower == "loan_details":
                return str((analysis_results or {}).get(
                    "loan_details") or "15 Jahre, 3,5% Zinsen")
...
def check_txt_system_requirements() -> dict[str, bool]:
    ...
    requirements = {
        ...
        'input_dir_exists': os.path.exists(
            os.path.join(
                base_dir,
                "input")),
    ...
    if requirements['input_dir_exists']:
        ...
        page_files = [f for f in txt_files if f.startswith(
            'seite_') and '_texte.txt' in f]
        if page_files:
            page_numbers = []
