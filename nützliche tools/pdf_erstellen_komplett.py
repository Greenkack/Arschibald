...
# enthält seite_*_*.txt und seite_*_bild_*.*
    ...
    with open(os.path.join(DATA_DIR, f"seite_{p}_details.txt"), encoding="utf-8") as f_det:
        for line in f_det:
            if line.startswith("Seitengröße:"):
                ...
                break
    if rect is None:
        # wenn Du hier lieber A4-Fallback möchtest, ersetze continue durch rect
        ...
        for block in open(txt_path, encoding="utf-8").read().split("-" * 40):
            ...
            for L in lines:
                if L.startswith("Text:"):
                    ...
                    d["bbox"] = ast.literal_eval(
                        L.split("Position:", 1)[1].strip())
