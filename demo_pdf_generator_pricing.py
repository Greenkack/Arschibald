...
def demo_basic_pdf_generator():
    ...
    print(f"Created PDF generator with {len(generator.pricing_keys)} pricing keys")

    # Show some generated keys
    if generator.pricing_keys:
        print("\nSample generated pricing keys:")
        for i, (key, value) in enumerate(list(generator.pricing_keys.items())[:5]):
            print(f"  {key}: {value}")
...
def demo_enhanced_pdf_generator():
    ...
    print(f"Created enhanced PDF generator with {len(enhanced_generator.pricing_keys)} pricing keys")
...
def demo_pricing_templates():
    ...
    print(f"Created PV template with {len(pv_template.placeholders)} placeholders")

    # Create heat pump template
    hp_template = template_manager.create_template('heatpump')
    print(f"Created heat pump template with {len(hp_template.placeholders)} placeholders")

    # Create combined template
    combined_template = template_manager.create_template('combined')
    print(f"Created combined template with {len(combined_template.placeholders)} placeholders")
...
def demo_dynamic_key_manager():
    ...
    component_category_keys = key_manager.get_keys_by_category(KeyCategory.COMPONENTS)
