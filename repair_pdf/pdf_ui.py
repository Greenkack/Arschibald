...
def render_pdf_ui(
    ...
    try:
        ...
        # Fallback, falls doch als String gespeichert
            ...
            try:
                ...
                if not isinstance(
                        pdf_presets, list):  # Sicherstellen, dass es eine Liste ist
                    st.warning(
    ...
    st.subheader(
        get_text_pdf_ui(
            texts,
            "pdf_template_management_header",
    ...
    with st.form(key="pdf_generation_form_v13_final_locked_options_main", clear_on_submit=False):
        ...
        with col_pdf_content1_form:
            ...
            append_after_main6_flag = bool(
                st.session_state.pdf_inclusion_options.get(
                    "append_additional_pages_after_main6", False))

            # Zusatzoptionen für Seiten ab 7 nur anzeigen, wenn der Schalter
            # aktiv ist
            if append_after_main6_flag:
                st.caption(
                    "Erweiterter Modus aktiv: Die 6 festen Hauptseiten werden erzeugt und zusätzliche Angebotsseiten ab Seite 7 angehängt.")
                ...
                st.info(
                    get_text_pdf_ui(
                        texts,
                        "pdf_only_main6_active_info",
                        "Standardmodus: Es werden nur die 6 Hauptseiten erzeugt. Erweiterte PDF-Ausgabe ist deaktiviert."))
        ...
        if submitted_generate_pdf:  # Werte aus temporären Keys in die Haupt-Session-State-Keys übernehmen
            append_after_main6_flag_submit = bool(
                st.session_state.pdf_inclusion_options.get(
                    "append_additional_pages_after_main6", False))
                            ...
                            if conn is None:
                                ...
                                if created_customer_id:
                                    ...
                                    proj_payload = {
                                        ...
                                        'project_name': proj_details.get('project_name') or f"PV Angebot {
...
def render_pdf_debug_section(
    ...
    with st.expander(" Debug: PDF-Anhänge-Prüfung", expanded=False):
        ...
        product_ids = [
            ('Modul',
             project_details.get('selected_module_id')),
            ('Wechselrichter',
             project_details.get('selected_inverter_id')),
            ('Batteriespeicher',
             project_details.get('selected_storage_id') if project_details.get('include_storage') else None)]
...
# 2025-06-08, Gemini Ultra: Datenstatus-Anzeige und Fallback-PDF-Option
# hinzugefügt.
