...
def calculate_depreciation(initial_value: float, useful_life_years: int,
    ...
    return {
        ...
        # Beispiel 30% Steuersatz
        "steuerersparnis_30_prozent": round(initial_value * 0.30, 2)
    }


def calculate_financing_comparison(investment: float,
                                   annual_interest_rate: float,
                                   duration_years: int,
                                   leasing_factor: float) -> dict[str,
                                                                  Any]:
    """
    ...
    credit_result = calculate_annuity(
        investment, annual_interest_rate, duration_years)

    # Leasingfinanzierung
    leasing_result = calculate_leasing_costs(
        investment, leasing_factor, duration_years * 12)

    # Cash-Kauf (Referenz)
    cash_opportunity_cost = investment * \
        (annual_interest_rate / 100) * duration_years
    ...
    return {
        ...
        "cash_kauf": {
            ...
            "gesamtkosten": round(
                ...
                cash_opportunity_cost,
                2)},
        "empfehlung": _get_financing_recommendation(
            credit_result,
            leasing_result,
            investment,
            cash_opportunity_cost)}
...
def _get_financing_recommendation(
        credit_result: dict,
        leasing_result: dict,
        cash_investment: float,
        opportunity_cost: float) -> str:
    ...
    return f"Empfehlung: {best_option} (Ersparnis: {
        round(
            max_cost -
            savings,
            2)}€)"
