...
# "apv1" steht für Admin Panel Version 1 (oder ein anderer eindeutiger Bezeichner)
WIDGET_KEY_SUFFIX = "_apv1"
...
def _dummy_parse_price_matrix_csv(
        csv_data: str | io.StringIO,
        errors_list: list[str]) -> pd.DataFrame | None:
    if errors_list is not None:
        errors_list.append("Dummy-Parser für Preis-Matrix CSV aktiv.")
    return None
...
_parse_price_matrix_excel_func: Callable[[
    bytes | None, list[str]], pd.DataFrame | None] | None = None
...
ADMIN_TAB_KEYS_DEFINITION_GLOBAL = [
    ...
    "admin_tab_pdf_title_images",
    "admin_tab_pdf_offer_titles",
    "admin_tab_pdf_cover_letters",
...
def parse_module_price_matrix_excel(
    ...
    if not excel_bytes:
        errors_list.append(
            "Preis-Matrix (Excel): Übergebene Excel-Daten sind leer.")
        return None
    try:
        excel_file_like = io.BytesIO(excel_bytes)
        df = pd.read_excel(excel_file_like, index_col=0, header=0)
        if df.empty:
            if df.index.name is None or str(
                    df.index.name).strip().lower() not in [
                    'anzahl module', 'anzahl_module']:
                errors_list.append(
                    "Preis-Matrix (Excel): Excel-Datei ist leer oder die erste Spalte (erwartet: 'Anzahl Module') fehlt oder ist leer.")
            else:
                errors_list.append(
                    "Preis-Matrix (Excel): Excel-Datei enthält keine Datenzeilen unterhalb des Headers und des Index.")
            return None
        ...
        if df.empty:
            errors_list.append(
                "Preis-Matrix (Excel): Index ('Anzahl Module') enthält keine gültigen Zahlen nach der Konvertierung oder alle Zeilen hatten ungültige Indexwerte.")
            return None
        ...
        if df.empty:
            errors_list.append(
                "Preis-Matrix (Excel): Nach der Verarbeitung (Bereinigung, Typkonvertierung, Entfernung leerer Zeilen/Spalten) ist die Matrix leer.")
            return None
        return df
    except ValueError as ve:
        errors_list.append(
            f"Preis-Matrix (Excel): Wert-Fehler beim Parsen der Excel-Datei: {ve}. Bitte Zahlenformate prüfen.")
        return None
    except Exception as e:
        errors_list.append(
            f"Preis-Matrix (Excel): Ein unbekannter Fehler ist beim Parsen der Excel-Datei aufgetreten: {e}")
        traceback.print_exc()
        return None
...
def render_product_management(
    ...
    st.subheader(
        get_text_local(
            "admin_product_management_header",
            "Produktverwaltung"))
    PRODUCT_DATASHEETS_BASE_DIR_ADMIN = os.path.join(
        os.getcwd(), "data", "product_datasheets")
    if not os.path.exists(PRODUCT_DATASHEETS_BASE_DIR_ADMIN):
        try:
            os.makedirs(PRODUCT_DATASHEETS_BASE_DIR_ADMIN)
        except OSError as e:
            st.error(
                f"Fehler Erstellen Verzeichnis '{PRODUCT_DATASHEETS_BASE_DIR_ADMIN}': {e}")
    ...
    with st.expander(get_text_local("admin_module_pdf_import_expander_title_pm", "Modul-PDFs importieren (Batch)"), expanded=False):
        st.caption(
            get_text_local(
                "admin_module_pdf_import_hint_pm",
                "Wähle eine oder mehrere Modul-PDFs. Alle erkannten Modulzeilen werden Produkten/Attributen zugeordnet."))
        sel_category_pm = st.selectbox(
            "Ziel-Kategorie (falls in Datei nicht angegeben)",
            options=[
                "Modul",
                "Wechselrichter",
                "Batteriespeicher",
                ...
                "Zubehör"],
            index=0,
            key=f"module_pdf_category_select_prod{WIDGET_KEY_SUFFIX}")
        uploaded_pdfs_pm = st.file_uploader(
            get_text_local(
                "admin_module_pdf_upload_label_pm",
                "Modul-PDF(s) auswählen"),
            type=["pdf"],
            accept_multiple_files=True,
            key=f"module_pdf_upload_prod{WIDGET_KEY_SUFFIX}")
        col_imp_pma, col_imp_pmb = st.columns([1, 2])
        start_import_pm = col_imp_pma.button(
            get_text_local(
                "admin_module_pdf_start_import_pm",
                "Import starten"),
            key=f"module_pdf_start_btn_prod{WIDGET_KEY_SUFFIX}")
        ...
        if start_import_pm:
            try:
                if not uploaded_pdfs_pm:
                    st.warning(
                        get_text_local(
                            "admin_module_pdf_no_files_pm",
                            "Keine PDF-Dateien ausgewählt."))
                else:
                    import time
                    from pathlib import Path
                    safe_dir_pm = os.path.join(
                        os.getcwd(), "data", "imports", "module_pdfs")
                    ...
                    try:
                        from tools.import_module_attributes_from_pdf import (
                            import_from_path as import_modules_batch_pm,
                        )
                        summary_pm = import_modules_batch_pm(
                            target_dir_pm, default_category=sel_category_pm)
                    except Exception as e_imp_pm:
                        st.error(
                            get_text_local(
                                "admin_module_pdf_import_error_pm",
                                f"Importer-Fehler: {e_imp_pm}"))
                        summary_pm = None

                    if summary_pm:
                        st.success(
                            get_text_local(
                                "admin_module_pdf_import_done_pm",
                        ...
                        if details_pm:
                            st.caption(
                                get_text_local(
                                    "admin_module_pdf_import_details_pm",
                                    "Details (erste 5):"))
                            st.json(details_pm[:max_show_pm])
                    else:
                        st.error(
                            get_text_local(
                                "admin_module_pdf_import_no_summary_pm",
                                "Kein Summary verfügbar."))
            except Exception as e_ui_imp_pm:
                st.error(
                    get_text_local(
                        "admin_module_pdf_import_exception_pm",
                        f"Fehler: {e_ui_imp_pm}"))
    ...
    with st.expander("Dateien importieren (PDF/CSV/XLSX/JSON/YAML/TXT/MD/JPG/PNG)", expanded=False):
        ...
        sel_category_any = st.selectbox(
            ...
            options=[
                ...
                "Zubehör"],
            index=0,
            key=f"generic_import_category_select_prod{WIDGET_KEY_SUFFIX}")
        generic_files_pm = st.file_uploader(
            "Dateien hochladen",
            accept_multiple_files=True,
            type=[
                "pdf",
                ...
                "jpeg",
                "png"],
            key=f"generic_import_upload_prod{WIDGET_KEY_SUFFIX}")
        run_generic_pm = st.button(
            "Import starten (alle Formate)",
            key=f"generic_import_start_prod{WIDGET_KEY_SUFFIX}")
        if run_generic_pm and generic_files_pm:
            ...
            for f in generic_files_pm:
                ...
                with open(out, 'wb') as w:
                    w.write(f.getbuffer())
            summary_any = import_any_from_path(
                str(base_dir), default_category=sel_category_any)
            ...
            with st.expander("Details anzeigen"):
                ...
                for d in details_any:
                    ...
                    if d.get("ok") and d.get("format") in ("csv", "xlsx"):
                        ...
                        if samples:
                            st.caption(
                                "Beispiel (normalisierte Felder, max. 5 Zeilen):")
                            st.table(samples)
                        # XLSX: je Sheet anzeigen
                        if d.get("format") == "xlsx" and isinstance(
                                d.get("sheets"), list):
                            ...
                            for s in d.get("sheets"):
                                st.markdown(
                                    f"- Blatt: {
                                        s.get('name')} | Zeilen: {
                                        s.get('total_rows')} | ensured: {
                                        s.get('ensured')} | updated: {
                                        s.get('updated')} | upserted: {
                                        s.get('upserted')}")
    ...
    st.subheader(
        get_text_local(
            "admin_product_db_upload_header",
            "Produktdatenbank hochladen/aktualisieren (Excel/CSV)"))
    ...
    uploaded_product_file_bulk = st.file_uploader(
        get_text_local(
            "admin_upload_product_file_label",
            "Produkt-Datei auswählen"),
        type=[
            "xlsx",
            "csv"],
    ...
    if uploaded_product_file_bulk is not None:
        if st.button(
                get_text_local(
                    "admin_process_product_file_button",
                    "Hochgeladene Produkt-Datei verarbeiten"),
                key=f"process_bulk_product_btn{WIDGET_KEY_SUFFIX}"):
            try:
                df_products_import = None
                if uploaded_product_file_bulk.name.endswith('.xlsx'):
                    df_products_import = pd.read_excel(
                        uploaded_product_file_bulk, engine='openpyxl')
                    ...
                    for enc in encodings_to_try:
                        try:
                            df_temp_auto = pd.read_csv(
                                io.BytesIO(csv_bytes_content),
                                sep=None,
                                decimal='.',
                                encoding=enc,
                                engine='python',
                            ...
                            if df_temp_auto is not None and not df_temp_auto.empty and len(
                                df_temp_auto.columns) > 1 and all(
                                isinstance(
                                    c,
                                    str) for c in df_temp_auto.columns):
                                st.info(
                                    f"CSV erfolgreich mit Kodierung '{enc}' und automatischem Trennzeichen gelesen.")
                                df_products_import = df_temp_auto
                                detected_encoding = enc
                                break
                            uploaded_product_file_bulk.seek(0)
                            df_temp_semicolon = pd.read_csv(
                                io.BytesIO(csv_bytes_content),
                                sep=';',
                                decimal=',',
                                encoding=enc,
                                ...
                                comment='#',
                                skipinitialspace=True)
                            if df_temp_semicolon is not None and not df_temp_semicolon.empty and len(
                                    df_temp_semicolon.columns) > 1:
                                st.info(
                                    f"CSV erfolgreich mit Kodierung '{enc}' und Trennzeichen ';' gelesen.")
                                df_products_import = df_temp_semicolon
                                detected_encoding = enc
                                break
                        except Exception:
                            df_products_import = None
                    ...
                    if not detected_encoding and df_products_import is None:
                        st.error(
                            "Konnte CSV-Datei mit gängigen Kodierungen/Trennzeichen nicht verarbeiten.")

                if df_products_import is not None:
                    df_products_import.columns = [
                        str(col).strip().lower().replace(
                            ' ', '_') for col in df_products_import.columns]
                    st.write(
                        "Vorschau der importierten Daten (erste 5 Zeilen):",
                        df_products_import.head())
                    ...
                    missing_required = [
                        col for col in required_cols_import if col not in df_products_import.columns]
                    if missing_required:
                        st.error(
                            f"Fehlende Pflichtspalten in der hochgeladenen Datei: {
                                ', '.join(missing_required)}. Import abgebrochen.")
                    else:
                        for index, row in df_products_import.iterrows():
                            ...
                            try:
                                ...
                                product_data_import_filtered = {db_col: product_data_import.get(
                                    db_col) for db_col in db_columns_from_product_db if db_col in product_data_import}
                                if not product_data_import_filtered.get(
                                        'model_name') or not product_data_import_filtered.get('category'):
                                    skipped_count += 1
                                    error_rows.append(
                                        {'row': index + 2, 'reason': "Modellname oder Kategorie fehlt in Zeile."})
                                    continue
                                for num_col in [
                                    'price_euro',
                                    'capacity_w',
                                    ...
                                    'max_cycles',
                                        'rating']:
                                    if num_col in product_data_import_filtered and product_data_import_filtered[
                                            num_col] is not None:
                                        try:
                                            val_str = str(
                                                product_data_import_filtered[num_col])
                                            if '.' in val_str and ',' in val_str:
                                                if val_str.rfind(
                                                    ...
                                                    val_str = val_str.replace(
                                                        '.', '')
                                            val_str = val_str.replace(',', '.')
                                            product_data_import_filtered[num_col] = float(
                                                val_str)
                                            if num_col in [
                                                'warranty_years',
                                                'max_cycles',
                                                    'rating'] and product_data_import_filtered[num_col] is not None:
                                                product_data_import_filtered[num_col] = int(
                                                    product_data_import_filtered[num_col])
                                        except (ValueError, TypeError):
                                            error_rows.append(
                                                {
                                                    'row': index + 2,
                                                    'model': product_data_import_filtered.get('model_name'),
                                                    'column': num_col,
                                                    'value': product_data_import_filtered[num_col],
                                                    'reason': 'Zahlenkonvertierung fehlgeschlagen'})
                                            product_data_import_filtered[num_col] = None
                                existing_product = get_product_by_model_name_func(
                                    str(product_data_import_filtered['model_name']))
                                if existing_product and 'id' in existing_product:
                                    if update_product_func(
                                            existing_product['id'], product_data_import_filtered):
                                        updated_count += 1
                                    else:
                                        skipped_count += 1
                                        error_rows.append(
                                            {
                                                'row': index + 2,
                                                'model': product_data_import_filtered.get('model_name'),
                                                'reason': 'Datenbank-Update fehlgeschlagen'})
                                else:
                                    if add_product_func(
                                            product_data_import_filtered):
                                        imported_count += 1
                                    else:
                                        skipped_count += 1
                                        error_rows.append(
                                            {
                                                'row': index + 2,
                                                'model': product_data_import_filtered.get('model_name'),
                                                'reason': 'Datenbank-Hinzufügen fehlgeschlagen (evtl. Duplikat oder anderer Fehler)'})
                            except Exception as e_row:
                                skipped_count += 1
                                current_model_name_for_error = product_data_import.get(
                                    'model_name', f"Zeile {index + 2}")
                                error_rows.append(
                                    {
                                        'row': index + 2,
                                        'model': current_model_name_for_error,
                                        'reason': f"Allgemeiner Fehler: {
                                            str(e_row)}"})
                        st.success(
                            f"Produktimport abgeschlossen: {imported_count} Produkte neu hinzugefügt, {updated_count} Produkte aktualisiert, {skipped_count} Produkte übersprungen/fehlerhaft.")
                        if error_rows:
                            st.warning(
                                "Details zu fehlerhaften/übersprungenen Zeilen (max. erste 10 Fehler):")
                            st.json(error_rows[:10])
                        st.session_state.selected_page_key_sui = "admin"
                        st.rerun()
            except Exception as e_bulk_import:
                st.error(
                    f"Fehler beim Verarbeiten der Produkt-Datei: {e_bulk_import}")
                traceback.print_exc()
    st.markdown("---")

    # Zusätzlicher, immer sichtbarer Attribut-Zugriff (unabhängig vom
    # Bearbeitungsmodus)
    with st.expander(get_text_local("product_attributes_quick_access_expander_title", "Erweiterte Eigenschaften / Attribute (Direkt-Zugriff)"), expanded=False):
        ...
        if not pa_list_attributes or not pa_upsert_attribute or not pa_delete_attribute:
            ...
            # Produktauswahl
            all_products_for_attr = list_products_func(
                None) if callable(list_products_func) else []
            if not all_products_for_attr:
                st.info(
                    get_text_local(
                        "product_attributes_no_products_info",
                        "Keine Produkte vorhanden. Lege zuerst ein Produkt an."))
            else:
                ...
                # Kategorien-Filter
                try:
                    category_options = list_product_categories_func() if callable(
                        list_product_categories_func) else []
                except Exception:
                    category_options = []
                if not category_options:
                    # Fallback aus vorhandenen Produkten ableiten
                    category_options = sorted({(p.get('category') or '').strip(
                    ) for p in all_products_for_attr if p.get('category')})
                all_label = get_text_local(
                    "product_attributes_filter_category_all",
                    "Alle Kategorien")
                ...
                search_text = st.text_input(
                    get_text_local(
                        "product_attributes_filter_search_label",
                        "Suche (Modell/Hersteller)"),
                    key=f"attr_quick_access_search{WIDGET_KEY_SUFFIX}")

                def _passes_filters(p: dict[str, Any]) -> bool:
                    ok_cat = (
                        selected_category == all_label) or (
                        (p.get('category') or '') == selected_category)
                    if not ok_cat:
                        return False
                    ...
                    if not q:
                        return True
                    return q in (
                        p.get('model_name') or '').lower() or q in (
                        p.get('brand') or '').lower()

                filtered_products = [
                    p for p in all_products_for_attr if _passes_filters(p)]
                options_labels = [
                    _label_for_prod(p) for p in filtered_products]
                options_map = {
                    _label_for_prod(p): int(
                        p.get('id')) for p in filtered_products if p.get('id') is not None}
                ...
                if selected_label and selected_label in options_map:
                    ...
                    try:
                        selected_prod = get_product_by_id_func(
                            selected_pid) if callable(get_product_by_id_func) else None
                    except Exception:
                        selected_prod = None
                    current_category_attr = (
                        selected_prod.get('category') if isinstance(
                            selected_prod, dict) else '') or ''

                    st.caption(
                        get_text_local(
                            "product_attributes_selected_product_caption",
                            "Ausgewähltes Produkt für Attribute"))
                    ...
                    if not attrs_list_q:
                        ...
                        for a in attrs_list_q:
                            ...
                            cols_aq[3].caption(
                                str(a.get('display_order') or 0))
                            del_key_q = f"del_attr_quick_{selected_pid}_{
                                a.get('id')}{WIDGET_KEY_SUFFIX}"
                            if cols_aq[4].button(
                                "", key=del_key_q, help=get_text_local(
                    ...
                    with st.form(key=form_key_attr_q, clear_on_submit=True):
                        a_key_q = st.text_input(
                            get_text_local(
                                "product_attributes_key_label",
                                "Key (z. B. cell_technology)"),
                            key=f"{form_key_attr_q}_key")
                        a_val_q = st.text_input(
                            get_text_local(
                                "product_attributes_value_label",
                                "Wert"),
                            key=f"{form_key_attr_q}_value")
                        cols_uoq = st.columns([1, 1])
                        a_unit_q = cols_uoq[0].text_input(
                            get_text_local(
                                "product_attributes_unit_label",
                                "Einheit (optional)"),
                            key=f"{form_key_attr_q}_unit")
                        try:
                            a_order_default_q = 0
                        except Exception:
                            a_order_default_q = 0
                        a_order_q = cols_uoq[1].number_input(
                            get_text_local(
                                "product_attributes_order_label",
                                "Reihenfolge"),
                            min_value=0,
                            value=a_order_default_q,
                            step=1,
                            key=f"{form_key_attr_q}_order")
                        submit_attr_q = st.form_submit_button(get_text_local(
                    ...
                    if submit_attr_q:
                        if not a_key_q or not a_key_q.strip():
                            ...
                            try:
                                saved_id_q = pa_upsert_attribute(
                                    selected_pid, current_category_attr, a_key_q.strip(), a_val_q.strip() if isinstance(
                                        a_val_q, str) else a_val_q, a_unit_q.strip() if isinstance(
                                        a_unit_q, str) else a_unit_q, int(a_order_q))
                                if saved_id_q:
                                    st.success(
                                        get_text_local(
    ...
    with st.expander(form_manual_title, expanded=(st.session_state.product_to_edit_id_manual is not None or not list_products_func(None))):
        ...
        with st.form(key=form_key_manual_prod_ui, clear_on_submit=False):
            ...
            p_warranty_form = st.number_input(
                label=get_text_local(
                    "product_warranty_years_label", "Garantie (Jahre)"), min_value=0, value=int(
                    product_data_for_manual_form.get(
                        'warranty_years', 0)), step=1, key=f"{form_key_manual_prod_ui}_warranty_man")
            ...
            if p_category_form == 'Modul':
                p_capacity_w_val = st.number_input(
                    label=get_text_local(
                        "module_capacity_w_label", "Leistung (Wp)"), min_value=0.0, value=float(
                        product_data_for_manual_form.get(
                ...
                p_weight_kg_val = st.number_input(
                    label=get_text_local(
                        "module_weight_kg_label", "Gewicht (kg)"), min_value=0.0, value=float(
                        product_data_for_manual_form.get(
                ...
                p_power_kw_val = st.number_input(
                    label=get_text_local(
                        "inverter_power_kw_label", "Nennleistung AC (kW)"), min_value=0.0, value=float(
                        product_data_for_manual_form.get(
                            'power_kw', 0.0)), step=0.1, key=f"{form_key_manual_prod_ui}_power_kw_inv_man")
                p_efficiency_percent_val = st.number_input(
                    label=get_text_local(
        ...
        if submitted_manual_product_form_btn:
            if not p_model_name_form.strip() or not p_category_form:
                ...
                product_data_to_save_db = {
                    ...
                    "image_base64": current_product_image_b64_form,
                ...
                if uploaded_product_image_manual_file_form:
                    if uploaded_product_image_manual_file_form.size <= 2 * 1024 * 1024:
                        product_data_to_save_db["image_base64"] = base64.b64encode(
    ...
    with st.form(f"global_constants_form{WIDGET_KEY_SUFFIX}"):
        ...
        st.subheader(
            get_text_local(
                ...
                "Ertragseinstellungen für PV-Anlage"))
...
def render_price_matrix(
        load_admin_setting_func: Callable,
        save_admin_setting_func: Callable,
        parse_csv_func_from_calculations: Callable,
        parse_excel_func_local_admin: Callable):
    ...
    st.subheader(
        get_text_local(
            "admin_price_matrix_header",
            "Preis-Matrix Verwaltung"))
    st.info(
        get_text_local(
            "admin_price_matrix_info_detailed_xlsx",
            "Laden Sie hier die Grundpreis-Matrix als Excel-Datei (.xlsx) hoch..."))
    uploader_key_excel = f"price_matrix_upload_widget_xlsx{WIDGET_KEY_SUFFIX}"
    uploaded_file_excel = st.file_uploader(
        get_text_local(
            "admin_upload_xlsx_label",
            "Excel-Datei (.xlsx) hochladen"),
        type=["xlsx"],
        key=uploader_key_excel)
    if uploaded_file_excel is not None:
        try:
            excel_bytes_content = uploaded_file_excel.getvalue()
            parser_errors_matrix_upload_excel: list[str] = []
            parsed_df_temp_excel = parse_excel_func_local_admin(
                excel_bytes_content, parser_errors_matrix_upload_excel)
            if parser_errors_matrix_upload_excel:
                for err_matrix in parser_errors_matrix_upload_excel:
                    st.warning(f"Parser-Warnung (Excel): {err_matrix}")
            if parsed_df_temp_excel is not None and not parsed_df_temp_excel.empty:
                ...
                st.session_state.parsed_excel_df_for_preview_admin = parsed_df_temp_excel
                st.success(
                    get_text_local(
                        "admin_xlsx_parse_success",
                        "Excel geparst."))
            else:
                st.error(
                    get_text_local(
                        "admin_xlsx_parse_error_empty",
                        "Excel geparst, aber leer."))
                st.session_state.uploaded_excel_bytes_for_save_admin = None
                st.session_state.parsed_excel_df_for_preview_admin = None
        except Exception as e_upload_process_excel:
            st.error(f"Fehler Verarbeitung Excel: {e_upload_process_excel}")
            traceback.print_exc()
            st.session_state.uploaded_excel_bytes_for_save_admin = None
            st.session_state.parsed_excel_df_for_preview_admin = None
    if st.session_state.get('parsed_excel_df_for_preview_admin') is not None:
        ...
        st.dataframe(
            st.session_state.parsed_excel_df_for_preview_admin,
            use_container_width=True)
        if st.button(
                get_text_local(
                    "admin_save_price_matrix_button_xlsx",
                    "...Excel...speichern"),
                key=f"save_uploaded_price_matrix_xlsx_btn{WIDGET_KEY_SUFFIX}"):
            if st.session_state.uploaded_excel_bytes_for_save_admin:
                if save_admin_setting_func(
                    'price_matrix_excel_bytes',
                        st.session_state.uploaded_excel_bytes_for_save_admin):
    ...
    st.subheader(
        get_text_local(
            "admin_current_saved_price_matrix_header_xlsx",
            "Gespeicherte Preis-Matrix (Excel):"))
    current_price_matrix_excel_bytes_from_db = load_admin_setting_func(
        'price_matrix_excel_bytes', None)
    if current_price_matrix_excel_bytes_from_db and isinstance(
            current_price_matrix_excel_bytes_from_db, bytes):
        display_errors_stored_matrix_pm_excel: list[str] = []
        parsed_stored_df_pm_excel = parse_excel_func_local_admin(
            current_price_matrix_excel_bytes_from_db,
            display_errors_stored_matrix_pm_excel)
        if display_errors_stored_matrix_pm_excel:
            for err_pm_stored_excel in display_errors_stored_matrix_pm_excel:
                st.warning(
                    f"Hinweis Laden Excel-Matrix: {err_pm_stored_excel}")
        if parsed_stored_df_pm_excel is not None and not parsed_stored_df_pm_excel.empty:
            ...
            if st.button(
                    get_text_local(
                        "admin_delete_saved_price_matrix_button_xlsx",
                        "Excel Preis-Matrix löschen"),
                    key=f"delete_price_matrix_excel_final{WIDGET_KEY_SUFFIX}"):
                if save_admin_setting_func('price_matrix_excel_bytes', None):
                    st.success("Excel Preis-Matrix gelöscht.")
                    st.session_state.selected_page_key_sui = "admin"
                    st.rerun()
                else:
                    st.error("Fehler Löschen Excel Preis-Matrix.")
        else:
            st.warning("Gespeicherte Excel Preis-Matrix ungültig.")
    else:
        st.info("Keine Excel Preis-Matrix gespeichert.")
    st.markdown("---")
    st.info(get_text_local("admin_price_matrix_info_detailed_csv",
                           "Laden Sie hier die Grundpreis-Matrix als CSV-Datei hoch..."))
    uploader_key_csv = f"price_matrix_upload_widget_csv{WIDGET_KEY_SUFFIX}"
    uploaded_file_csv = st.file_uploader(
        get_text_local(
            "admin_upload_csv_label",
            "CSV-Datei hochladen"),
        type=["csv"],
        key=uploader_key_csv)
    if uploaded_file_csv is not None:
        try:
            ...
            if csv_content_temp is not None:
                parser_errors_matrix_upload_csv: list[str] = []
                parsed_df_temp_csv = parse_csv_func_from_calculations(
                    io.StringIO(csv_content_temp), parser_errors_matrix_upload_csv)
                if parser_errors_matrix_upload_csv:
                    for err_matrix_csv in parser_errors_matrix_upload_csv:
                        st.warning(f"Parser-Warnung (CSV): {err_matrix_csv}")
                if parsed_df_temp_csv is not None and not parsed_df_temp_csv.empty:
                    ...
                    st.session_state.parsed_csv_df_for_preview_admin = parsed_df_temp_csv
                    st.success(
                        get_text_local(
                            "admin_csv_parse_success",
                            "CSV geparst."))
                else:
                    st.error(
                        get_text_local(
                            "admin_csv_parse_error_empty",
                            "CSV geparst, aber leer."))
                    st.session_state.uploaded_csv_content_for_save_admin = None
                    st.session_state.parsed_csv_df_for_preview_admin = None
        except Exception as e_upload_process_csv:
            st.error(f"Fehler Verarbeitung CSV: {e_upload_process_csv}")
            traceback.print_exc()
            st.session_state.uploaded_csv_content_for_save_admin = None
            st.session_state.parsed_csv_df_for_preview_admin = None
    if st.session_state.get('parsed_csv_df_for_preview_admin') is not None:
        ...
        st.dataframe(
            st.session_state.parsed_csv_df_for_preview_admin,
            use_container_width=True)
        if st.button(
                get_text_local(
                    "admin_save_price_matrix_button_csv",
                    "...CSV...speichern"),
                key=f"save_uploaded_price_matrix_csv_btn{WIDGET_KEY_SUFFIX}"):
            if st.session_state.uploaded_csv_content_for_save_admin:
                if save_admin_setting_func(
                    'price_matrix_csv_data',
                        st.session_state.uploaded_csv_content_for_save_admin):
    ...
    st.subheader(
        get_text_local(
            "admin_current_saved_price_matrix_header_csv",
            "Gespeicherte Preis-Matrix (CSV):"))
    current_price_matrix_csv_from_db = load_admin_setting_func(
        'price_matrix_csv_data', "")
    if current_price_matrix_csv_from_db and isinstance(
            current_price_matrix_csv_from_db,
            str) and current_price_matrix_csv_from_db.strip():
        display_errors_stored_matrix_pm_csv: list[str] = []
        parsed_stored_df_pm_csv = parse_csv_func_from_calculations(
            io.StringIO(current_price_matrix_csv_from_db),
            display_errors_stored_matrix_pm_csv)
        if display_errors_stored_matrix_pm_csv:
            for err_pm_stored_csv in display_errors_stored_matrix_pm_csv:
                st.warning(f"Hinweis Laden CSV-Matrix: {err_pm_stored_csv}")
        if parsed_stored_df_pm_csv is not None and not parsed_stored_df_pm_csv.empty:
            ...
            if st.button(
                    get_text_local(
                        "admin_delete_saved_price_matrix_button_csv",
                        "...CSV...löschen"),
                    key=f"delete_price_matrix_csv_final{WIDGET_KEY_SUFFIX}"):
                if save_admin_setting_func('price_matrix_csv_data', None):
                    st.success("CSV Preis-Matrix gelöscht.")
                    st.session_state.selected_page_key_sui = "admin"
                    st.rerun()
                else:
                    st.error("Fehler Löschen CSV Preis-Matrix.")
        else:
            st.warning("Gespeicherte CSV Preis-Matrix ungültig.")
            st.text_area(
                "Rohdaten CSV (max. 500 Z.)",
                value=current_price_matrix_csv_from_db[
                    :500] +
                "..." if len(current_price_matrix_csv_from_db) > 500 else current_price_matrix_csv_from_db,
                height=100,
                disabled=True,
                key=f"raw_db_csv_preview{WIDGET_KEY_SUFFIX}")
    else:
...
def render_visualization_settings(
    ...
    st.subheader(
        get_text_local(
            "admin_visualization_settings_header",
            "Globale Visualisierungs-Einstellungen"))
    st.info(
        get_text_local(
            "admin_visualization_settings_info",
            "Passen Sie hier die Standardfarben... an."))
    ...
    with st.form(f"visualization_settings_form{WIDGET_KEY_SUFFIX}"):
        st.markdown(
            f"**{get_text_local('admin_viz_general_colors_header', 'Allgemeine Diagrammfarben')}**")
        selected_palette_val = viz_settings.get(
            "default_color_palette",
            default_viz_settings_fallback.get("default_color_palette"))
        selected_palette = st.selectbox(
            get_text_local(
                "admin_viz_default_palette_label",
            ...
            help=get_text_local(
                "admin_viz_default_palette_help",
                "..."))
        primary_color = st.color_picker(
            get_text_local(
                "admin_viz_primary_color_label",
        ...
        secondary_color = st.color_picker(
            get_text_local(
                "admin_viz_secondary_color_label",
        ...
        st.markdown(
            f"---<br>**{
                get_text_local(
                    'admin_viz_font_settings_header',
                    'Schriftart-Einstellungen')}**")
        font_family = st.text_input(
            get_text_local(
                "admin_viz_font_family_label",
                "Schriftfamilie..."),
            value=viz_settings.get(
                "chart_font_family",
        ...
        font_size_title = c1f.number_input(
            get_text_local(
                "admin_viz_font_size_title_label",
                "Schriftgröße Titel"),
            min_value=8,
            max_value=30,
            value=int(
        ...
        font_size_axis = c2f.number_input(
            get_text_local(
                "admin_viz_font_size_axis_label",
                "Schriftgröße Achsenbeschr."),
            min_value=6,
            max_value=24,
            value=int(
                viz_settings.get(
        ...
        font_size_tick = c3f.number_input(
            get_text_local(
                "admin_viz_font_size_tick_label",
        ...
        submitted_viz_settings = st.form_submit_button(
            get_text_local(
                "admin_save_visualization_settings_button",
                "Visualisierungs-Einstellungen Speichern"))
    if submitted_viz_settings:
        new_viz_settings = {"default_color_palette": selected_palette,
                            ...
                            **{k: v for k,
                               v in viz_settings.items() if isinstance(v,
                                                                       dict)}}
        ...
        if save_admin_setting_func(
            ...
            st.success(
                get_text_local(
                    "admin_visualization_settings_saved_success",
                    "Visualisierungs-Einstellungen gespeichert."))
            st.session_state.selected_page_key_sui = "admin"
            st.rerun()
        else:
            st.error(
                get_text_local(
                    "admin_visualization_settings_save_error",
...
def render_pdf_design_settings(
    ...
    if _get_company_by_id_safe and callable(
        ...
        if active_company_data:
            ...
            company_logo_b64_for_pdf = active_company_data.get('logo_base64')
...
def render_admin_panel(
    ...
    save_admin_setting_func: Callable[[str, Any], bool],
    load_admin_setting_func: Callable[[str, Any], Any],
    parse_price_matrix_csv_func: Callable[[str | io.StringIO, list[str]], pd.DataFrame | None],
    list_products_func: Callable[[str | None], list[dict[str, Any]]],
    add_product_func: Callable[[dict[str, Any]], int | None],
    update_product_func: Callable[[int | float, dict[str, Any]], bool],
    delete_product_func: Callable[[int | float], bool],
    ...
    **kwargs: Any
    ...
    if passed_excel_parser and callable(passed_excel_parser):
        _parse_price_matrix_excel_func = passed_excel_parser
    elif callable(parse_module_price_matrix_excel):
        _parse_price_matrix_excel_func = parse_module_price_matrix_excel
    else:
        def _dummy_parse_excel_admin_final(excel_bytes, errors_list_local):
            if errors_list_local is not None:
                errors_list_local.append(
                    "Admin: Dummy Excel-Parser (final) aktiv.")
            return
    ...
    tab_functions_map = {
        ...
        "admin_tab_general_settings": lambda: render_general_settings_extended(
            load_admin_setting_func,
            save_admin_setting_func),
        "admin_tab_price_matrix": lambda: render_price_matrix(
            load_admin_setting_func,
            save_admin_setting_func,
            _parse_price_matrix_csv_safe,
            _parse_price_matrix_excel_func),
    ...
    for i, tab_key_loop in enumerate(admin_tab_keys_definition):
        if i < len(admin_tabs_rendered_ui) and i < len(
                admin_tab_labels_definition):
                ...
                if tab_key_loop in tab_functions_map:
                    ...
                    if callable(render_func):
                        try:
                            ...
                            st.error(
                                f"Fehler beim Rendern des Tabs '{
                                    admin_tab_labels_definition[i]}': {e_tab_render_loop}")
                            st.text(traceback.format_exc())
                    else:
                        st.warning(
                            f"Render-Funktion für Tab '{admin_tab_labels_definition[i]}' ist nicht aufrufbar.")
                else:
                    st.warning(
                        f"Tab-Funktion für '{admin_tab_labels_definition[i]}' nicht implementiert.")
        # Fallback, falls admin_tabs_rendered_ui kürzer ist
        elif i < len(admin_tab_labels_definition):
            st.error(
                f"Fehler: Konnte Tab-Container für '{admin_tab_labels_definition[i]}' nicht erstellen.")
        else:  # Sollte nicht passieren, wenn Längenprüfungen oben greifen
            st.error(
                f"Fehler: Inkonsistenz in Tab-Definitionen bei Index {i}.")
...
# 2025-06-21, Gemini Ultra: Firmenspezifische Angebotsvorlagen
# implementiert - Textvorlagen und Bildvorlagen für Firmen hinzugefügt.
