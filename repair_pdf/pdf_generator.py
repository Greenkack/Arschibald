...
class PDFGenerator:
    ...
    def _draw_offer_table(self):
        ...
        data.append(
            ["", "", "", "Netto:", f"{self.offer_data.get('net_total', 0):.2f} €"])
        data.append(["", "", "", "MwSt. 19%:",
                    f"{self.offer_data.get('vat', 0):.2f} €"])
        data.append(["", "", "", Paragraph("<b>Gesamt</b>", self.styles['Body']),
                    f"{self.offer_data.get('grand_total', 0):.2f} €"])
...
def generate_main_template_pdf_bytes(
    ...
    debug_templates = os.environ.get(
        "DING_TEMPLATE_DEBUG", "0").lower() in {
        "1", "true", "yes", "on"}
    if debug_templates:
    ...
    try:
        ...
        overlay_bytes = generate_overlay(
            coords_dir, dyn_data, total_pages=total_pages)
        if debug_templates:
...
def generate_offer_pdf_with_main_templates(
    ...
    **kwargs,
    ...
    """
    # Erzeuge optional die Zusatz-PDF zuerst, damit die 6-Seiten-Nummerierung
    # "von XX" korrekt ist
    additional_pdf = None
    try:
        append_after_main6 = bool(
    ...
    main6 = generate_main_template_pdf_bytes(
        project_data,
        analysis_results,
        company_info,
    ...
    try:
        ...
        if additional_pdf:
            ...
            logo_b64 = None
            try:
                logo_b64 = company_logo_base64 or (
                    company_info.get('logo_base64') if isinstance(
                        company_info, dict) else None)
            except Exception:
...
# Der coords-Ordner enthält seite1.yml … seite6.yml mit den
# Positionsinformationen.
COORDS_DIR = os.path.join(_PDF_GENERATOR_BASE_DIR, "coords")

# Der Ordner pdf_templates_static/notext enthält die sechs PDF-Vorlagen
# nt-nt-01 … nt-nt-06.
BG_DIR = os.path.join(
    _PDF_GENERATOR_BASE_DIR,
    ...
    **kwargs,
    ...
    try:
        return generate_offer_pdf_with_main_templates(
            ...
            company_logo_base64=(
                company_info or {}).get('logo_base64'),
            ...
            save_admin_setting_func=kwargs.get(
                'save_admin_setting_func',
                _noop),
            list_products_func=kwargs.get(
                'list_products_func',
                    _noop),
            get_product_by_id_func=kwargs.get(
                'get_product_by_id_func',
                _noop),
...
if _REPORTLAB_AVAILABLE:
    ...
    STYLES.add(
        ParagraphStyle(
            ...
            fontSize=8,
            textColor=colors.darkgrey))  # Dunkleres Grau für Footer
    STYLES.add(
        ParagraphStyle(
            name='OfferTitle',
            parent=STYLES['h1'],
            ...
            leading=28))  # Größer und eleganter
    STYLES.add(
        ParagraphStyle(
            name='SectionTitle',
            parent=STYLES['h2'],
    ...
    STYLES.add(
        ParagraphStyle(
            ...
            leading=16))  # Sekundärfarbe für Abwechslung
    STYLES.add(
        ParagraphStyle(
            name='ComponentTitle',
    ...
    STYLES.add(
        ParagraphStyle(
            ...
            leftIndent=0,
            rightIndent=0,
            textColor=colors.HexColor(TEXT_COLOR_HEX)))  # Etwas mehr Zeilenabstand    # Neuer Stil für rechtsbündige Kundenadresse auf Deckblatt
    STYLES.add(
        ParagraphStyle(
    ...
    STYLES.add(
        ParagraphStyle(
            ...
            spaceBefore=0.5 * cm,
            spaceAfter=0.8 * cm,
            textColor=colors.HexColor(TEXT_COLOR_HEX)))  # Für Anschreiben etc.
    ...
    STYLES.add(
        ParagraphStyle(
            ...
            fontSize=9,
            leading=11,
            textColor=colors.HexColor(TEXT_COLOR_HEX)))  # Helvetica für Tabellentext
    STYLES.add(
        ParagraphStyle(
            name='TableTextSmall',
            parent=STYLES['NormalLeft'],
    ...
    def create_zebra_table_style(
        ...
        for row in range(1, num_rows):
            if row % 2 == 0:  # Jede zweite Zeile
                style.append(
                    ('BACKGROUND', (0, row), (-1, row), colors.HexColor(alt_bg)))
...
def page_layout_handler(canvas_obj: canvas.Canvas,
    ...
    print(
        f"DEBUG: page_layout_handler aufgerufen für Seite {
            canvas_obj.getPageNumber()}")  # DEBUG
...
def _prepare_cost_table_for_pdf(
    ...
    for result_key, label_key, is_euro_val, base_style_name in cost_items_ordered_pdf:
        ...
        if value_cost is not None:
            if value_cost == 0.0 and result_key not in [
            ...
            label_text_pdf = get_text(
                texts, label_key, label_key.replace(
                    "_", " ").title())
...
def _prepare_simulation_table_for_pdf(
    ...
    header_config_pdf = [
        ...
        (get_text(
            texts,
            "annual_maintenance_cost_sim",
            "Wartung"),
         'annual_maintenance_costs_sim',
         "€",
         2,
         'TableNumber'),
        (get_text(
            texts,
            "analysis_table_annual_cf_header",
            "Jährl. CF"),
         'annual_cash_flows_sim',
         "€",
         2,
         'TableNumber'),
        (get_text(
            texts,
            "analysis_table_cumulative_cf_header",
            "Kum. CF"),
         'cumulative_cash_flows_sim_display',
         "€",
         2,
         'TableBoldRight')]
...
def _create_product_table_with_image(
    ...
    if details_data_prod and product_image_flowables_prod:
        ...
        text_table = Table(
            details_data_prod,
            colWidths=[
                text_table_width * 0.4,
                text_table_width * 0.6])
...
def generate_offer_pdf(
    ...
    if not skip_cover_and_letter:
        try:
            ...
            story.append(Paragraph(f"{get_text(texts,
    ...
    ordered_section_definitions_pdf = [
        ("ProjectOverview",
         "pdf_section_title_overview",
         "1. Projektübersicht & Eckdaten"),
        ("TechnicalComponents",
         "pdf_section_title_components",
            ...
            try:
                ...
                if section_key_current == "ProjectOverview":
                    ...
                    overview_data_content_pdf = [
                        [
                            get_text(
                                texts, "anlage_size_label_pdf", "Anlagengröße"), format_kpi_value(
                                current_analysis_results_pdf.get('anlage_kwp'), "kWp", texts_dict=texts, na_text_key="value_not_available_short_pdf")], [
                            get_text(
                                texts, "module_quantity_label_pdf", "Anzahl Module"), str(
                                pv_details_pdf.get(
                                    'module_quantity', get_text(
                                        texts, "value_not_available_short_pdf")))], [
                            get_text(
                                texts, "annual_pv_production_kwh_pdf", "Jährliche PV-Produktion (ca.)"), format_kpi_value(
                                current_analysis_results_pdf.get('annual_pv_production_kwh'), "kWh", precision=0, texts_dict=texts, na_text_key="value_not_available_short_pdf")], [
                            get_text(
                                texts, "self_supply_rate_percent_pdf", "Autarkiegrad (ca.)"), format_kpi_value(
                                current_analysis_results_pdf.get('self_supply_rate_percent'), "%", precision=1, texts_dict=texts, na_text_key="value_not_available_short_pdf")], ]
                    ...
                    if pv_details_pdf.get("include_storage"):
                        main_components.append(
                            (pv_details_pdf.get("selected_storage_id"), get_text(
                                texts, "pdf_component_storage_title", "Batteriespeicher")))
    ...
    if custom_content_items and len(custom_content_items) > 0:
        ...
        top_items = [
            item for item in custom_content_items if item.get('position') == 'top']
        middle_items = [
            item for item in custom_content_items if item.get('position') == 'middle']
        bottom_items = [
            item for item in custom_content_items if item.get('position') == 'bottom']
    ...
    if pv_details_pdf.get(
        'include_additional_components',
            True):  # Nur wenn Zubehör überhaupt aktiv ist
        for opt_id_key in [
            'selected_wallbox_id',
            'selected_ems_id',
    ...
    # Duplikate entfernen, falls ein Produkt mehrfach auftaucht
    # (unwahrscheinlich, aber sicher)
                ...
                if datasheet_path_from_db:
                    ...
                    if os.path.exists(full_datasheet_path):
                        ...
                        debug_info['product_datasheets_found'].append(
                            {'id': prod_id, 'model': product_info.get('model_name'), 'path': full_datasheet_path})
                    else:
                        debug_info['product_datasheets_missing'].append(
                            {'id': prod_id, 'model': product_info.get('model_name'), 'path': full_datasheet_path})
                else:
                    debug_info['product_datasheets_missing'].append({'id': prod_id, 'model': product_info.get(
                        'model_name'), 'reason': 'Kein Datenblatt-Pfad in DB'})
            else:
                debug_info['product_datasheets_missing'].append(
                    {'id': prod_id, 'reason': 'Produkt nicht in DB gefunden'})
        except Exception:
            # Fehler beim Laden von Produktinformationen - wird still behandelt
            pass

    # Firmendokumente
    if company_document_ids_to_include_opt and active_company_id is not None and callable(
            db_list_company_documents_func):
        try:
            all_company_docs_for_active_co = db_list_company_documents_func(
                active_company_id, None)  # doc_type=None für alle
                ...
                if doc_info.get('id') in company_document_ids_to_include_opt:
                    ...
                    if relative_doc_path:
                        ...
                        if os.path.exists(full_doc_path):
                            ...
                            debug_info['company_docs_found'].append({'id': doc_info.get(
                                'id'), 'name': doc_info.get('display_name'), 'path': full_doc_path})
                        else:
                            debug_info['company_docs_missing'].append({'id': doc_info.get(
                                'id'), 'name': doc_info.get('display_name'), 'path': full_doc_path})
                    else:
                        debug_info['company_docs_missing'].append({'id': doc_info.get(
                            'id'), 'name': doc_info.get('display_name'), 'reason': 'Kein relativer Pfad in DB'})
        except Exception:
