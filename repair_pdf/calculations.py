...
def load_price_matrix_df_with_cache(
    price_matrix_excel_bytes: bytes | None,
    price_matrix_csv_content: str | None,
    errors_list: list[str],
) -> tuple[pd.DataFrame | None, str]:
    global _PRICE_MATRIX_CACHE

    excel_hash = _hash_bytes(
        price_matrix_excel_bytes) if price_matrix_excel_bytes else None
    csv_hash = _hash_text(
        price_matrix_csv_content) if price_matrix_csv_content else None

    # Wenn Excel-Daten vorhanden sind, bevorzugen wir diese; sonst CSV
    if excel_hash:
        if _PRICE_MATRIX_CACHE.get(
                "excel_hash") == excel_hash and _PRICE_MATRIX_CACHE.get("df") is not None:
            return _PRICE_MATRIX_CACHE.get(
                "df"), _PRICE_MATRIX_CACHE.get("source", "Excel")
        df_excel = parse_module_price_matrix_excel(
            price_matrix_excel_bytes, errors_list)
    ...
    if csv_hash:
        if _PRICE_MATRIX_CACHE.get(
                "csv_hash") == csv_hash and _PRICE_MATRIX_CACHE.get("df") is not None:
            return _PRICE_MATRIX_CACHE.get(
                "df"), _PRICE_MATRIX_CACHE.get("source", "CSV")
        df_csv = parse_module_price_matrix_csv(
            price_matrix_csv_content, errors_list)
...
def parse_module_price_matrix_csv(
    csv_data: str | io.StringIO, errors_list: list[str]
) -> pd.DataFrame | None:
    if not csv_data:
    ...
    try:
        ...
        # traceback.print_exc() # Auskommentiert für produktiven Code, bei
        # Bedarf für Entwicklung aktivieren
        return None


def parse_module_price_matrix_excel(
    excel_bytes: bytes | None, errors_list: list[str]
) -> pd.DataFrame | None:
    if not excel_bytes:
    ...
    try:
        ...
        for col in df.columns:
            if df[col].dtype == "object":  # String-Spalten vor Konvertierung bereinigen
                # Entferne Tausendertrennzeichen (Punkte), ersetze Dezimalkomma
                # durch Punkt
                df[col] = (
                    df[col]
                    .astype(str)
        ...
        if df.empty:
            ...
            return None
        # print(f"CALCULATIONS: Preis-Matrix erfolgreich aus Excel-Bytes
        # geparst. Shape: {df.shape}") # Bereinigt
        return df
    except ValueError as ve:
        errors_list.append(
            f"Preis-Matrix (Excel): Wert-Fehler beim Parsen: {ve}")
        return None
    except Exception as e:
        errors_list.append(
            f"Preis-Matrix (Excel): Unbekannter Fehler beim Parsen: {e}")
        traceback.print_exc()
        return None
...
def perform_calculations(
    ...
    price_matrix_excel_bytes = real_load_admin_setting(
        "price_matrix_excel_bytes", None)
    price_matrix_csv_content = real_load_admin_setting(
    ...
    # if app_debug_mode_is_enabled: print(f"CALC: Preis-Matrix für Lookup
    # geladen: {results['price_matrix_loaded_successfully']} (Quelle:
    # {results.get('price_matrix_source_type', 'Keine')})") # Bereinigt
    ...
    for i in range(12):
        ...
        # 3. Speicherlogik (vereinfachtes Monatsmodell): Speicher wird nur mit
        # PV-Überschuss geladen
    ...
    if (
        ...
        storage_name_for_matrix_lookup = storage_details_from_db.get(
            "model_name")
    ...
    if (
        ...
        # Finde die passende Zeile in der Matrix (genau oder nächstkleinere
        # Modulanzahl)
        ...
        if not relevant_rows_matrix.empty:
            ...
            if (
                ...
                # Speichere den verwendeten Spaltennamen
                matrix_column_used_for_price = original_price_col_key_for_df_access

            # Fallback auf "Ohne Speicher", wenn spezifischer Speicher nicht
            # gefunden oder Preis ungültig
            if price_value_from_matrix is None or not pd.notna(
                ...
                if (
                    ...
                    # Speichere "Ohne Speicher" als verwendeten Spaltennamen
                    matrix_column_used_for_price = original_no_storage_column_name
            ...
            try:  # Konvertiere den gefundenen Preiswert sicher zu float
                ...
                errors_list.append(
                    (texts.get(
                        ...
                        matrix_price_value=price_value_from_matrix))
    ...
    # if app_debug_mode_is_enabled: print(f"CALC: Ermittelter
    # base_matrix_price_netto: {results['base_matrix_price_netto']:.2f} €
    # (verwendete Matrix-Spalte: '{matrix_column_used_for_price}')") #
    # Bereinigt

    # Logik für Zusatzkosten, abhängig davon, ob ein Pauschalpreis aus der
    ...
    # if app_debug_mode_is_enabled: print(f"CALC: matrix_price_is_pauschal:
    # {matrix_price_is_pauschal}") # Bereinigt
    ...
    if not matrix_price_is_pauschal:  # Nur wenn KEIN Pauschalpreis aus der Matrix gilt
        ...
        cost_accessories_aufpreis_netto = float(
            global_constants.get(
                "additional_components_flat_rate_netto",
                500.0) or 500.0)  # Zubehörpauschale
    ...
    if include_storage and storage_details_from_db:
        ...
        # Bedingung: Speicher ist gewünscht UND (Matrix-Spalte war "Ohne
        # Speicher" ODER es gab gar keinen Matrixpreis)
