    ...
    def _generate_company_pdf(
        ...
        try:
            ...
            if callable(generate_offer_pdf):
                # Vorbereitung der Berechnungsergebnisse - ECHTE DATEN
                # verwenden!
                ...
                if not calc_results:
                    ...
                    calc_results = {
                        # Geschätzt
                        'anlage_kwp': offer_data.get('module_quantity', 20) * 0.4,
                ...
                # KRITISCH: Skalierten Endpreis in project_details UND analysis_results ablegen,
                # damit die PDF-Platzhalter pro Firma unterschiedliche Preise
                # zeigen.
                try:
                    project_details = offer_data.get(
                        "project_details", {}) or {}

                    # Netto-Preis priorisiert aus calc_results
                    # WICHTIG: Zuerst total_investment_netto (wird sicher skaliert),
                    # danach andere Felder als Fallback
                    net_price = (
                        calc_results.get("total_investment_netto")
                        or calc_results.get("final_price_net")
                        or calc_results.get("final_price_netto")
                    )
                    gross_price = (
                        calc_results.get("final_price_brutto")
                        or calc_results.get("total_investment_brutto")
                        or (net_price * 1.19 if isinstance(net_price, (int, float)) else None)
                    )

                    if isinstance(net_price, (int, float)):
                        # In project_details für placeholders.py
                        ...
                        project_details["final_price_net"] = net_price

                        # In calc_results sicherstellen (falls Felder fehlen)
                        ...
                        # Brutto-Preis optional setzen
                        if isinstance(gross_price, (int, float)):
                            project_details["final_price_brutto"] = gross_price
                            calc_results.setdefault(
                                "final_price_brutto", gross_price)

                        # Zurück in offer_data schreiben
                        offer_data["project_details"] = project_details

                        logging.info(
                            f"Multi-Offer Firma {
                                company_index +
                                1}: Preis gesetzt -> netto={
                                net_price:,.2f}€ | brutto={
                                (
                                    gross_price if gross_price else 0):,.2f}€")
                    else:
                        logging.warning(
                            f"Multi-Offer Firma {
                                company_index +
                                1}: Kein skaliertes net_price gefunden in calc_results")
                except Exception as _e_prices:
                    logging.warning(
                        f"Konnte skalierten Preis nicht in project_details/calc_results setzen: {_e_prices}")

                logging.info(
                    f"PDF-Generierung für Firma {company_index + 1}: Preise angepasst")
                # KRITISCH: PDF-kompatible Datenstruktur erstellen
                # Die PDF-Funktion erwartet project_data mit customer_data und
                # project_details
                pdf_project_data = {
                ...
                # Falls ursprüngliche project_data vorhanden, deren Struktur
                # beibehalten
