...
# kg CO2 pro kWh aus dem deutschen Strommix (Wert für 2023)
CO2_EMISSIONS_GRID_KWH = 0.401
CO2_EMBODIED_PV_KWP = 1500  # "Graues" CO2 für Herstellung/Transport pro kWp PV


def calculate_dynamic_payback_period(
        investment: float,
        initial_annual_savings: float,
        price_increase_percent: float) -> float:
    ...
    while cumulative_savings < investment:
        ...
        years += 1
        if years > 50:
            return float('inf')  # Sicherheitsabbruch
    # Feinjustierung für unterjährige Amortisation
    return years - (cumulative_savings - investment) / \
        (current_annual_savings / (1 + price_increase_percent / 100))
...
def calculate_self_consumption_quote(
...
def calculate_total_yield_over_lifetime(
        annual_yield: float,
...
def calculate_alternative_investment_value(
        investment: float,
        interest_rate: float,
        lifetime_years: int) -> float:
...
def calculate_performance_ratio(
    ...
    # Annahme: 'global_radiation' ist die Einstrahlung auf die Modulfläche
    # (kWh).
...
def calculate_area_specific_yield(
        annual_yield_kwh: float,
...
def calculate_pv_module_efficiency(
...
def calculate_shading_loss(
        yield_with_shading: float,
        optimal_yield: float) -> float:
...
def calculate_temperature_corrected_power(
        p_nominal: float,
        temp_coefficient_percent: float,
        temperature: float,
        ref_temp: float = 25.0) -> float:
    """27. Temperaturkorrektur des PV-Ertrags """
    return p_nominal * (1 + (temp_coefficient_percent / 100)
                        * (temperature - ref_temp))


def calculate_degradation_yield(
        initial_yield: float,
...
def calculate_total_maintenance_costs(
...
def calculate_self_consumption_increase_with_storage(
        old_self_consumption_kwh: float,
        additional_kwh_from_storage: float) -> float:
...
def calculate_optimal_storage_size(
...
def calculate_load_shifting_potential(
        controllable_load_kwh: float,
...
def compare_pv_vs_balcony(
        costs_pv: float,
        yield_pv: float,
        costs_balcony: float,
        yield_balcony: float) -> dict:
...
def calculate_yield_after_inverter_degradation(
        initial_yield: float,
        degradation_rate_percent: float,
        years: int) -> float:
...
def calculate_battery_lifespan_years(
        max_cycles: int,
        cycles_per_year: int) -> float:
...
def simulate_ev_charging_profile(
        pv_surplus_kwh: float,
        ev_demand_kwh: float,
        charging_efficiency_percent: float = 90.0) -> float:
    """44. Ladeprofil-Simulation für E-Auto (geladene PV-Energie) """
    return min(pv_surplus_kwh, ev_demand_kwh) * \
        (charging_efficiency_percent / 100)


def calculate_cumulative_co2_savings(
...
def calculate_value_after_inflation(
        value: float,
...
def calculate_peak_shaving_effect(
        max_load_kw: float,
        optimized_load_kw: float) -> float:
...
def calculate_lcoe(investment: float, annual_production_kwh: float) -> float:
    ...
    annuity_factor = (DISCOUNT_RATE * (1 + DISCOUNT_RATE) **
                      LIFESPAN_YEARS) / ((1 + DISCOUNT_RATE)**LIFESPAN_YEARS - 1)
    annualized_investment = investment * annuity_factor
    return (annualized_investment / annual_production_kwh) * \
        100  # Umrechnung in Cent
...
def calculate_co2_avoidance_per_year(annual_production_kwh: float) -> float:
    ...
    return (annual_production_kwh * CO2_EMISSIONS_GRID_KWH) / \
        1000  # Umrechnung in Tonnen


def calculate_energy_payback_time(
        total_embodied_energy_kwh: float,
...
def calculate_profit_after_x_years(
        investment: float,
        annual_savings: float,
        years: int) -> float:
...
def run_all_extended_analyses(offer_data: dict[str, Any]) -> dict[str, Any]:
    ...
    # Annahme: Graue Energie für Speicher/Wallbox wird hier vereinfacht
    # hinzugerechnet
