                    ...
                    if validation_result['valid']:
                        ...
                        if validation_result['type'] == 'formula':
                            ...
                            if circular_check['has_circular']:
                                st.error(f"Zirkelbezug erkannt: {circular_check['message']}")
                        ...
                        if validation_result.get('warning'):
                            st.warning(f"{validation_result['warning']}")
                        ...
                        st.error(f"Validierungsfehler: {validation_result['error']}")
...
def _render_grid():
    ...
    with col3:
        ...
        if st.button("Zeile löschen", use_container_width=True, help="Löscht die angegebene Zeile und passt Formeln an"):
            if info['rows'] > 1:
    ...
    with col4:
        ...
        if st.button("Spalte löschen", use_container_width=True, help="Löscht die angegebene Spalte und passt Formeln an"):
            if info['columns'] > 1:
    ...
    if error_cells:
        st.markdown("---")
        st.warning(f"{len(error_cells)} Zelle(n) mit Fehlern")
...
def _render_csv_import_dialog():
    ...
    if uploaded_file is not None:
        try:
            ...
            if validation['valid']:
                ...
                if validation['has_formulas']:
                    st.info("Die Datei enthält Formeln (beginnen mit =)")
                ...
                st.error("CSV-Datei ist ungültig")
        ...
        # Hilfe-Text
        with st.expander("Hilfe zum CSV-Import"):
            st.markdown("""
...
def render_excel_grid_ui():
    ...
    with st.expander("Was ist die Excel Preis Matrix?", expanded=False):
        st.markdown("""
    ...
    with st.expander(" Hilfe & Anleitung", expanded=False):
        ...
        with col2:
            ...
            **Tipp**: Detaillierte Hilfe finden Sie ganz unten unter "Hilfe & Tastenkombinationen"
            """)
...
def _render_excel_export_dialog():
    ...
    if info['formula_count'] > 0:
        st.info(f"Die Matrix enthält {info['formula_count']} Formel(n)")
...
def _render_export_info_dialog():
    ...
    try:
        ...
        if export_info['has_errors']:
            st.warning(f"{export_info['error_count']} Zelle(n) enthalten Fehler")
        else:
...
def _render_validation_dialog():
    ...
    try:
        ...
        if info:
            with st.expander("Matrix-Informationen", expanded=False):
