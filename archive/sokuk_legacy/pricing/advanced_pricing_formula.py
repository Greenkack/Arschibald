...
class AdvancedPricingFormula:
    ...
    def _calculate_standard_formula(
        ...
        if total_pct_discount > 0:
            discount_multiplier = 1 - (total_pct_discount / 100)
            new_amount = current_amount * discount_multiplier
            steps.append(PricingStep(
                step_number=len(steps) + 1,
                step_name="Apply Percentage Discounts",
                input_amount=current_amount,
                operation="MULTIPLY",
                operation_value=discount_multiplier,
                output_amount=new_amount,
                description=f"Apply {total_pct_discount}% discount: {current_amount} × {discount_multiplier}",
                formula_part=f"× (1 - {total_pct_discount / 100})"
            ))
        ...
        if total_pct_surcharge > 0:
            surcharge_multiplier = 1 + (total_pct_surcharge / 100)
            new_amount = current_amount * surcharge_multiplier
            steps.append(PricingStep(
                step_number=len(steps) + 1,
                step_name="Apply Percentage Surcharges",
                input_amount=current_amount,
                operation="MULTIPLY",
                operation_value=surcharge_multiplier,
                output_amount=new_amount,
                description=f"Apply {total_pct_surcharge}% surcharge: {current_amount} × {surcharge_multiplier}",
                formula_part=f"× (1 + {total_pct_surcharge / 100})"
            ))
            current_amount = new_amount
        ...
        # Calculate totals
        total_savings = (
            (total_pct_discount / 100 * base_with_accessories) +
            total_fixed_discount
        )
        total_additions = (
            accessories_cost +
            (total_pct_surcharge / 100 * (base_with_accessories - total_savings)) +
