...
def render_pdf_ui(
    ...
    with st.sidebar:
        ...
        total_aufpreise_zuschlaege = surcharge_amount + \
        ...
        st.write(
            f"**Summe Rabatte / Nachlässe:** -{total_rabatte_nachlaesse:,.2f} €")
        st.write(
            f"**Summe Aufpreise / Zuschläge:** +{total_aufpreise_zuschlaege:,.2f} €")
        st.write(
            f"**Endpreis (inkl. Zuschläge, Rabatte, usw.):** {final_price:,.2f} €")
    ...
    with ext_tab6:
        ...
        st.caption(
            "Konfigurieren Sie die Zahlungsbedingungen, die auf Seite 7 des PDFs erscheinen sollen.")
        ...
        try:
            ...
            try:
                ...
                if isinstance(project_data,
                              dict) and 'total_price' in project_data:
    ...
    with st.form(key="pdf_generation_form_v13_final_locked_options_main", clear_on_submit=False):
        ...
        col_pdf_content1_form, col_pdf_content2_form, col_pdf_content3_form = st.columns(
            3)
        with col_pdf_content1_form:
            # NEU: Klarer Hauptschalter für erweiterte PDF-Ausgabe (steuert
            # Zusatzseiten ab Seite 8)
            st.session_state.pdf_inclusion_options["append_additional_pages_after_main7"] = st.checkbox(
                get_text_pdf_ui(
                    texts,
                    "pdf_append_after_main7_label",
                    "Erweiterte PDF-Ausgabe (ab Seite 8) aktivieren?"),
                value=st.session_state.pdf_inclusion_options.get(
                    "append_additional_pages_after_main7",
                    False),
                key="pdf_cb_append_after_main7_v1")
            append_after_main7_flag = bool(
                st.session_state.pdf_inclusion_options.get(
                    "append_additional_pages_after_main7", False))

            # Zusatzoptionen für Seiten ab 8 nur anzeigen, wenn der Schalter
            # aktiv ist
            if append_after_main7_flag:
                st.caption(
                    "Erweiterter Modus aktiv: Die 7 festen Hauptseiten werden erzeugt und zusätzliche Angebotsseiten ab Seite 8 angehängt.")
                ...
                if active_company_id_for_docs is not None and isinstance(
                        active_company_id_for_docs, int) and callable(db_list_company_documents_func):
                    company_docs_list_form = db_list_company_documents_func(
                        active_company_id_for_docs, None)
                    if company_docs_list_form:
                        current_selected_doc_ids_form = st.session_state.pdf_inclusion_options.get(
                            "company_document_ids_to_include", [])
                        selected_doc_ids_in_form = []
                        for doc_item_form in company_docs_list_form:
                            if isinstance(
                                    doc_item_form, dict) and 'id' in doc_item_form:
                                doc_id_item_form = doc_item_form['id']
                                doc_label_item_form = f"{
                                    doc_item_form.get(
                                        'display_name', doc_item_form.get(
                                            'file_name', 'Unbenannt'))} ({
                                    doc_item_form.get('document_type')})"
                                if st.checkbox(
                                        doc_label_item_form,
                                        value=(
                                            doc_id_item_form in current_selected_doc_ids_form),
                                        key=f"pdf_cb_company_doc_form_{doc_id_item_form}_v13_stable"):
                                    selected_doc_ids_in_form.append(
                                        doc_id_item_form)
                        st.session_state.pdf_inclusion_options[
                            "_temp_company_document_ids_to_include"] = selected_doc_ids_in_form
                    else:
                        st.caption(
                            get_text_pdf_ui(
                                texts,
                ...
                st.info(
                    get_text_pdf_ui(
                        texts,
                        "pdf_only_main7_active_info",
                        "Standardmodus: Es werden nur die 7 Hauptseiten erzeugt. Erweiterte PDF-Ausgabe ist deaktiviert."))
            ...
            if append_after_main7_flag:
                st.markdown(
                    ...
                    get_text_pdf_ui(
                        ...
                        "Hauptsektionen im Angebot (Zusatzseiten)") +
                    "**")
            ...
            if append_after_main7_flag:
                st.markdown(
                    ...
                    get_text_pdf_ui(
                        ...
                        "Diagramme & Visualisierungen (Zusatzseiten)") +
                    "**")
                if analysis_results and isinstance(analysis_results, dict):
                    available_chart_keys_form = _get_all_available_chart_keys(
                        analysis_results, chart_key_to_friendly_name_map)
                    ordered_display_keys_form = [
                        k_map for k_map in chart_key_to_friendly_name_map if k_map in available_chart_keys_form]
                    ...
                    current_selected_charts_in_state_form = st.session_state.pdf_inclusion_options.get(
                        "selected_charts_for_pdf", [])
                    ...
                    for chart_key_form in ordered_display_keys_form:
                        friendly_name_form = chart_key_to_friendly_name_map.get(
                            chart_key_form, chart_key_form.replace(
                                '_chart_bytes', '').replace(
                                '_', ' ').title())
                        if st.checkbox(
                                friendly_name_form,
                                value=(
                                    chart_key_form in current_selected_charts_in_state_form),
        ...
        if submitted_generate_pdf:  # Werte aus temporären Keys in die Haupt-Session-State-Keys übernehmen
            append_after_main7_flag_submit = bool(
                st.session_state.pdf_inclusion_options.get(
                    "append_additional_pages_after_main7", False))
            if append_after_main7_flag_submit:
                st.session_state.pdf_inclusion_options["company_document_ids_to_include"] = st.session_state.pdf_inclusion_options.pop(
                ...
                st.session_state.pdf_inclusion_options["selected_charts_for_pdf"] = st.session_state.pdf_inclusion_options.pop(
                    "_temp_selected_charts_for_pdf", [])
            else:
                # Zusatzauswahlen zurücksetzen, wenn Zusatzseiten deaktiviert
                # sind
                st.session_state.pdf_inclusion_options.pop(
                    "_temp_company_document_ids_to_include", None)
                st.session_state.pop("_temp_pdf_selected_main_sections", None)
                st.session_state.pdf_inclusion_options.pop(
                    "_temp_selected_charts_for_pdf", None)
                st.session_state.pdf_inclusion_options["company_document_ids_to_include"] = [
                ]
                st.session_state.pdf_selected_main_sections = []
                st.session_state.pdf_inclusion_options["selected_charts_for_pdf"] = [
    ...
    with st.expander(" Debug: PDF-Anhänge-Prüfung", expanded=False):
        ...
        product_ids = [
            ...
            ('Batteriespeicher', project_details.get('selected_storage_id')
             if project_details.get('include_storage') else None)
        ]
...
def prepare_payment_data_for_pdf_generation(
    ...
    try:
        ...
        position = st.session_state.get(
            'payment_position', 'Seite 7 (Standard)')
