...
def format_payment_text_with_placeholders(
    ...
    calculated_amounts = []
    for i, (percent, fixed_amount) in enumerate(
            zip(percents, amounts, strict=False)):
        if fixed_amount > 0:
    ...
    for i in range(3):
        ...
        replacements[f"b{i + 1}"] = calculated_amounts[i] if i < len(
            ...
            if not include_amounts_in_text:
                ...
                preview_text = format_payment_text_with_placeholders(
                    selected_variant.get(
                        ...
                        '({b1} €)', '').replace(
                        '({b2} €)', '').replace(
                        '({b3} €)', ''), selected_variant.get(
                        'percents', []), [
                            0.0, 0.0, 0.0], example_total, selected_variant.get('custom_labels'))
...
def get_payment_variant_for_pdf_generation(
    ...
    try:
        ...
        for i, percent in enumerate(percents):
            if percent > 0:
                # Verwende festen Betrag falls definiert, sonst berechne aus
                # Prozent
                if amounts and len(amounts) > i and amounts[i] > 0:
                    amount = amounts[i]
                else:
