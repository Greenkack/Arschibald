...
try:
    ...
    pytest.skip(
        "PDF pricing templates modules not available",
...
class TestPVPricingTemplate:
    ...
    def test_pv_template_structure(self):
        ...
        components_section = next(
            s for s in sections if s["name"] == "components")
...
class TestHeatPumpPricingTemplate:
    ...
    def test_heatpump_template_structure(self):
        ...
        subsidies_section = next(
            s for s in sections if s["name"] == "subsidies")
...
class TestCombinedPricingTemplate:
    ...
    def test_combined_template_structure(self):
        ...
        synergies_section = next(
            s for s in sections if s["name"] == "synergies")
...
class TestPricingTemplateManager:
    ...
    def test_find_pricing_value(self):
        ...
        value2 = self.manager._find_pricing_value(
            'PV_MODULES_TOTAL', pricing_data)
        assert value2 == 3600.0

        # Test non-existent key
        value3 = self.manager._find_pricing_value(
            'NONEXISTENT_KEY', pricing_data)
...
class TestTemplateIntegration:
    ...
    def test_combined_template_includes_all_systems(self):
        ...
        combined_keys = [
            k for k in rendered.keys() if k.startswith('COMBINED_')]
