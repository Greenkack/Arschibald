...
def render_data_input(texts: dict[str, str]) -> None:
    ...
    # Initialize demand mode in session state if not exists
    if 'demand_mode_selection' not in st.session_state:
        st.session_state.demand_mode_selection = None
    ...
    with col1:
        if st.button(
            "Photovoltaik",
            key="pv_only_mode_btn",
                use_container_width=True):
            st.session_state.demand_mode_selection = "Nur Photovoltaik"
            st.success("Nur Photovoltaik-Modus ausgewählt")
            st.rerun()

    with col2:
        if st.button(
            "Wärmepumpe",
            key="wp_only_mode_btn",
                use_container_width=True):
            st.session_state.demand_mode_selection = "Nur Wärmepumpe"
            st.success("Nur Wärmepumpe-Modus ausgewählt")
            st.rerun()

    with col3:
        if st.button(
            "Photovoltaik + Wärmepumpe",
            key="pv_wp_combined_mode_btn",
                use_container_width=True):
            st.session_state.demand_mode_selection = "pv_wp_combined"
            st.success("Photovoltaik + Wärmepumpe-Modus ausgewählt")
            st.rerun()

    # Display current selection
    current_mode = st.session_state.get('demand_mode_selection', None)
    if current_mode:
        mode_names = {
            'Photovoltaik_only': 'Nur Photovoltaik',
            'wp_only': 'Nur Wärmepumpe',
            'pv_wp_combined': 'Photovoltaik + Wärmepumpe'
        }
        st.info(
            f"**Aktueller Modus:** {mode_names.get(current_mode, current_mode)}")
    else:
        st.warning("Bitte wählen Sie einen Bedarfsanalyse-Modus aus")

    # Informative mode requirements (not blocking)
    if current_mode:
        mode_info = []
        if current_mode in ['Nur Photovoltaik', 'Photovoltaik + Wärmepumpe']:
            mode_info.append(
