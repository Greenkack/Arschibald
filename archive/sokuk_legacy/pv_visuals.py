...
def render_yearly_production_pv_data(
    ...
    st.subheader(
        get_text_pv_viz(
            texts,
            "viz_yearly_prod_3d_subheader",
            "Jahresproduktion – 3D-Monatsbalken"))
    ...
    if not production_data or not isinstance(
        ...
        st.warning(
            get_text_pv_viz(
                ...
                "Monatliche Produktionsdaten für 3D-Jahresdiagramm nicht verfügbar oder unvollständig."))
    ...
    for i, p_val_raw in enumerate(production_data):
        p_val = float(p_val_raw) if isinstance(
            p_val_raw, (int, float)) and not (
            math.isnan(p_val_raw) or math.isinf(p_val_raw)) else 0.0
...
def render_break_even_pv_data(
    ...
    st.subheader(
        get_text_pv_viz(
            texts,
            "viz_break_even_3d_subheader",
            "Break-Even Punkt – Kapitalfluss in 3D"))
    ...
    fig_break_even.update_layout(
        title=get_text_pv_viz(
            texts,
            "viz_break_even_3d_title",
            "Kumulierter Kapitalfluss über die Laufzeit"),
        scene=dict(
            xaxis_title=get_text_pv_viz(
                texts,
                "viz_year_axis_label",
                "Jahr"),
            yaxis_title='',
...
def render_amortisation_pv_data(
    ...
    st.subheader(
        get_text_pv_viz(
            texts,
            "viz_amortisation_3d_subheader",
            "Amortisation – Rückflusskurve in 3D"))
    simulation_years = analysis_results.get(
        'simulation_period_years_effective', 0)
    ...
    fig_amort.add_trace(
        go.Scatter3d(
            x=years_axis,
            y=[0.1] * len(years_axis),
            z=kumulierte_rueckfluesse,
            mode='lines+markers',
            name=get_text_pv_viz(
                texts,
...
def render_co2_savings_visualization(
    ...
    if trees_equiv > 0:
        ...
        # Max 20 Bäume für Übersichtlichkeit
        tree_count = min(int(trees_equiv), 20)
    ...
    if car_km_equiv > 0:
        ...
        # Räder
        wheel_positions = [(8.2, -1.2, 0.3), (8.2, 1.2, 0.3),
                           (9.8, -1.2, 0.3), (9.8, 1.2, 0.3)]
        ...
        fig_co2.add_trace(
            go.Scatter3d(
                ...
                marker=dict(
                    ...
                    symbol='diamond'),
                name=f'{
                    airplane_km_equiv:.0f} km Flugzeug',
                hovertemplate=f'Vermiedene Flugkilometer: {
                    airplane_km_equiv:.0f} km<extra></extra>'))
    ...
    fig_co2.update_layout(
        title=dict(
            text=get_text_pv_viz(
                texts,
                "co2_3d_title",
                "Ihre CO₂-Einsparung in anschaulichen Vergleichen"),
            font=dict(
                size=18,
                color='darkgreen')),
        scene=dict(
    ...
    st.plotly_chart(
        fig_co2,
        use_container_width=True,
        key="co2_savings_3d_viz")

    # Export für PDF
    analysis_results['co2_savings_chart_bytes'] = _export_plotly_fig_to_bytes_pv_viz(
        fig_co2, texts)
