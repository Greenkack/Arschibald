...
def generate_offer_pdf_with_main_templates(
    ...
    **kwargs,
    ...
    additional_pdf = None
    try:
        append_after_main7 = bool((inclusion_options or {}).get(
    ...
    if append_after_main7:
        ...
        try:
            if additional_pdf and _PYPDF_AVAILABLE:
                ...
                print(
                    f"[PDF EXTENDED] Zusatz-PDF erzeugt: {_dbg_pages} Seiten (append_after_main7=True)")
            else:
                print(
    ...
    try:
        ...
        if additional_pdf:
            ...
            # Zusatzseiten mit Footer versehen: Startnummer = 8 (da Seiten 1-7
            # schon vorhanden)
            logo_b64 = None
            try:
...
def generate_offer_pdf(
    ...
    product_ids_for_datasheets = list(filter(None, [
        ...
        pv_details_pdf.get("selected_storage_id") if pv_details_pdf.get(
            "include_storage") else None
    ]))
    # Nur wenn Zubehör überhaupt aktiv ist
    if pv_details_pdf.get('include_additional_components', True):
        for opt_id_key in [
            'selected_wallbox_id',
            'selected_ems_id',
