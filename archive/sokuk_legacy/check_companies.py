...
if os.path.exists(DB_PATH):
    ...
    try:
        ...
        if 'company_documents' in [table[0] for table in tables]:
            ...
            if doc_count > 0:
                cursor.execute(
                    'SELECT company_id, document_type, display_name FROM company_documents LIMIT 5')
        ...
        if 'pdf_templates' in [table[0] for table in tables]:
            ...
            if template_count > 0:
                cursor.execute(
                    'SELECT template_type, name FROM pdf_templates LIMIT 5')
        ...
        if settings_count > 0:
            cursor.execute(
                "SELECT key FROM admin_settings WHERE key LIKE '%template%'")
