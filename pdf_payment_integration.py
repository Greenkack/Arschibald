...
class EnhancedPaymentPDFGenerator:
    ...
    def create_comprehensive_payment_page(
        ...
        if "error" in payment_calculation:
            return self._create_fallback_payment_page(
                analysis_results, payment_option_id)
    ...
    def _draw_cost_breakdown(self,
                             c: canvas.Canvas,
                             analysis_results: dict[str,
                                                    Any],
                             payment_calculation: dict[str,
                                                       Any]):
        ...
        c.setFont("Helvetica", 10)
        c.drawString(70, y_pos, "Angebotssumme (brutto):")
        c.drawRightString(width - 50, y_pos,
        ...
        if total_savings > 0:
            ...
            c.drawString(
                70, y_pos, f"Ihre Ersparnis: {
                    total_savings:,.2f}€ ({
                    savings_percent:.1f}%)".replace(
                    ",", "."))

    def _draw_payment_details(self,
                              c: canvas.Canvas,
                              payment_calculation: dict[str,
                                                        Any],
                              total_amount: float):
        ...
        c.setFont("Helvetica", 10)
        c.drawString(
            70, y_pos, f"Gewählte Zahlungsart: {
                payment_option.get(
                    'name', '')}")
        y_pos -= 20
    ...
    def _draw_immediate_payment_details(
            self, c: canvas.Canvas, payment_calculation: dict[str, Any], start_y: float):
        ...
        c.setFont("Helvetica", 10)
        c.drawString(
            70, start_y, f"Zahlungsbetrag: {
                final_amount:,.2f}€".replace(
                ",", "."))
        start_y -= 15
        c.drawString(70, start_y, "Fällig: Bei Vertragsabschluss")
        start_y -= 15
        ...
        if payment_option.get("discount_percent", 0) > 0:
            ...
            c.drawString(
                70, start_y, f"Inkl. {
                    payment_option['discount_percent']:.1f}% Zahlungsrabatt")

    def _draw_installment_details(
            self, c: canvas.Canvas, payment_calculation: dict[str, Any], start_y: float):
    ...
    def _draw_financing_details(
        ...
        for option in financing_options:
            ...
            c.drawString(
                90, start_y, f"Laufzeit {years} Jahre ({
                    rate:.1f}% p.a.):")
            c.drawRightString(
                500, start_y, f"~{monthly_payment:,.2f}€/Monat".replace(",", "."))
            start_y -= 15

    def _draw_leasing_details(self,
                              c: canvas.Canvas,
                              payment_calculation: dict[str,
                                                        Any],
                              start_y: float):
        ...
        for option in leasing_options:
            ...
            c.drawRightString(
                500, start_y, f"{monthly_rate:,.2f}€/Monat".replace(",", "."))
            start_y -= 15

    def _draw_discount_summary(
            self, c: canvas.Canvas, payment_calculation: dict[str, Any]):
    ...
    def _create_fallback_payment_page(self, analysis_results: dict[str, Any],
        ...
        c.setFont("Helvetica", 10)
        c.drawString(
            50, y_pos, f"Gesamtbetrag: {
                total_amount:,.2f}€".replace(
                ",", "."))
        y_pos -= 30
...
def generate_comprehensive_offer_pdf(
    ...
    try:
        ...
        if payment_page:
            # PDFs kombinieren (vereinfacht - in Produktionsumgebung PyPDF2
            # verwenden)
...
def create_payment_summary_page(
    ...
    generator = EnhancedPaymentPDFGenerator()
    return generator._create_fallback_payment_page(
