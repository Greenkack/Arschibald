...
def scan_formulas_openpyxl(
    ...
    """Scannt Excel-Formeln mit openpyxl"""
    results = []
    try:
            ...
            for row in sheet.iter_rows():
                for cell in row:
                    if cell.value and isinstance(cell.value, str) and cell.value.startswith('='):
                        results.append((sheet_name, cell.coordinate, cell.value))
...
class ExcelEngineXlwings:
    ...
    def __init__(self, path: str | Path):
        ...
        try:
            self._wb = self._app.books.open(
                str(path), read_only=True, update_links=False)
        except Exception:
...
class ExcelEngineHeadless:
    ...
    def __init__(self, path: str | Path):
        from xlcalculator import Model, ModelCompiler
        comp = ModelCompiler()
...
def safety_check_parity(
    ...
    for er, hr in zip(excel_rows, head_rows, strict=False):
    ...
    if out_report_csv:
        ...
        with out_report_csv.open("w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(["ref", "excel_value", "headless_value",
                       "abs_delta", "rel_delta", "note"])
            for m in mismatches:
                w.writerow([m.ref, m.excel_value, m.headless_value,
                           m.abs_delta, m.rel_delta, m.note])
...
def main_cli():
    ...
    if args.cmd == "export":
        if args.fmt == "csv":
            out = export_formulas_to_csv(
                args.infile,
                args.outfile,
                limit=args.limit,
                engine=args.engine)
        else:
            out = export_formulas_to_xlsx(
                args.infile,
                args.outfile,
                limit=args.limit,
                engine=args.engine)
        ...
        res = batch_export(
            ...
            engine=args.engine,
            to=args.to)
        _print_json({"count": len(res), "outputs": [
                    (str(i), str(o)) for i, o in res]})
